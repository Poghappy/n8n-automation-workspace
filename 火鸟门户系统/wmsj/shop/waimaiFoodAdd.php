...
if($_POST){
    ...
    if($id){
        ...
        $sql = $dsql->SetQuery("SELECT `id`, `price` FROM `#@__$dbname` WHERE `id` = $id");
    ...
    $businessChangePrice = (int)$custom_businessChangePrice;  //0不需要审核  1需要审核
    ...
    if($id){
        ...
        //加价需要审核
        if($businessChangePrice == 1 && $price > $_price){
            $review_price = $price;
            $price = '`price`';
        }
        ...
        if($ret == "ok"){
            ...
            //记录用户行为日志
            memberLog($userid, 'waimai', 'detail', $id, 'update', '修改商品('.$title.')', '', $sql);
        ...
        //初始化日志
	    $_waimaiFoodEditLog = new CLogFileHandler(HUONIAOROOT . '/log/waimaiFoodEdit/'.date('Y-m-d').'.log', true);
    ...
    //新增
		...
		if($aid){
            ...
            //记录用户行为日志
            memberLog($userid, 'waimai', 'detail', $aid, 'insert', '添加商品('.$title.')', '', $sql);
...
if(file_exists($tpl."/".$templates)){
  ...
  if($cfg_remoteStatic){
    ...
    $sql = $dsql->SetQuery("SELECT * FROM `#@__waimai_list_type` WHERE `sid` = $sid AND `del` = 0 ORDER BY `sort` DESC, `id` DESC");
