...
if($userType == 3){
	//查询信息资讯
	$sql = $dsql->SetQuery("SELECT `id` FROM `#@__articlelist_all` WHERE `arcrank` = 1" . getCityFilter('`cityid`'));
...
if($dopost == "getDetail"){
	...
	if(count($results) > 0){
		...
		if($results[0]["userid"] == 0 || $results[0]["userid"] == -1){
			...
			$archives = $dsql->SetQuery("SELECT `username`, `nickname` FROM `#@__member` WHERE `id` = ".$results[0]["userid"]);
			$member = $dsql->dsqlOper($archives, "results");
			$username = $member[0]["nickname"] ? $member[0]["nickname"] : $member[0]["username"];
		}
...
//更新评论信息
	...
	//会员通知
	$sql = $dsql->SetQuery("SELECT l.`id`, l.`title`, l.`pubdate`, l.`admin` uid, c.`userid`, c.`ischeck`, c.`dtime`,c.`rid`,c.`pid` FROM `#@__".$action."_comment_all` c LEFT JOIN `#@__articlelist_all` l ON l.`id` = c.`aid` WHERE c.`id` = " . $id . $where);
	$ret = $dsql->dsqlOper($sql, "results");
	if($ret){
		$aid     = $ret[0]['id'];
		$title   = $ret[0]['title'];
		...
		if($ischeck_ != $isCheck){
			...
			if($isCheck == 1){
                ...
                $sql = $dsql->SetQuery("SELECT `username`,`nickname`,`point` FROM `#@__member` WHERE `id` = $userid");
				$ret = $dsql->dsqlOper($sql, "results");
				if($ret){
...
//更新评论状态
	...
	foreach($each as $val){
		...
		//会员通知
		$sql = $dsql->SetQuery("SELECT l.`id`, l.`title`, l.`pubdate`, l.`admin` uid, c.`userid`, c.`ischeck`, c.`dtime`,c.`rid`,c.`pid` FROM `#@__".$action."_comment_all` c LEFT JOIN `#@__articlelist_all` l ON l.`id` = c.`aid` WHERE c.`id` = " . $val.$where);
		$ret = $dsql->dsqlOper($sql, "results");
		if($ret){
			$aid     = $ret[0]['id'];
			$title   = $ret[0]['title'];
...
//获取评论列表
	...
	if ($adminCity){
		$sql = $dsql->SetQuery("SELECT `id` FROM `#@__articlelist_all` WHERE `arcrank` = 1" . getWrongCityFilter('`cityid`', $adminCity));
		$ret = $dsql->dsqlOper($sql, "results");
		if($ret){
			foreach ($ret as $key => $value) {
				array_push($ids, $value['id']);
	...
	if($sKeyword != ""){
		...
		//按评论人搜索
			...
			if($sKeyword == "游客"){
				...
				$archives = $dsql->SetQuery("SELECT `id` FROM `#@__member` WHERE (`username` like '%$sKeyword%' OR `nickname` like '%$sKeyword%' OR `phone` like '%$sKeyword%')");
				$results = $dsql->dsqlOper($archives, "results");
	...
	if(count($results) > 0){
		...
		foreach ($results as $key=>$value) {
			...
			$member = $dsql->SetQuery("SELECT `username`, `nickname` FROM `#@__member` WHERE `id` = ".$value["userid"]);
			$username = $dsql->dsqlOper($member, "results");
			$list[$key]["username"]  = $username[0]["username"] == null ? "游客" : ($username[0]["nickname"] ? $username[0]["nickname"] : $username[0]["username"]);
