...
//删除
	...
	if($id != ""){
        ...
        dataAsync("travel",$async,"daytravel");  // 删除旅游
        dataAsync("travel",$async,"grouptravel");  // 删除旅游
		if(!empty($error)){
...
//更新状态
	...
	if($id != ""){
		...
		dataAsync("travel",$async,"daytravel");  // 更新旅游状态
		dataAsync("travel",$async,"grouptravel");  // 更新旅游状态
		if(!empty($error)){
			echo '{"state": 200, "info": '.json_encode($error).'}';
		}else{
			checktravelagencyCache($id);
