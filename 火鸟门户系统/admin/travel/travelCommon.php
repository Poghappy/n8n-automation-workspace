...
if($userType == 3){
	//查询信息
	if($typeid == 2){
		$sql = $dsql->SetQuery("SELECT `id` FROM `#@__travel_ticket` WHERE `state` = 1" . getCityFilter('`cityid`'));
	}elseif($typeid == 0){
		...
		$sql = $dsql->SetQuery("SELECT `id` FROM `#@__travel_agency` WHERE `state` = 1" . getCityFilter('`cityid`'));
	}elseif($typeid == 4){
		$comSql = $dsql->SetQuery("SELECT `id` FROM `#@__travel_store` WHERE `state` = 1" . getCityFilter('`cityid`'));
		...
		$sql = $dsql->SetQuery("SELECT `id` FROM `#@__travel_hotel` WHERE `state` = 1" . getCityFilter('`cityid`'));
	}
...
if($dopost == "getDetail"){
	...
	$archives = $dsql->SetQuery("SELECT `aid`, `userid`, `content`, `dtime`, `ip`, `ipaddr`, `rating`, `ischeck`, `sco1`, `pics` FROM `#@__".$action."_comment_all` WHERE `id` = ".$id.$where);
	...
	if(count($results) > 0){
		...
		if($typeid == 2){
			...
			$typeSql = $dsql->SetQuery("SELECT p.`title` FROM `#@__travel_hotel` p WHERE p.`id` = ". $results[0]["aid"]);
		}
...
//更新评论信息
	...
	//会员通知
	if($typeid == 2){
		$sql = $dsql->SetQuery("SELECT l.`id`, l.`title`, l.`pubdate`, l.`userid` uid, c.`userid`, c.`ischeck`, c.`dtime`,c.`rid`,c.`pid` FROM `#@__".$action."_comment_all` c LEFT JOIN `#@__travel_ticket` l ON l.`id` = c.`aid` WHERE c.`id` = " . $id . $where);
	}elseif($typeid == 0){
		$sql = $dsql->SetQuery("SELECT l.`id`, l.`title`, l.`pubdate`, l.`userid` uid, c.`userid`, c.`ischeck`, c.`dtime`,c.`rid`,c.`pid` FROM `#@__".$action."_comment_all` c LEFT JOIN `#@__travel_video` l ON l.`id` = c.`aid` WHERE c.`id` = " . $id . $where);
	}elseif($typeid == 1){
		$sql = $dsql->SetQuery("SELECT l.`id`, l.`title`, l.`pubdate`, l.`userid` uid, c.`userid`, c.`ischeck`, c.`dtime`,c.`rid`,c.`pid` FROM `#@__".$action."_comment_all` c LEFT JOIN `#@__travel_strategy` l ON l.`id` = c.`aid` WHERE c.`id` = " . $id . $where);
	}elseif($typeid == 3){
		$sql = $dsql->SetQuery("SELECT l.`id`, l.`title`, l.`pubdate`, l.`userid` uid, c.`userid`, c.`ischeck`, c.`dtime`,c.`rid`,c.`pid` FROM `#@__".$action."_comment_all` c LEFT JOIN `#@__travel_agency` l ON l.`id` = c.`aid` WHERE c.`id` = " . $id . $where);
	}elseif($typeid == 4){
		$sql = $dsql->SetQuery("SELECT l.`id`, l.`title`, l.`pubdate`, l.`userid` uid, c.`userid`, c.`ischeck`, c.`dtime`,c.`rid`,c.`pid` FROM `#@__".$action."_comment_all` c LEFT JOIN `#@__travel_visa` l ON l.`id` = c.`aid` WHERE c.`id` = " . $id . $where);
	}elseif($typeid == 5){
		$sql = $dsql->SetQuery("SELECT l.`id`, l.`title`, l.`pubdate`, l.`userid` uid, c.`userid`, c.`ischeck`, c.`dtime`,c.`rid`,c.`pid` FROM `#@__".$action."_comment_all` c LEFT JOIN `#@__travel_hotel` l ON l.`id` = c.`aid` WHERE c.`id` = " . $id . $where);
	}
	$ret = $dsql->dsqlOper($sql, "results");
	if($ret){
		$aid     = $ret[0]['id'];
	...
	$archives = $dsql->SetQuery("UPDATE `#@__".$action."_comment_all` SET `content` = '$content', `dtime` = '$dtime', `ip` = '$ip', `ischeck` = '$isCheck', `sco1` = '$rating', `pics`='$pics' WHERE `id` = ".$id.$where1);
...
//更新评论状态
	...
	foreach($each as $val){
		...
		//会员通知
		if($typeid == 2){
			$sql = $dsql->SetQuery("SELECT l.`id`, l.`title`, l.`pubdate`, l.`userid` uid, c.`userid`, c.`ischeck`, c.`dtime`,c.`rid`,c.`pid` FROM `#@__".$action."_comment_all` c LEFT JOIN `#@__travel_ticket` l ON l.`id` = c.`aid` WHERE c.`id` = " . $val.$where);
		}elseif($typeid == 0){
			$sql = $dsql->SetQuery("SELECT l.`id`, l.`title`, l.`pubdate`, l.`userid` uid, c.`userid`, c.`ischeck`, c.`dtime`,c.`rid`,c.`pid` FROM `#@__".$action."_comment_all` c LEFT JOIN `#@__travel_video` l ON l.`id` = c.`aid` WHERE c.`id` = " . $val . $where);
		}elseif($typeid == 1){
			$sql = $dsql->SetQuery("SELECT l.`id`, l.`title`, l.`pubdate`, l.`userid` uid, c.`userid`, c.`ischeck`, c.`dtime`,c.`rid`,c.`pid` FROM `#@__".$action."_comment_all` c LEFT JOIN `#@__travel_strategy` l ON l.`id` = c.`aid` WHERE c.`id` = " . $val . $where);
		}elseif($typeid == 3){
			$sql = $dsql->SetQuery("SELECT l.`id`, l.`title`, l.`pubdate`, l.`userid` uid, c.`userid`, c.`ischeck`, c.`dtime`,c.`rid`,c.`pid` FROM `#@__".$action."_comment_all` c LEFT JOIN `#@__travel_agency` l ON l.`id` = c.`aid` WHERE c.`id` = " . $val . $where);
		}elseif($typeid == 4){
			$sql = $dsql->SetQuery("SELECT l.`id`, l.`title`, l.`pubdate`, l.`userid` uid, c.`userid`, c.`ischeck`, c.`dtime`,c.`rid`,c.`pid` FROM `#@__".$action."_comment_all` c LEFT JOIN `#@__travel_visa` l ON l.`id` = c.`aid` WHERE c.`id` = " . $val . $where);
		}elseif($typeid == 5){
			$sql = $dsql->SetQuery("SELECT l.`id`, l.`title`, l.`pubdate`, l.`userid` uid, c.`userid`, c.`ischeck`, c.`dtime`,c.`rid`,c.`pid` FROM `#@__".$action."_comment_all` c LEFT JOIN `#@__travel_hotel` l ON l.`id` = c.`aid` WHERE c.`id` = " . $val . $where);
		}
		$ret = $dsql->dsqlOper($sql, "results");
		if($ret){
			$aid     = $ret[0]['id'];
...
//获取评论列表
	...
	if($typeid == 2){
		...
		$where = " AND `type` = 'travel-hotel'";
	}

	if ($adminCity){
        if($typeid == 2){
			$sql = $dsql->SetQuery("SELECT `id` FROM `#@__travel_ticket` WHERE `state` = 1" . getWrongCityFilter('`cityid`', $adminCity));
		}elseif($typeid == 0){
			$sql = $dsql->SetQuery("SELECT `id` FROM `#@__travel_video` WHERE `state` = 1");
		}elseif($typeid == 1){
			$sql = $dsql->SetQuery("SELECT `id` FROM `#@__travel_strategy` WHERE `state` = 1");
		}elseif($typeid == 3){
			$sql = $dsql->SetQuery("SELECT `id` FROM `#@__travel_agency` WHERE `state` = 1" . getWrongCityFilter('`cityid`', $adminCity));
		}elseif($typeid == 4){
			$comSql = $dsql->SetQuery("SELECT `id` FROM `#@__travel_store` WHERE `state` = 1" . getWrongCityFilter('`cityid`', $adminCity));
			$comResult = $dsql->dsqlOper($comSql, "results");
			if($comResult) {
				$comid = array();
				foreach ($comResult as $key => $com) {
			...
			$sql = $dsql->SetQuery("SELECT `id` FROM `#@__travel_hotel` WHERE `cityid` = '$adminCity' AND `state` = 1");
		}
		$ret = $dsql->dsqlOper($sql, "results");
		if($ret){
			foreach ($ret as $key => $value) {
	...
	if($sKeyword != ""){
		...
		//按信息标题搜索
			...
			if($typeid == 2){
				...
				$archives = $dsql->SetQuery("SELECT l.`id` FROM `#@__travel_hotel` l WHERE l.`title` like '%$sKeyword%'");
				$results = $dsql->dsqlOper($archives, "results");
				if(count($results) > 0){
					$list = array();
					foreach ($results as $key=>$value) {
	...
	$archives = $dsql->SetQuery("SELECT `id`, `aid`, `userid`, `content`, `dtime`, `ip`, `ipaddr`, `ischeck` FROM `#@__".$action."_comment_all` WHERE 1 = 1".$where);
	$results = $dsql->dsqlOper($archives, "results");
	...
	if(count($results) > 0){
		...
		foreach ($results as $key=>$value) {
			...
			if($typeid == 2){
				...
				$typeSql = $dsql->SetQuery("SELECT `title` FROM `#@__travel_hotel` WHERE `id` = ". $value["aid"]);
			}
			$typename = $dsql->getTypeName($typeSql);
