...
if($_POST['submit'] == "提交"){
	...
	$factag = $factag ? join("|", $factag) : "";

}

if($dopost == "save" && $submit == "提交"){
	//保存到表
	$archives = $dsql->SetQuery("INSERT INTO `#@__".$tab."` (`title`, `company`, `cityid`, `typeid`, `addrid`, `address`, `lng`, `lat`, `pics`, `video`, `price`, `tag`, `click`, `pubdate`, `weight`, `state`,`note`,`factag`) VALUES ('$title', '$comid', '$cityid', '$typeid', '$addrid', '$address', '$lng', '$lat', '$pics', '$video', '$price', '$tag', '$click', '$pubdate', '$weight', '$state','$note','$factag')");
	...
	if($aid){
        ...
        dataAsync("travel",$aid,"hotel");  // 新增旅游酒店
	...
	if($submit == "提交"){
		//保存到表
		$archives = $dsql->SetQuery("UPDATE `#@__".$tab."` SET `title` = '$title', `company` = '$comid', `cityid` = '$cityid', `typeid` = '$typeid', `addrid` = '$addrid', `address` = '$address', `lng` = '$lng', `lat` = '$lat', `pics` = '$pics', `video` = '$video', `price` = '$price', `tag` = '$tag', `click` = '$click', `weight` = '$weight', `state` = '$state',`note` = '$note',`factag` = '$factag' WHERE `id` = ".$id);
		...
		if($results == "ok"){
			...
			$url = getUrlPath($param);
            dataAsync("travel",$id,"hotel");  // 修改旅游酒店
...
if(file_exists($tpl."/".$templates)){
	...
	$huoniaoTag->assign('factag', $factag);
	$huoniaoTag->assign('factagSel', $factag ? explode("|", $factag) : array());
	...
	//酒店特色标签
	$tagArr = $customtravelhotelTag ? explode("|", $customtravelhotelTag) : array();
