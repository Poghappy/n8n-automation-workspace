...
$templates = "tuanStatistics.html";
$action = empty($action) ? "chartrevenue" : $action;


//查询所有店铺
$shopArr = array();
$sql = $dsql->SetQuery("SELECT  t.`id` id, b.`title` shopname  FROM `#@__tuan_store` t LEFT JOIN `#@__business_list` b ON t.`uid` = b.`uid` ORDER BY `id`");
...
$shopname = "";
if(!empty($shop_id)){
    $sql = $dsql->SetQuery("SELECT b.`title` shopname FROM `#@__tuan_store` t LEFT JOIN `#@__business_list` b ON t.`uid` = b.`uid`  WHERE t.`id` = $shop_id");
    $ret = $dsql->dsqlOper($sql, "results");
    if($ret){
...
if($dopost == "getresults"){
    ...
    if($action == "chartrevenue"){
        ...
        for($start = $endtime; $start >= $begintime; $start -= 24 * 3600) {
            ...
            if($shop_id){
                $tuannsql = $dsql->SetQuery("SELECT `id` FROM `#@__tuanlist` WHERE `sid` = ".$shop_id);
                $tuanre   = $dsql->dsqlOper($tuannsql,"results");

                $pidarr   = implode(",", array_column($tuanre, "id"));
                $where .= " AND `proid` in ($pidarr)";
            }


            $archives = $dsql->SetQuery("SELECT o.`id`, o.`ordernum`  FROM `#@__tuan_order` o WHERE 1 = 1".$where);
            ...
            $where = empty($orderList) ? " AND 1 = 2" : " AND `id` in (".join(",", $orderList).")";


            //余额支付
            $sql = $dsql->SetQuery("SELECT SUM(`balance`) total FROM `#@__tuan_order` WHERE 1 = 1".$where);
            $money = $dsql->dsqlOper($sql, "results");

            //在线支付
            $sql = $dsql->SetQuery("SELECT SUM(`payprice`) total FROM `#@__tuan_order` WHERE 1 = 1".$where);
            $online = $dsql->dsqlOper($sql, "results");

            //总营业额
            $total = $money[0]['total'] + $online[0]['total'];
    ...
    elseif($action == "chartorder"){
        ...
        for($start = $endtime; $start >= $begintime; $start -= 24 * 3600) {
            ...
            //成功订单数
            $sql = $dsql->SetQuery("SELECT count(`id`) total FROM `#@__tuan_order` WHERE `orderstate` = 3 AND `orderdate` >= $time1 AND `orderdate` < $time2" . $where);
