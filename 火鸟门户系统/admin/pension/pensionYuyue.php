 ...
 * 管理机构预约信息
 *
 * @version        $Id: pensionYuyue.php 2019-07-18 下午20:22:45 $
...
checkPurview("pensionYuyue");
$dsql = new dsql($dbo);
$tpl = dirname(__FILE__)."/../templates/pension";
$huoniaoTag->template_dir = $tpl; //设置后台模板目录
$templates = "pensionYuyue.html";

$action = "pension_yuyue";
...
if($dopost == "delAppoint"){
	...
	if(!empty($error)){
		...
		checkpensionYuyueCache($id);
		adminLog("删除机构预约信息", $id);
...
//获取预约列表
	...
	$archives = $dsql->SetQuery("SELECT `id`, `userid`, `people`, `tel`, `store`, `pubdate`, `state` FROM `#@__".$action."` WHERE 1 = 1".$where);
	$results = $dsql->dsqlOper($archives, "results");
...
// 检查缓存
function checkpensionYuyueCache($id){
