...
if($userType == 3){
	//查询信息
	if($typeid == 0){
		$sql = $dsql->SetQuery("SELECT `id` FROM `#@__marry_store` WHERE `state` = 1" . getCityFilter('`cityid`'));
	}elseif($typeid == 1){
		$comSql = $dsql->SetQuery("SELECT `id` FROM `#@__marry_store` WHERE `state` = 1" . getCityFilter('`cityid`'));
...
//更新评论信息
	...
	//会员通知
	if($typeid == 0){
		$sql = $dsql->SetQuery("SELECT l.`id`, l.`title`, l.`pubdate`, l.`userid` uid, c.`userid`, c.`ischeck`, c.`dtime`,c.`rid`,c.`pid` FROM `#@__".$action."_comment_all` c LEFT JOIN `#@__marry_store` l ON l.`id` = c.`aid` WHERE c.`id` = " . $id . $where);
	}elseif($typeid == 1){
		$sql = $dsql->SetQuery("SELECT l.`id`, l.`title`, l.`pubdate`, l.`company`, l.`userid` uid, c.`userid`, c.`ischeck`, c.`dtime`,c.`rid`,c.`pid` FROM `#@__".$action."_comment_all` c LEFT JOIN `#@__marry_weddingcar` l ON l.`id` = c.`aid` WHERE c.`id` = " . $id . $where);
	}
	$ret = $dsql->dsqlOper($sql, "results");
	if($ret){
		$aid     = $ret[0]['id'];
...
//更新评论状态
	...
	foreach($each as $val){
		...
		//会员通知
		if($typeid == 0){
			$sql = $dsql->SetQuery("SELECT l.`id`, l.`title`, l.`pubdate`, l.`userid` uid, c.`userid`, c.`ischeck`, c.`dtime`,c.`rid`,c.`pid` FROM `#@__".$action."_comment_all` c LEFT JOIN `#@__marry_store` l ON l.`id` = c.`aid` WHERE c.`id` = " . $val.$where);
		}elseif($typeid == 1){
			$sql = $dsql->SetQuery("SELECT l.`id`, l.`title`, l.`pubdate`, l.`company`, l.`userid` uid, c.`userid`, c.`ischeck`, c.`dtime`,c.`rid`,c.`pid` FROM `#@__".$action."_comment_all` c LEFT JOIN `#@__marry_weddingcar` l ON l.`id` = c.`aid` WHERE c.`id` = " . $val . $where);
		}
		$ret = $dsql->dsqlOper($sql, "results");
		if($ret){
			$aid     = $ret[0]['id'];
...
//获取评论列表
	...
	if ($adminCity){
        if($typeid == 0){
			$sql = $dsql->SetQuery("SELECT `id` FROM `#@__marry_store` WHERE `state` = 1" . getWrongCityFilter('`cityid`', $adminCity));
		}elseif($typeid == 1){
			$comSql = $dsql->SetQuery("SELECT `id` FROM `#@__marry_store` WHERE `state` = 1" . getWrongCityFilter('`cityid`', $adminCity));
			$comResult = $dsql->dsqlOper($comSql, "results");
			if($comResult) {
				$comid = array();
				foreach ($comResult as $key => $com) {
