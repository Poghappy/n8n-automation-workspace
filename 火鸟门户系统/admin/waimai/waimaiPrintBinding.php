...
checkPurview("waimaiPrintBinding");

$dbname = "waimai_shop";
$templates = "waimaiPrintBinding.html";


// 获取店铺打印机
if($action == "printList"){
    $sids = array();
    $businessId= $dsql->SetQuery("SELECT l.`id` FROM `#@__waimai_shop` s LEFT JOIN `#@__waimai_shop_manager` m  ON s.`id`=m.`shopid` LEFT JOIN `#@__business_list` l ON m.`userid` = l.`uid`  WHERE s.`id` ='".$id."'");
    ...
    foreach ($results as $key =>    $value) {
        ...
        $printSql= $dsql->SetQuery("SELECT `id` FROM `#@__business_shop_print` WHERE `sid` = '".$id."' AND `printid` ='".$value['id']."' AND `service` = 'waimai'");
...
if($action == "printBinding"){
    ...
    foreach ($sidres  as  $k=>$v){
        $insert = $dsql->SetQuery("INSERT INTO `#@__business_shop_print` (`service`,`sid`,`printid`,`printname`) value ('waimai','$sid','".$v['id']."','".$v['title']."')");
...
if(!empty($printcode)){
        $archives = $dsql->SetQuery("SELECT pt.`sid`  FROM `#@__business_print` pt LEFT JOIN `#@__business_shop_print` t ON pt.`id` = t.`printid` WHERE pt.`mcode` like '%$printcode%' OR pt.`title` like '%$printcode%' AND  t.`service` = 'waimai'");
        $sidres = $dsql->dsqlOper($archives,"results");
        $sidArr =array();
...
foreach ($results as $key => $value) {
    ...
    $archives = $dsql->SetQuery("SELECT sp.`id`, p.`title` FROM `#@__business_shop_print` sp LEFT JOIN `#@__business_print` p ON p.`id` = sp.`printid` WHERE sp.`sid` ='".$value['id']."' AND sp.`service` ='waimai' AND p.`id` > 0");
    $results = $dsql->dsqlOper($archives, "results");
...
if(file_exists($tpl."/".$templates)){
    ...
    $jsFile = array(
        ...
        'admin/waimai/waimaiPrintBinding.js'
    );
