...
if ($type != "") {
    ...
    if ($type == "site") {
        ...
        $customClearingswitch   = (int)$clearingswitch;
        $customDataShare        = (int)$dataShareSwtich;
        $customfenXiao          = (int)$fenXiao;
        $customIsopenqd         = (int)$isopenqd;
        $customIsopencode       = (int)$isopencode;
        $custompaotuiMaxjuli    = (int)$paotuiMaxjuli;
        $custompaotuiMaxAmount  = (int)$paotuiMaxAmount;
		$serviceMoney           = $pserviceMoney;
		$ptweight               = $pweight;
		$maxtip                 = $pmaxtip;
		$cstime                 = $pcstime;
		$csprice                = $pcsprice;
        $custompeerpay          = (int)$peerpay;
        $cuszhipeisong          = (int)$zhipeisong;
        $custom_businessChangeDelivery          = (int)$businessChangeDelivery;
        $custom_businessChangePrice          = (int)$businessChangePrice;
        $custom_paotuiStartTime        = $paotuiStartTime;
        $custom_paotuiEndTime          = $paotuiEndTime;
        $customDeskQrType       = (int)$deskQrType;

        $customwaimaiCourierP   = (float)$waimaiCourierP;
        $custompaotuiCourierP   = (float)$paotuiCourierP;
        $customtakeLimit        = $takeLimit;
        $customsuccessLimit     = $successLimit;
        $customcourierFree      = (float)$courierFree;

        /*第三方配送*/
        $custom_otherpeisong    = $otherpeisong;

        //骑手地址黑白名单
        $custom_phoneaddrwhite  = $phoneaddrwhite;
        if(isset($custom_phoneaddrwhite)){
            $custom_phoneaddrwhite = str_replace(" ","",$custom_phoneaddrwhite);
            $custom_phoneaddrwhite = str_replace("，",",",$custom_phoneaddrwhite);
        }
        $custom_phoneaddrblack  = $phoneaddrblack;
        if(isset($custom_phoneaddrblack)){
            $custom_phoneaddrblack = str_replace(" ","",$custom_phoneaddrblack);
            $custom_phoneaddrblack = str_replace("，",",",$custom_phoneaddrblack);
        }
        ...
        $custom_systemturnnum       = $systemturnnum;
        $custom_paotuitime          = $paotuitime;
        ...
        if ($customMark == 1) {
            ...
            if ($custom_waterMarkType == 1) {
                if ($custom_waterMarkText == "" || $custom_markFontfamily == "")
                    die('{"state": 200, "info": ' . json_encode("请填写完整！") . '}');
        ...
        $customClient_secret = $client_secret;
        $customAcceptType    = $acceptType;
        $customPrint_user    = $user;
        $customPrint_ukey    = $ukey;
        $customPrint_ucount    = $ucount;
        $customPrintType     = $printType;
        $customPrintTemplate = $_POST['printTemplate'] != 'false' ? serialize(json_decode($_POST['printTemplate'], true)) :'';

        adminLog("修改外卖设置", "打印机配置");
    }elseif ($type == "zsb") {
        $customZsbspe = serialize($zsbspe);
        adminLog("修改外卖设置", "准时宝配置");
    }elseif ($type =="zsbxy") {
        $customZsbxy    = $zsbxy;
        adminLog("修改外卖设置", "准时宝协议");
    }elseif ($type =="hyxy") {
        $customHyxy     = $hyxy;
        adminLog("修改外卖设置", "会员协议");
    }elseif ($type == 'paotui'){
        $paotui_delivery_fee_value = array();
        if($rangedeliveryfee){
            foreach ($rangedeliveryfee['start'] as $key => $value) {
                array_push($paotui_delivery_fee_value, array(
                    $value, $rangedeliveryfee['stop'][$key], $rangedeliveryfee['value'][$key]
                ));
            }
        }
        $custompaotui_delivery = serialize($paotui_delivery_fee_value);
        ...
        if($addservice){
            foreach ($addservice as $key => $value) {
                array_push($addserviceArr, array(
                    $value['start'], $value['stop'], $value['price']
                ));
            }
        }
        $customaddservice = serialize($addserviceArr);

        $customweight     = serialize($weight);
        adminLog("修改外卖设置", "跑腿计价规则");
    } elseif ($type == 'waimaiaddition'){
        /*外卖订单额外加成*/
        $waimaiorderprice       = $orderprice;
        $waimaiadditionkm       = $additionkm;
        $waimai_orderprice_arr = array();
        if($waimaiorderprice){
            foreach ($waimaiorderprice['start'] as $key => $value) {
                array_push($waimai_orderprice_arr, array(
                    $value, $waimaiorderprice['stop'][$key], $waimaiorderprice['value'][$key]
                ));
            }
        }
        $custom_waimaiorderprice = serialize($waimai_orderprice_arr);

        $waimai_additionkm_arr = array();
        if($waimaiadditionkm){
            foreach ($waimaiadditionkm['number1'] as $key => $value) {
                array_push($waimai_additionkm_arr, array(
                    $value,$waimaiadditionkm['timelimit'][$key],$waimaiadditionkm['number2'][$key]
                ));
            }
        }
        $custom_waimaadditionkm = serialize($waimai_additionkm_arr);

        adminLog("修改外卖设置", "外卖订单额外加成");

    }elseif($type == 'sale'){
        $customSaleState  = (int)$saleState;
        $customSaleTitle  = $saleTitle ? $saleTitle : '优惠推荐';
        $customSaleSubTitle  = $saleSubTitle ? $saleSubTitle : '0元外卖限量抢~';
        $saleTimes = $_GET['saleTimes'];
        $saleTimes  = $saleTimes ? json_decode($saleTimes, true) : array();

        $customSaleTimes = array();
        foreach ($saleTimes as $key => $value) {
            $count = (int)$value['count'];
            if($value['stime'] && $value['etime'] && $count){
                array_push($customSaleTimes, array(
                    'stime' => $value['stime'],
                    'etime' => $value['etime'],
                    'count' => $value['count']
                ));
        ...
        $customSaleTimes = serialize($customSaleTimes);
        adminLog("修改外卖设置", "优惠推荐");
    }
    //域名操作
    ...
    $customInc .= "\$custompaotuiMaxAmount    = '" . (int)$custompaotuiMaxAmount . "';\r\n";
    $customInc .= "\$custompeerpay = '".(int)$custompeerpay."';\r\n";
    $customInc .= "\$cuszhipeisong = '".(int)$cuszhipeisong."';\r\n";
    $customInc .= "\$custom_businessChangeDelivery = '".(int)$custom_businessChangeDelivery."';\r\n";
    $customInc .= "\$custom_businessChangePrice = '".(int)$custom_businessChangePrice."';\r\n";
    $customInc .= "\$custom_otherpeisong = '".(int)$custom_otherpeisong."';\r\n";

    //骑手注册地址限制黑白
    $customInc .= "\$custom_phoneaddrwhite = '".$custom_phoneaddrwhite."';\r\n";
    $customInc .= "\$custom_phoneaddrblack = '".$custom_phoneaddrblack."';\r\n";
    ...
    //外卖默认分成
    $customInc .= "\$custom_fencheng_foodprice = " . (int)$custom_fencheng_foodprice . ";\r\n";
    $customInc .= "\$custom_fencheng_delivery = " . (int)$custom_fencheng_delivery . ";\r\n";
    $customInc .= "\$custom_fencheng_addservice = " . (int)$custom_fencheng_addservice . ";\r\n";
    $customInc .= "\$custom_fencheng_zsb = " . (int)$custom_fencheng_zsb . ";\r\n";
    $customInc .= "\$custom_fencheng_dabao = " . (int)$custom_fencheng_dabao . ";\r\n";
    $customInc .= "\$custom_fencheng_discount = " . (int)$custom_fencheng_discount . ";\r\n";
    $customInc .= "\$custom_fencheng_promotion = " . (int)$custom_fencheng_promotion . ";\r\n";
    $customInc .= "\$custom_fencheng_firstdiscount = " . (int)$custom_fencheng_firstdiscount . ";\r\n";
    $customInc .= "\$custom_fencheng_quan = " . (int)$custom_fencheng_quan . ";\r\n";
...
if (file_exists($tpl . "/" . $templates)) {
    ...
    $huoniaoTag->assign('paotuiMaxAmount', $custompaotuiMaxAmount ?: 500);
    $huoniaoTag->assign('courierFree', $customcourierFree);
    $huoniaoTag->assign('waimaiCourierP', $customwaimaiCourierP);
    $huoniaoTag->assign('paotuiCourierP', $custompaotuiCourierP);
    $huoniaoTag->assign('takeLimit', $customtakeLimit);
    $huoniaoTag->assign('successLimit', $customsuccessLimit);
    $huoniaoTag->assign('courierFree', $customcourierFree);

    $huoniaoTag->assign('paotui_delivery_fee_value', unserialize($custompaotui_delivery));
    $huoniaoTag->assign('weight', unserialize($customweight));
    $huoniaoTag->assign('addservice', unserialize($customaddservice));


    /*外卖额外订单加成*/
    $huoniaoTag->assign('waimaiorderprice', unserialize($custom_waimaiorderprice));
    $huoniaoTag->assign('waimaadditionkm', unserialize($custom_waimaadditionkm));
    ...
    //外卖骑手抢单
    $huoniaoTag->assign('isOpenqdSwitch', array('1', '0'));
    $huoniaoTag->assign('isOpenqdSwitchNames',array('开启','关闭'));
    $huoniaoTag->assign('isOpenqdSwitchChecked', (int)$customIsopenqd);

    //骑手注册黑白地址
    $huoniaoTag->assign("phoneaddrwhite",$custom_phoneaddrwhite);
    $huoniaoTag->assign("phoneaddrblack",$custom_phoneaddrblack);

    //跑腿验证码开关
    $huoniaoTag->assign('isOpencodeSwitch', array('1', '0'));
    $huoniaoTag->assign('isOpencodeSwitchNames',array('开启','关闭'));
    $huoniaoTag->assign('isOpencodeSwitchChecked', (int)$customIsopencode);
    //跑腿服务费
    ...
    // 转单限制
    $huoniaoTag->assign('systemturnnum', (int)$custom_systemturnnum);
    $huoniaoTag->assign('paotuitime', (int)$custom_paotuitime);

    $huoniaoTag->assign('paotuiStartTime', $custom_paotuiStartTime ? $custom_paotuiStartTime : '08:00');
    $huoniaoTag->assign('paotuiEndTime', $custom_paotuiEndTime ? $custom_paotuiEndTime : '20:00');
    ...
    //地图配置
    $huoniaoTag->assign('mapList', array(0 => '系统默认', 1 => '谷歌', 2 => '百度', 4 => '高德'));
    ...
    /*打印机打印方式*/
    $huoniaoTag->assign('printTypeList', array(0 => '普通打印', 1 => '图片打印'));
    $huoniaoTag->assign('printTypeed', $customPrintType);

    $huoniaoTag->assign('partnerId', $customPartnerId);
    $huoniaoTag->assign('printKey', $customPrintKey);
    $huoniaoTag->assign('clientId', $customClientId);
    $huoniaoTag->assign('client_secret', $customClient_secret);
    ...
    //小票样式
    $huoniaoTag->assign('printTemplate', json_encode(unserialize($customPrintTemplate)));
    ...
    //外卖结算-单选
    $huoniaoTag->assign('clearingSwitchCheck', array('0', '1'));
    $huoniaoTag->assign('clearingSwitchCheckNames', array('关闭', '开启'));
    $huoniaoTag->assign('clearingSwitched', (int)$customClearingswitch);
    ...
    //准时宝
    $huoniaoTag->assign('zsbspe', unserialize($customZsbspe));

    //会员协议
    $huoniaoTag->assign('zsbxy', $customZsbxy);
    $huoniaoTag->assign('hyxy', $customHyxy);
    ...
    $huoniaoTag->assign('zhipeisong', array('1', '0'));
    $huoniaoTag->assign('zhipeisongNames',array('启用','禁用'));
    $huoniaoTag->assign('zhipeisongChecked', (int)$cuszhipeisong);


    $huoniaoTag->assign('businessChangeDeliverySwitch', array('1', '0'));
    $huoniaoTag->assign('businessChangeDeliverySwitchNames',array('启用','禁用'));
    $huoniaoTag->assign('businessChangeDeliverySwitchChecked', (int)$custom_businessChangeDelivery);

    $huoniaoTag->assign('businessChangePriceSwitch', array('0', '1'));
    $huoniaoTag->assign('businessChangePriceSwitchNames',array('不需要审核','需要审核'));
    $huoniaoTag->assign('businessChangePriceSwitchChecked', (int)$custom_businessChangePrice);

    //优惠推荐
    $huoniaoTag->assign('saleState', (int)$customSaleState);
    $huoniaoTag->assign('saleTitle', $customSaleTitle ? $customSaleTitle : '优惠推荐');
    $huoniaoTag->assign('saleSubTitle', $customSaleSubTitle ? $customSaleSubTitle : '0元外卖限量抢~');
    $huoniaoTag->assign('saleTimes', $customSaleTimes ? unserialize($customSaleTimes) : array());

    /*第三方配送员*/
    $huoniaoTag->assign('otherpeisong', array(0 => '关闭', 1 => 'UU跑腿', 3 => '麦芽田'));
    $huoniaoTag->assign('otherpeisongChecked', (int)$custom_otherpeisong);

    //扫码点餐二维码类型
    $huoniaoTag->assign('deskQrType', array('0', '1'));
    $huoniaoTag->assign('deskQrTypeNames',array('普通二维码','微信小程序码'));
    $huoniaoTag->assign('deskQrTypeChecked', (int)$customDeskQrType);
