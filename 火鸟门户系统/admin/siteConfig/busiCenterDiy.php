 ...
 * 个人会员移动端首页模板自定义
 *
 * @version        $Id: busiCenterDiy.php 2023-05-29 上午09:57:43 $
...
checkPurview("busiCenterDiy");
$dsql = new dsql($dbo);
$tpl = dirname(__FILE__) . "/../templates/siteConfig";
$huoniaoTag->template_dir = $tpl; //设置后台模板目录
$templates = "busiCenterDiy.html";

$platform = trim($platform);  //设置指定终端的页面，app、wxmini、dymini
...
$config = array();
$siteConfigHandlers = new handlers("siteConfig", "busiCenterDiy");
...
if ($dopost == 'save') {
    ...
    if ($type == 1) {
        ...
        //保存到主表
        if($platform == 'h5'){
            $archives = $dsql->SetQuery("SELECT `id`, `config` FROM `#@__site_config` WHERE (`title` = '$platform' OR `title` = '' OR `title` IS NULL) AND `type` = 'busiCenter' ORDER BY `id` DESC LIMIT 1");
        }else{
            $archives = $dsql->SetQuery("SELECT `id`, `config` FROM `#@__site_config` WHERE `title` = '$platform' AND `type` = 'busiCenter' ORDER BY `id` DESC LIMIT 1");
        }
        ...
        if ($results) {
            ...
            //保存到主表
            $archives = $dsql->SetQuery("INSERT INTO `#@__site_config` (`type`, `browse`, `title`) VALUES ('busiCenter', '$browse', '$platform')");
            $aid = $dsql->dsqlOper($archives, "update");
        }
    ...
    //保存
        ...
        if($terminal){
            ...
            foreach ($terminalArr as $k => $v){
                ...
                if($v == 'h5'){
                    $archives = $dsql->SetQuery("SELECT `id`, `config` FROM `#@__site_config` WHERE (`title` = '$v' OR `title` = '' OR `title` IS NULL) AND `type` = 'busiCenter' ORDER BY `id` DESC LIMIT 1");
                }else{
                    $archives = $dsql->SetQuery("SELECT `id`, `config` FROM `#@__site_config` WHERE `title` = '$v' AND `type` = 'busiCenter' ORDER BY `id` DESC LIMIT 1");
                }
                $results = $dsql->dsqlOper($archives, "results");
                if ($results) {
                    $_id = $results[0]['id'];
                    ...
                    //保存到主表
                    $archives = $dsql->SetQuery("INSERT INTO `#@__site_config` (`type`, `config`, `title`, `pubdate`) VALUES ('busiCenter', '$config', '$v', '$time')");
                    $aid = $dsql->dsqlOper($archives, "update");
                }
        ...
        if ($results) {
            ...
            //保存到主表
            $archives = $dsql->SetQuery("INSERT INTO `#@__site_config` (`type`, `config`, `title`, `pubdate`) VALUES ('busiCenter', '$config', '$platform', '$time')");
            $aid = $dsql->dsqlOper($archives, "update");
        }
        if ($aid  == "ok") {
...
if ($dopost == 'edit') {
    ...
    if ($type == 1) {
        ...
        //保存到主表
        $archives = $dsql->SetQuery("UPDATE  `#@__site_config` SET `browse` = '$browse' WHERE `title` = '$platform' AND `type` = 'busiCenter'");
        $aid = $dsql->dsqlOper($archives, "update");
        $param = array(
            "service"     => "member",
            "type"        => "user"
    ...
    //保存
        ...
        $archives = $dsql->SetQuery("UPDATE  `#@__site_config` SET `config` = '$config', `pubdate` = '$time' WHERE `title` = '$platform' AND `type` = 'busiCenter'");
        $aid = $dsql->dsqlOper($archives, "update");
        if ($aid  == "ok") {
            echo json_encode(array(
                'state' => 100,
...
if (file_exists($tpl . "/" . $templates)) {
    ...
    //是否设置过系统默认模板，通过type=busiCenter
    $sql = $dsql->SetQuery("SELECT * FROM `#@__site_config` WHERE (`title` = 'h5' OR `title` = '' OR `title` IS NULL) AND `type` = 'busiCenter' ORDER BY `id` DESC LIMIT 1");
...
function getSiteConfig($platform){
    ...
    $data = array();
    if($platform == 'h5'){
        $sql = $dsql->SetQuery("SELECT `id`, `config` FROM `#@__site_config` WHERE `type` = 'busiCenter' AND (`title` = 'h5' OR `title` = '' OR `title` IS NULL) ORDER BY `id` DESC LIMIT 1");
    }else{
        $sql = $dsql->SetQuery("SELECT `id`, `config` FROM `#@__site_config` WHERE `type` = 'busiCenter' AND `title` = '$platform' ORDER BY `id` DESC LIMIT 1");
    }
    $ret = $dsql->dsqlOper($sql, "results");
    if($ret){
        $data['state'] = 1;
