...
if ($type != "") {
    ...
    if ($type == "site") {
        ...
        $custominfoTag          = $infoTag;
        $customFabuTips = $fabuTips;
		...
		$customExpireShow      = (int)$expireShowSwtich;
        $customFabuCheckPhone = (int)$fabuCheckPhone;
        $customFabuMapConfig = (int)$fabuMapConfig;
        $customFabuMapDisplayLocation = (int)$fabuMapDisplayLocation;

        $custom_excitation = is_array($excitationopen) && $excitationopen ? join(',',$excitationopen) : '' ;
        //手续费
        $cfg_cost = (int)$cost;
    ...
    }elseif($type =='infotime'){
        $info_refreshArr = array();
        if($info_refresh && $info_refresh['day']){
            for ($i = 0; $i < count($info_refresh['day']); $i++) {
                if($info_refresh['daytext'][$i] == 1){
                   $daytime =  (int)$info_refresh['day'][$i] * (365 * 86400);
                    $dayText = '年';
                    }elseif ($info_refresh['daytext'][$i] == 2){
                    $daytime= (int)$info_refresh['day'][$i] * (30 * 86400);
                    $dayText = '月';
                }else{
                    $daytime = (int)$info_refresh['day'][$i] * 86400;
                    $dayText = '天';
                }
                array_push($info_refreshArr, array(
                    'times' => (int)$info_refresh['times'],
                    'day' => (int)$info_refresh['day'][$i],
                    'daytext' => (int)$info_refresh['daytext'][$i],
                    'daytime' =>$daytime,
                    'dayText' =>$dayText,
...
//        $info_refr = array_column($info_refreshArr,'day');
//        if ($info_refreshArr[0]['daytext'] == 1){
//            $infoday = $info_refreshArr[0]['day'] * 365;
//        }elseif($info_refreshArr[0]['daytext'] == 2){
//            $infoday = $info_refreshArr[0]['day'] * 30;
//        }else{
//            $infoday = $info_refreshArr[0]['day'];
//        }
//       $danjia = sprintf("%.2f",$info_refreshArr[0]['price'] / $infoday);
        $cfg_info_Smart = serialize($info_refreshArr);
        //永久有效
        $info_valid = array();
        if($valid && $valid['day']){
            for ($i = 0; $i < count($valid['day']); $i++) {
                array_push($info_valid, array(
                    // 'day' => (int)$valid['day'][$i],
                    // 'daytime' => (int)$valid['day'][$i] * 86400,
                    'day' => 365 * 10 * 2,  //20年
                    'daytime' => 365 * 10 * 2 * 86400,
                    'price' => (float)$valid['price'][$i]
                ));
            }
        }
        $cfg_valid = serialize($info_valid);
        //默认有效期
        $info_defaultvalid = array();
        // if($valid && $valid['day']){
        //     for ($i = 0; $i < count($defaultvalid['day']); $i++) {
        //         array_push($info_defaultvalid, array(
        //             'day' => (int)$defaultvalid['day'][$i],
        //             'daytime' => (int)$defaultvalid['day'][$i] * 86400,
        //         ));
        //     }
        // }
        $cfg_defaultvalid  = serialize($info_defaultvalid);



        //vip有效期
        $info_membervalid = array();
        // if($valid && $valid['day']){
        //     for ($i = 0; $i < count($membervalid['day']); $i++) {
        //         array_push($info_membervalid, array(
        //             'day' => (int)$membervalid['day'][$i],
        //             'daytime' => (int)$membervalid['day'][$i] * 86400,
        //         ));
        //     }
        // }

        $cfg_membervalid  = serialize($info_membervalid);

        $cfg_isvalid = $isvaild ?  $isvaild : 0;               //是否设置永久有效

        //企业会员每周发布
        $cfg_qymember = $qymember;

        //企业会员默认有效时长
         $info_qymembervalid = array();
        if($valid && $valid['day']){
            for ($i = 0; $i < count($qymembervalid['day']); $i++) {
                array_push($info_qymembervalid, array(
                    'day' => (int)$qymembervalid['day'][$i],
                    'daytime' => (int)$qymembervalid['day'][$i] * 86400,
                ));
            }
        }
        $cfg_qymembervalid  = serialize($info_qymembervalid);
	...
	$customInc .= "\$customExpireShow = ".(int)$customExpireShow.";\r\n";
    $customInc .= "\$customFabuCheckPhone = '" . $customFabuCheckPhone . "';\r\n";
    $customInc .= "\$customFabuMapConfig = '" . $customFabuMapConfig . "';\r\n";
    $customInc .= "\$customFabuMapDisplayLocation = '" . $customFabuMapDisplayLocation . "';\r\n";
    $customInc .= "\$customfenXiao = ".(int)$customfenXiao.";\r\n";
    $customInc .= "\$custominfoTag = '".$custominfoTag."';\r\n";
    $customInc .= "\$customFabuTips = '".$customFabuTips."';\r\n";
    $customInc .= "\$custom_excitation = '".$custom_excitation."';\r\n";
    $customInc .= "\$cfg_cost = '".$cfg_cost."';\r\n";
    //模板风格
    ...
    //信息有效期
    $customInc .= "\$cfg_info_Smart = '".$cfg_info_Smart."';\r\n";
    //永久有效期
    $customInc .= "\$cfg_valid = '".$cfg_valid."';\r\n";
    //默认有效期
    $customInc .= "\$cfg_defaultvalid = '".$cfg_defaultvalid."';\r\n";
    //vip有效期
    $customInc .= "\$cfg_membervalid = '".$cfg_membervalid."';\r\n";
    //是否设置永久有效
    $customInc .= "\$cfg_isvalid = '".$cfg_isvalid."';\r\n";

    $customInc .= "\$cfg_qymember = '".$cfg_qymember."';\r\n";
    $customInc .= "\$cfg_qymembervalid = '".$cfg_qymembervalid."';\r\n";
    $customInc .= "?" . ">";
...
if (file_exists($tpl . "/" . $templates)) {
    ...
    //标签
    if(!isset($custominfoTag)){
        $custominfoTag = '本季热卖|超值特价|人气推荐';
    }
    $huoniaoTag->assign('infoTag', $custominfoTag);
    //频道LOGO
	...
	//过期继续显示-单选
	$huoniaoTag->assign('expireShowSwitch', array('0', '1'));
	$huoniaoTag->assign('expireShowSwitchNames',array('禁用', '启用'));
	$huoniaoTag->assign('expireShowSwitchChecked', (int)$customExpireShow);

    $huoniaoTag->assign('fabuCheckPhoneNum', array('0', '1'));
    $huoniaoTag->assign('fabuCheckPhoneNames', array('不需要验证', '需要验证'));
    $huoniaoTag->assign('fabuCheckPhoneChecked', (int)$customFabuCheckPhone);

    $huoniaoTag->assign('fabuMapConfig', array(0, 1, 2));
    $huoniaoTag->assign('fabuMapConfigNames', array('系统默认', '选项一', '选项二'));
    $huoniaoTag->assign('fabuMapConfigChecked', (int)$customFabuMapConfig);

    $huoniaoTag->assign('fabuMapDisplayLocation', array(0, 1));
    $huoniaoTag->assign('fabuMapDisplayLocationNames', array('显示', '隐藏'));
    $huoniaoTag->assign('fabuMapDisplayLocationChecked', (int)$customFabuMapDisplayLocation);

    /*用户激励*/
    $huoniaoTag->assign('excitationopt', array( '1', '2'));
    $huoniaoTag->assign('excitationnames',array('阅读红包','分享有奖'));
    $huoniaoTag->assign('excitationopen', $custom_excitation=== '' ? "" : explode(",", $custom_excitation));
    $huoniaoTag->assign('cost', (int)$cfg_cost);
    if (empty(unserialize($cfg_info_Smart))){
        $cfg_info_Smart = 'a:2:{i:0;a:6:{s:5:"times";i:0;s:3:"day";i:3;s:7:"daytext";i:3;s:7:"daytime";i:259200;s:7:"dayText";s:3:"天";s:5:"price";d:5;}i:1;a:6:{s:5:"times";i:0;s:3:"day";i:7;s:7:"daytext";i:3;s:7:"daytime";i:604800;s:7:"dayText";s:3:"天";s:5:"price";d:10;}}';  //默认 有效期
    }
    $huoniaoTag->assign('info_refreshSmart', $cfg_info_Smart ? unserialize($cfg_info_Smart) : array());
    $huoniaoTag->assign('cfg_valid', $cfg_valid ? unserialize($cfg_valid) : array());
    $huoniaoTag->assign('cfg_defaultvalid', $cfg_defaultvalid ? unserialize($cfg_defaultvalid) : array());
    $huoniaoTag->assign('cfg_membervalid', $cfg_membervalid ? unserialize($cfg_membervalid) : array());
    $huoniaoTag->assign('cfg_isvalid', $cfg_isvalid);
    $huoniaoTag->assign('cfg_qymember', $cfg_qymember);
    $huoniaoTag->assign('cfg_qymembervalid', $cfg_qymembervalid ? unserialize($cfg_qymembervalid) : array());
    ...
    //年月天
    $huoniaoTag->assign('dayType', array('1','2','3'));
    $huoniaoTag->assign('dayTypeNames',array('年','月','天'));
    $huoniaoTag->assign('dayTypeChecked', (int)$custom_dayType);
    //水印字体
