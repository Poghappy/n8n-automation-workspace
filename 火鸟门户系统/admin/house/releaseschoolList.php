 ...
 * 管理学校
 *
 * @version        $Id: releaseschoolList.php 2014-1-15 上午11:47:04 $
...
checkPurview("releaseschoolList");
$dsql  = new dsql($dbo);
$tpl   = dirname(__FILE__)."/../templates/house";
$huoniaoTag->template_dir = $tpl; //设置后台模板目录
$templates = "releaseschoolList.html";
...
if($dopost == "getList"){
  ...
  if ($cityid) {
	...
	$archives = $dsql->SetQuery("SELECT `id`, `title`, `logo`, `addrid`, `type`, `state`, `pubdate` FROM `#@__".$tab."` WHERE 1 = 1".$where);
	...
	if(count($results) > 0){
		...
		foreach ($results as $key=>$value) {
            ...
            if($value["type"]){
                ...
                $typesql  = $dsql->SetQuery("SELECT `typename` FROM `#@__houseitem` WHERE `id` IN (".$value['type'].")");

                $typeres  = $dsql->dsqlOper($typesql,"results");

                if(is_array($typeres)){
                    $typename = array_column($typeres,"typename");
                }
                $list[$key]["typename"]  = implode(',',$typename);
            }
...
//删除
	...
	if(!testPurview("releaseschoolDel")){
	...
	if($id != ""){
		...
		foreach($each as $val){
			...
			//删除学校logo
			delPicFile($results[0]['logo'], "delThumb", "house");

			//删除学校背景图
			delPicFile($results[0]['banner'], "delThumb", "house");
...
//更新状态
	...
	if(!testPurview("releaseschoolEdit")){
		die('{"state": 200, "info": '.json_encode("对不起，您无权使用此功能！").'}');
	};
	$each = explode(",", $id);
	$error = array();
...
if(file_exists($tpl."/".$templates)){
	...
	$jsFile = array(
		...
		'admin/house/releaseschoolList.js'
	);
