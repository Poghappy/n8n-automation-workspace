	...
	if(count($results) > 0){
		...
		foreach ($results as $key=>$value) {
			...
			if($userid != 0 && $value['usertype'] == 1){
				...
				//会员
				//$userSql = $dsql->SetQuery("SELECT `username`, `contact` FROM `#@__house_zjuser` WHERE `id` = ". $userid);
				//$username = $dsql->getTypeName($userSql);
	...
	if($id != ""){
		...
		foreach($each as $val){
			...
			if($results != "ok"){
                ...
                dataAsync("house",$val,"cf");  // 房产-厂房-删除信息
            }
		}
		if(!empty($error)){
...
//                        $pointuser   = (int)($userpoint+$cfg_returnInteraction_house);
			...
			if($results != "ok"){
			    ...
			    dataAsync("house",$val,"cf");  // 房产-厂房-修改状态
            }
		}
		if(!empty($error)){
			echo '{"state": 200, "info": '.json_encode($error).'}';
