...
if($dopost == "save" && $submit == "提交"){
	//保存到表
	$archives = $dsql->SetQuery("INSERT INTO `#@__".$tab."` (`cityid`, `type`, `title`, `loupan`, `addrid`, `address`, `nearby`, `litpic`, `proprice`, `protype`, `area`, `price`, `usertype`, `userid`, `username`, `contact`, `zhuangxiu`, `bno`, `floor`, `note`, `mbody`, `weight`, `config`, `state`, `sellstate`,`pubdate`, `video`, `qj_type`, `qj_file`, `longitude`, `latitude`, `loupanid`, `fg`, `level`, `peitao`, `floortype`, `floorspr`, `sex`, `wx_tel`, `wuye_in`,`is_vipguanggao`) VALUES ('$cityid', '$type', '$title', '$loupan', '$addrid', '$address', '$nearby', '$litpic', '$proprice', '$protype', '$area', '$price', '$usertype', '$userid', '$username', '$contact', '$zhuangxiu', '$bno', '$floor', '$note', '$mbody', '$weight', '$config', '$state','$sellstate', '".GetMkTime(time())."', '$video', '$qj_type', '$qj_file', '$longitude', '$latitude', '$loupanid', $fg, $level, '$peitao', '$floortype', '$floorspr', '$sex', '$wx_tel', '$wuye_in','$advertising')");
	...
	if($aid){
        ...
        dataAsync("house",$aid,"xzl");  // 写字楼、新增信息
	...
	if($submit == "提交"){
		...
		if($results == "ok"){
			...
			$url = getUrlPath($param);
            dataAsync("house",$id,"xzl");  // 写字楼、更新信息
...
if(file_exists($tpl."/".$templates)){
    ...
    $archives = $dsql->SetQuery("SELECT * FROM `#@__houseitem` WHERE `parentid` = 113 ORDER BY `weight` ASC");
    $results = $dsql->dsqlOper($archives, "results");
	$peitaolist = array();
	$peitaoval = array();
    foreach($results as $value){
        array_push($peitaolist, $value['typename']);
        array_push($peitaoval, $value['id']);
    }
	...
	$huoniaoTag->assign('sellopt', array('0', '2',));
	$huoniaoTag->assign('selltypename', "出租状态");
	$huoniaoTag->assign('sellnames',array('未出租','已出租'));
	$huoniaoTag->assign('sell', $sellstate == "" ? 0 : $sellstate);


	$huoniaoTag->assign('sellopt1', array('0', '1'));
	$huoniaoTag->assign('selltypename1', "出售状态");
	$huoniaoTag->assign('sellnames1',array('未出售','已出售'));
