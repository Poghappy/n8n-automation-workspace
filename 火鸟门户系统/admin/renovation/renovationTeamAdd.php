...
if($_POST['submit'] == "提交"){
	...
	if($companyid == 0 && trim($company) == '' && $type == 1){
...
if($dopost == "save" && $submit == "提交"){
	//保存到表
	$archives = $dsql->SetQuery("INSERT INTO `#@__".$tab."` (`name`,`tel`, `works`, `post`,`type`, `userid`, `photo`, `company`, `special`, `style`, `idea`, `note`, `weight`, `click`, `state`, `pubdate`) VALUES ('$name','$tel', '$works', '$post','$type','$userid', '$litpic', '$companyid', '$special', '$style', '$idea', '$note', '$weight', '$click', '$state', '".GetMkTime(time())."')");
	...
	if($aid){
        ...
        dataAsync("renovation",$aid,"designer");  // 装修门户-设计师-新增
	...
	if($submit == "提交"){
		//保存到表
		$archives = $dsql->SetQuery("UPDATE `#@__".$tab."` SET `name` = '$name',`tel` = '$tel' , `works` = '$works', `post` = '$post', `userid` = '$userid', `photo` = '$litpic',`type`=  '$type',`company` = '$companyid', `special` = '$special', `style` = '$style', `idea` = '$idea', `note` = '$note', `weight` = '$weight', `click` = '$click', `state` = '$state' WHERE `id` = ".$id);
		...
		if($results == "ok"){
			...
			$url = getUrlPath($param);
            dataAsync("renovation",$id,"designer");  // 装修门户-设计师-修改
...
if(file_exists($tpl."/".$templates)){
	...
	$huoniaoTag->assign('statenames',array('未审核','已审核','审核失败'));
