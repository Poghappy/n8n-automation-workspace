 ...
 * 管理分类信息评论
 *
 * @version        $Id: huodongCommon.php 2013-7-7 上午10:33:36 $
...
checkPurview("huodongCommon");
$dsql = new dsql($dbo);
$tpl = dirname(__FILE__)."/../templates/huodong";
$huoniaoTag->template_dir = $tpl; //设置后台模板目录
$templates = "huodongCommon.html";
...
if($dopost == "getDetail"){
	...
	if(count($results) > 0){
		...
		$typeSql = $dsql->SetQuery("SELECT p.`title`  FROM `#@__huodong_list` p WHERE p.`id` = ". $results[0]["aid"]);
...
//更新评论信息
	...
	//会员通知
	$sql = $dsql->SetQuery("SELECT l.`id`, l.`title`, l.`pubdate`, l.`uid`, c.`userid`, c.`ischeck`, c.`dtime`,c.`rid`,c.`pid` FROM `#@__".$action."_comment_all` c LEFT JOIN `#@__huodong_list` l ON l.`id` = c.`aid` WHERE c.`id` = " . $id . $where);
	$ret = $dsql->dsqlOper($sql, "results");
	if($ret){
		$aid     = $ret[0]['id'];
		$title   = $ret[0]['title'];
...
//更新评论状态
	...
	foreach($each as $val){
		...
		//会员通知
		$sql = $dsql->SetQuery("SELECT l.`id`, l.`title`, l.`pubdate`, l.`uid`, c.`userid`, c.`ischeck`, c.`dtime`,c.`rid`,c.`pid` FROM `#@__".$action."_comment_all` c LEFT JOIN `#@__huodong_list` l ON l.`id` = c.`aid` WHERE c.`id` = " . $val.$where);
		$ret = $dsql->dsqlOper($sql, "results");
		if($ret){
			$aid     = $ret[0]['id'];
			$title   = $ret[0]['title'];
			...
			if($ischeck != $arcrank){
				...
				if($arcrank == 1){
                    ...
                    global $cfg_returnInteraction_comment;
...
//获取评论列表
	...
	$where = " AND `type` = 'huodong-detail'";

	if ($adminCity){
		$sql = $dsql->SetQuery("SELECT `id` FROM `#@__huodong_list` WHERE `state` = 1" . getWrongCityFilter('`cityid`', $adminCity));
		$ret = $dsql->dsqlOper($sql, "results");
		if($ret){
			foreach ($ret as $key => $value) {
				array_push($ids, $value['id']);
	...
	if($sKeyword != ""){
		...
		//按信息标题搜索
			...
			$archives = $dsql->SetQuery("SELECT l.`id` FROM `#@__huodong_list` l WHERE l.`title` like '%$sKeyword%'");
			$results = $dsql->dsqlOper($archives, "results");
			if(count($results) > 0){
				$list = array();
...
if(file_exists($tpl."/".$templates)){
	...
	$jsFile = array(
		...
		'admin/huodong/huodongCommon.js'
	);
