...
checkPurview("jobCompanyFollow");
$dsql  = new dsql($dbo);
$tpl   = dirname(__FILE__)."/../templates/job";
$huoniaoTag->template_dir = $tpl; //设置后台模板目录
$templates = "jobCompanyFollow.html";
...
if($dopost == "getList"){
    ...
    if($stime != ""){
        $where .= " AND g.`time` >= ". GetMkTime($stime." 00:00:00");
    }

    if($etime != ""){
        $where .= " AND g.`time` <= ". GetMkTime($etime." 23:59:59");
    }
	...
	$archives = $dsql->SetQuery("SELECT c.`id` FROM `#@__".$tab."` g left join `#@__job_company` c on g.`cid`=c.`id` left join `#@__member` m on c.`userid`=m.`id` WHERE 1 = 1");
	...
	$archives = $dsql->SetQuery("SELECT g.`admin`,g.`time` 'pubdate',g.`content`,g.`type`,c.`userid`,c.`combo_id`,c.`combo_enddate`,c.`cityid`,c.`id`, c.`title`, c.`logo`, c.`userid`, c.`people`, c.`contact`, c.`addrid`, c.`state`, c.`changeState`,c.`tag` FROM `#@__".$tab."` g left join `#@__job_company` c on g.`cid`=c.`id` left join `#@__member` m on m.`id`=c.`userid` WHERE 1 = 1".$where);
	...
	if(count($results) > 0){
		...
		foreach ($results as $key=>$value) {
			...
			$list[$key]["show_time"] = date("Y-m-d H:i:s",$value["pubdate"]);
            ...
            $adminTag = $value["tag"];
            if($adminTag){
                $adminTag = $dsql->getArr($dsql::SetQuery("select `typename` from `#@__job_companytag` where `id` in({$adminTag})"));
            }else{
                $adminTag = array();
            }
    ...
    if($do == "export"){
        ...
        array_push($tit, iconv('utf-8', 'gb2312//IGNORE', '在招职位'));
...
//绑定管理员·
}elseif($dopost == "catAdmin"){
	if(!testPurview("jobCompanyCat")){
		die('{"state": 200, "info": '.json_encode("对不起，您无权使用此功能！").'}');
	};
	...
	if($id != ""){
		foreach($each as $val){
			$sql = $dsql->SetQuery("update `#@__".$tab."` set `admin`= $admin WHERE `admin`=0 and `id` = ".$val);
			$res = $dsql->update($sql);
			if($res=="ok"){
			    //判断是否未释放而重复领取
                $sql = $dsql::SetQuery("select `id` from `#@__job_company_bind` where `admin`=$admin and `cid`=$val and `release_type`=0");
                $exist = $dsql->getOne($sql);
                if(!$exist){
			        $sql = $dsql::SetQuery("insert into `#@__job_company_bind`(`admin`,`cid`,`pubdate`) values($admin,$val,$time)");
                    $dsql->update($sql);
                }
            }else{
                $state .= $val;
