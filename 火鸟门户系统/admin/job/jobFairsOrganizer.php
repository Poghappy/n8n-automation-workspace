 ...
 * 管理招聘主办单位
 *
 * @version        $Id: jobFairsOrganizer.php 2015-3-16 上午10:09:21 $
 * @package        HuoNiao.Job
...
checkPurview("jobFairsOrganizer");
$dsql  = new dsql($dbo);
$tpl   = dirname(__FILE__)."/../templates/job";
$huoniaoTag->template_dir = $tpl; //设置后台模板目录

$tab = "job_fairs_organizer";
//css
$cssFile = array(
...
if($dopost != ""){
    $templates = "jobFairsOrganizerAdd.html";
    ...
    $jsFile = array(
        ...
        'publicUpload.js',
        'publicAddr.js',
        'admin/job/jobFairsOrganizerAdd.js'
    );
    $huoniaoTag->assign('jsFile', includeFile('js', $jsFile));
}else{
    $templates = "jobFairsOrganizer.html";
    ...
    $jsFile = array(
        ...
        'ui/chosen.jquery.min.js',
        'admin/job/jobFairsOrganizer.js'
    );
    $huoniaoTag->assign('jsFile', includeFile('js', $jsFile));
}
...
if($dopost == "getList"){
    ...
    $archives = $dsql->SetQuery("SELECT `id`,`title`,`phone`,`email`,`addrid`,`address` FROM `#@__".$tab."` WHERE 1 = 1");
...
//新增
    ...
    if($submit == "提交"){
        ...
        $lng = $lnglat[0];
        $lat = $lnglat[1];
        $archives = $dsql->SetQuery("INSERT INTO `#@__".$tab."` (`cityid`,`title`, `phone`, `fax`, `addrid`, `address`, `lng`, `lat`, `email`,`postCode`, `description`,`pubdate`) VALUES ($cityid,'$title', '$tel', '$fax', '$addr', '$address', '$lng', '$lat', '$email','$postCode', '$note', '".GetMkTime(time())."')");
...
//修改
    ...
    if($submit == "提交"){
        ...
        $lng = $lnglat[0];
        $lat = $lnglat[1];
        $archives = $dsql->SetQuery("UPDATE `#@__".$tab."` SET `cityid` = '$cityid', `title` = '$title', `phone` = '$tel', `fax` = '$fax', `addrid` = '$addr', `address` = '$address', `lng` = '$lng', `lat`='$lat', `email` = '$email', `description` = '$note',`postCode`='$postCode' WHERE `id` = ".$id);
