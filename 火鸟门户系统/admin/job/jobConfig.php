...
if($type != ""){
	...
	if($type == "site"){
        ...
        $custom_adminCompanyRelease    = (int)$adminCompanyRelease;
		$custom_fair_atlasMax  = (int)$fair_atlasMax;
		$resume_point          = (int)$_POST['resume_point'];
		$custom_fabuCheck      = (int)$fabuCheck;
		$custom_fabuResumeCheck = (int)$fabuResumeCheck;
		$customDataShare       = (int)$dataShareSwtich;
		$custom_jobFairJoinState       = (int)$jobFairJoinState;
		$customJobQQ       = $jobQQ;
		$customJobGsName       = $jobGsName;
		$customKefuImg       = $jobKefuImg;
		$customJobWechat       = $jobWechat;
		$customJobQrCode       = $jobQrCode;
        $customagentCheck      = (int)$agentCheck;
        $customNewCheck      = (int)$newCheck;
        $customFair_join_jobs      = (int)$fair_join_jobs;
        $customFair_join_jobs = $customFair_join_jobs ?: 1;
        $customPgCustomName      = $pgCustomName;
        $customPgCustomDescription      = $pgCustomDescription;
        $customChangeCheck      = (int)$changeCheck;
    ...
    elseif($type == 'charge'){
		...
		$customJob_fee = (float)$job_fee;
		if($customJob_fee<=0){
            die('{"state": 200, "info": '.json_encode("职位上架费用必须大于0元").'}');
        }

		$customResume_down_fee = (float)$resume_down_fee;
		if($customResume_down_fee<=0){
            die('{"state": 200, "info": '.json_encode("下载简历费用必须大于0元").'}');
        }

		$customJob_top_fee = (float)$job_top_fee;
		if($customJob_top_fee<=0){
            die('{"state": 200, "info": '.json_encode("职位置顶单天价格必须大于0元").'}');
        }

		$customJob_refresh_fee = (float)$job_refresh_fee;
		if($customJob_refresh_fee<=0){
            die('{"state": 200, "info": '.json_encode("职位单次刷新价格必须大于0元").'}');
        }

		$customFree_jobs = (int)$free_jobs;
		$customFree_job_resume_down = (int)$free_job_resume_down;
		$customFree_job_top = (int)$free_job_top;
		$customFree_job_refresh = (int)$free_job_refresh;

        adminLog("修改招聘设置", "设置基础收费规则");
    ...
    elseif($type == 'switch'){
        ...
        $customNewCheck = isset($newCheck) ? (int)$newCheck : $customNewCheck;
        $customChangeCheck = isset($changeCheck) ? (int)$changeCheck : $customChangeCheck;
        $customagentCheck = isset($agentCheck) ? (int)$agentCheck : $customagentCheck;
		$custom_fabuCheck = isset($fabuCheck) ? (int)$fabuCheck : $custom_fabuCheck;
		$custom_fabuResumeCheck = isset($fabuResumeCheck) ? (int)$fabuResumeCheck : $custom_fabuResumeCheck;
		$custom_jobFairJoinState = isset($jobFairJoinState) ? (int)$jobFairJoinState : $custom_jobFairJoinState;

        adminLog("修改招聘设置", "快速设置审核规则");
	...
	$customInc .= "\$custom_gs_atlasMax = ".(int)$custom_gs_atlasMax.";\r\n";
	$customInc .= "\$custom_adminCompanyRelease = ".(int)$custom_adminCompanyRelease.";\r\n";
	$customInc .= "\$custom_fair_atlasMax = ".(int)$custom_fair_atlasMax.";\r\n";
	$customInc .= "\$resume_point = ".(int)$resume_point.";\r\n";
	$customInc .= "\$custom_fabuCheck = ".(int)$custom_fabuCheck.";\r\n";
	$customInc .= "\$custom_fabuResumeCheck = ".(int)$custom_fabuResumeCheck.";\r\n";
	$customInc .= "\$custom_map = ".(int)$custom_map.";\r\n";
	$customInc .= "\$customDataShare = ".(int)$customDataShare.";\r\n";
	$customInc .= "\$customResume_down_fee = ".(float)$customResume_down_fee.";\r\n";
	$customInc .= "\$customJob_top_fee = ".(float)$customJob_top_fee.";\r\n";
	$customInc .= "\$customJob_fee = ".(float)$customJob_fee.";\r\n";
	$customInc .= "\$custom_jobFairJoinState = ".(float)$custom_jobFairJoinState.";\r\n";
	$customInc .= "\$customJob_refresh_fee = ".(float)$customJob_refresh_fee.";\r\n";
	$customInc .= "\$customJobQQ = '".$customJobQQ."';\r\n";
	$customInc .= "\$customJobGsName = '".$customJobGsName."';\r\n";
	$customInc .= "\$customKefuImg = '".$customKefuImg."';\r\n";
	$customInc .= "\$customFree_jobs = ".(int)$customFree_jobs.";\r\n";
	$customInc .= "\$customFree_job_resume_down = ".(int)$customFree_job_resume_down.";\r\n";
	$customInc .= "\$customFree_job_refresh = ".(int)$customFree_job_refresh.";\r\n";
	$customInc .= "\$customFree_job_top = ".(int)$customFree_job_top.";\r\n";
	$customInc .= "\$customJobWechat = '".$customJobWechat."';\r\n";
	$customInc .= "\$customJobQrCode = '".$customJobQrCode."';\r\n";
    $customInc .= "\$customagentCheck = " . (int)$customagentCheck . ";\r\n";
    $customInc .= "\$customNewCheck = " . (int)$customNewCheck . ";\r\n";
    $customInc .= "\$customFair_join_jobs = " . (int)$customFair_join_jobs . ";\r\n";
    $customInc .= "\$customPgCustomName = '" . $customPgCustomName . "';\r\n";
    $customInc .= "\$customPgCustomDescription = '" . $customPgCustomDescription . "';\r\n";
    $customInc .= "\$customChangeCheck = " . (int)$customChangeCheck . ";\r\n";
	//模板风格
...
if(file_exists($tpl."/".$templates)){
	...
	$huoniaoTag->assign('adminCompanyRelease', $custom_adminCompanyRelease);
	...
	$huoniaoTag->assign('fabuResumeCheckNum', array('0','1'));
	$huoniaoTag->assign('fabuResumeCheckNames', array('需要审核', '不需要审核'));
	$huoniaoTag->assign('fabuResumeCheckChecked', (int)$custom_fabuResumeCheck);
	...
	//简历下载费用
	$huoniaoTag->assign('resume_down_fee', (float)$customResume_down_fee);
	$huoniaoTag->assign('job_top_fee', (float)$customJob_top_fee);
	$huoniaoTag->assign('job_fee', (float)$customJob_fee);
	$huoniaoTag->assign('job_refresh_fee', (float)$customJob_refresh_fee);
	$huoniaoTag->assign('jobQQ', $customJobQQ);
	$huoniaoTag->assign('jobGsName', $customJobGsName);
	$huoniaoTag->assign('jobKefuImg', $customKefuImg);
	$huoniaoTag->assign('free_jobs', $customFree_jobs ?: 0);
	$huoniaoTag->assign('free_job_resume_down', $customFree_job_resume_down ?: 0);
	$huoniaoTag->assign('free_job_refresh', $customFree_job_refresh ?: 0);
	$huoniaoTag->assign('free_job_top', $customFree_job_top ?: 0);
	$huoniaoTag->assign('jobWechat', $customJobWechat);
	$huoniaoTag->assign('jobQrCode', $customJobQrCode);



    //商家新入驻审核
    $huoniaoTag->assign('newCheckNum', array('0','1'));
    $huoniaoTag->assign('newCheckNames', array('需要审核', '不需要审核'));
    $huoniaoTag->assign('newCheckChecked', (int)$customNewCheck);
    $huoniaoTag->assign('fair_join_jobs', (int)$customFair_join_jobs);
    $huoniaoTag->assign('pgCustomName', $customPgCustomName);
    $huoniaoTag->assign('pgCustomDescription', $customPgCustomDescription);

    //商家修改敏感信息审核
    $huoniaoTag->assign('changeCheckNum', array('1', '0'));
    $huoniaoTag->assign('changeCheckNames', array('需要审核', '不需要审核'));
    $huoniaoTag->assign('changeCheckChecked', (int)$customChangeCheck);
	...
	//地图配置
	$huoniaoTag->assign('mapList', array(0 => '系统默认', 1 => '谷歌', 2 => '百度', 4 => '高德', 5 => '天地图'));
	// $huoniaoTag->assign('mapList', array(2 => '百度'));  //暂只支持百度地图
    ...
    $huoniaoTag->assign('jobFairJoinStates', array('0', '1'));
    $huoniaoTag->assign('jobFairJoinStateNames',array('需要审核','不需要审核'));
    $huoniaoTag->assign('jobFairJoinStateCheck', (int)$custom_jobFairJoinState);
