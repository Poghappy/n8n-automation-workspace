...
if ($action == "getList") {
    ...
    $archives = $dsql->SetQuery("SELECT f.`id`, f.`uid`, f.`level`, f.`backcount`, m.`mtype`, m.`username`, m.`nickname`, m.`realname`, m.`from_uid`, m.`cityid`, m2.`username` recuser, m2.`nickname` recname, m2.`mtype` from_mtype, f.`state`, f.`pubdate`, f.`phone` FROM `#@__member_fenxiao_user` f LEFT JOIN `#@__member` m  ON m.`id` = f.`uid` LEFT JOIN `#@__member` m2  ON m2.`id` = m.`from_uid` WHERE 1 = 1" . $where);
    ...
    if ($results) {
        ...
        foreach ($results as $key => $value) {
            ...
            $list[$key]['backcount'] = $value['backcount'];
...
if ($action == 'updateState') {
    ...
    if ($id != "") {
        ...
        foreach ($each as $val) {
            ...
            if ($arcrank == 1) {
                ...
                //防止来回重复审核
                $archives = $dsql->SetQuery("SELECT `status`, `level` FROM `#@__member_fenxiao_user`  WHERE `id` = " . $val);
    ...
    $sql = $dsql->SetQuery("SELECT `id`,`status` FROM `#@__member_fenxiao_user` WHERE `uid` = $id");
    $res = $dsql->dsqlOper($sql, "results");
    $status = $res[0]['status'];        //防止重复审核
    if ($res) {
            ...
            if ($status == 0) {
                ...
                if ($cfg_fenxiaoRecAmount > 0) {
                    ...
                    if ($ret) {
                        ...
                        //防止来回重复审核
                        $archives = $dsql->SetQuery("UPDATE `#@__member_fenxiao_user` SET `status` = '1' WHERE `id` = " . $lastid);
                        $results = $dsql->dsqlOper($archives, "update");
                    }
                }
            }
    ...
    $count = (int)$count;
    $level = (int)$level;
    $sql = $dsql->SetQuery("UPDATE `#@__member_fenxiao_user` SET `backcount` = '$count', `level` = '$level' WHERE `id` = $id");
    $res = $dsql->dsqlOper($sql, "update");
    if ($res == 'ok') {
        die('{"state": 100, "info": ' . json_encode("操作成功") . '}');
    } else {
