...
if($dopost == "getList"){
    ...
    if($sType != ""){
        if($dsql->getTypeList($sType, "paimaitype")){
            global $arr_data;
            $arr_data = array();
            $lower = arr_foreach($dsql->getTypeList($sType, "paimaitype"));
    ...
    $where .= " order by `jointime` desc";

    $atpage = $pagestep*($page-1);
    $where .= " LIMIT $atpage, $pagestep";
    $archives = $dsql->SetQuery("SELECT `id`, `uid`,`addrid`,`stype`, `address`, `tel`, `state`, `jointime` FROM `#@__".$tab."` WHERE 1 = 1".$where);
    ...
    if(count($results) > 0){
        ...
        foreach ($results as $key=>$value) {
            ...
            $typeSql = $dsql->SetQuery("SELECT `typename` FROM `#@__paimaitype` WHERE `id` = ". $value["stype"]);
        ...
        if(count($list) > 0){
            echo '{"state": 100, "info": '.json_encode("获取成功").', "pageInfo": {"totalPage": '.$totalPage.', "totalCount": '.$totalCount.', "totalGray": '.$totalGray.', "totalAudit": '.$totalAudit.', "totalRefuse": '.$totalRefuse.'}, "paimaiStore": '.json_encode($list).'}';
        }else{
