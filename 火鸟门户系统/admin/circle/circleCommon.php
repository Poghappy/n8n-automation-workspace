...
checkPurview("circleCommon");
$dsql = new dsql($dbo);
$tpl = dirname(__FILE__)."/../templates/circle";
$huoniaoTag->template_dir = $tpl; //设置后台模板目录
$templates = "circleCommon.html";
...
if($userType == 3){
	//查询分类信息
	$sql = $dsql->SetQuery("SELECT `id` FROM `#@__circle_dynamic_all` WHERE `state` = 1" . getCityFilter('`cityid`'));
...
if($dopost == "getDetail"){
	...
	if(count($results) > 0){
		...
		$typeSql = $dsql->SetQuery("SELECT p.`content` title FROM `#@__circle_dynamic_all` p WHERE p.`id` = ". $results[0]["aid"]);
...
//更新评论信息
	...
	//会员通知
	$sql = $dsql->SetQuery("SELECT l.`id`, l.`content` title, l.`addtime` pubdate, l.`userid` uid, c.`userid`, c.`ischeck`, c.`dtime`,c.`rid`,c.`pid` FROM `#@__".$action."_comment_all` c LEFT JOIN `#@__circle_dynamic_all` l ON l.`id` = c.`aid` WHERE c.`id` = " . $id . $where);
	$ret = $dsql->dsqlOper($sql, "results");
	if($ret){
		$aid     = $ret[0]['id'];
		$title   = $ret[0]['title'];
...
//更新评论状态
	...
	foreach($each as $val){
		...
		//会员通知
		$sql = $dsql->SetQuery("SELECT l.`id`, l.`content` title, l.`addtime` pubdate, l.`userid` uid, c.`userid`, c.`ischeck`, c.`dtime`,c.`rid`,c.`pid` FROM `#@__".$action."_comment_all` c LEFT JOIN `#@__circle_dynamic_all` l ON l.`id` = c.`aid` WHERE c.`id` = " . $val.$where);
		$ret = $dsql->dsqlOper($sql, "results");
		if($ret){
			$aid     = $ret[0]['id'];
			$title   = $ret[0]['title'];
...
//获取评论列表
	...
	$where = " AND `type` = 'circle-dynamic'";

	if ($adminCity){
		$sql = $dsql->SetQuery("SELECT `id` FROM `#@__circle_dynamic_all` WHERE `state` = 1" . getWrongCityFilter('`cityid`', $adminCity));
		$ret = $dsql->dsqlOper($sql, "results");
		if($ret){
			foreach ($ret as $key => $value) {
				array_push($ids, $value['id']);
	...
	if($sKeyword != ""){
		...
		//按信息标题搜索
			...
			$archives = $dsql->SetQuery("SELECT l.`id` FROM `#@__circle_dynamic_all` l WHERE l.`title` like '%$sKeyword%'");
			$results = $dsql->dsqlOper($archives, "results");
			if(count($results) > 0){
				$list = array();
	...
	if(count($results) > 0){
		...
		foreach ($results as $key=>$value) {
			...
			$typeSql = $dsql->SetQuery("SELECT `content` title FROM `#@__circle_dynamic_all` WHERE `id` = ". $value["aid"]);
			$typename = $dsql->getTypeName($typeSql);
