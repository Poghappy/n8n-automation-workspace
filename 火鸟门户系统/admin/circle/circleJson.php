 ...
 * 文章模块JSON
 *
 * @version        $Id: circleJson.php 2013-7-7 上午10:33:36 $
...
$dotitle = $dopost == "circle" ? "新闻" : "图片";

$table_all = '#@__circle_dynamic_all';
...
if ($action == "") {
    ...
    //获取分表信息
    $sub = new SubTable('circle', '#@__circle_dynamic');

    //总条数
    $sql = $dsql->SetQuery("SELECT count(`id`) total FROM `#@__circle_dynamic_all` l WHERE `waitpay` = 0 AND `del` = $del");
    ...
    if (count($results) > 0) {
        ...
        foreach ($results as $key => $value) {
            ...
            $selfmedia_id = 0;
            $selfmedia_name = "";
            $sql = $dsql->SetQuery("SELECT `id`, `ac_name` FROM `#@__circle_selfmedia` WHERE `userid` = $admin");
            $ret = $dsql->dsqlOper($sql, "results");
            if ($ret) {
            ...
            // $auditResult = array();
            // if($need_audit){
            //     $auditResult = getInfoAudit("circle", $value['id'], $value["arcrank"]);
            // }
...
//删除
    ...
    if ($id != "") {
        ...
        if(preg_match('/founder/i', $purview)){
            ...
            $delArcrank = true;
            $auditConfig = getAuditConfig("circle");
        ...
        foreach ($each as $val) {
            if($auditSwitch){
                if($delArcrank){
                    $sql = $dsql->SetQuery("SELECT `audit_log` FROM `#@__circle_dynamic_all` WHERE `id` = $val");
                    $ret = $dsql->dsqlOper($sql, "results");
                    if($ret){
            ...
            // $sub = new SubTable('circle', '#@__circlelist');
            // $break_table = $sub->getSubTableById($id);
            $archives = $dsql->SetQuery("UPDATE `#@__circle_dynamic_all` SET `del` = 1 WHERE `id` = " . $val);
...
//彻底删除
    ...
    if ($id != "") {
        ...
        foreach ($each as $val) {
            ...
            // $sub = new SubTable('circle', '#@__circlelist');
            // $break_table = $sub->getSubTableById($id);
            $archives = $dsql->SetQuery("SELECT * FROM `#@__circle_dynamic_all` WHERE `id` = " . $val);
            ...
            //删除表
            $archives = $dsql->SetQuery("DELETE FROM `#@__circle_dynamic_all` WHERE `id` = " . $val);
            $results = $dsql->dsqlOper($archives, "update");
            if ($results != "ok") {
                $error[] = $val;
            }
...
//还原
    ...
    if ($id != "") {
        foreach ($each as $val) {
            // $sub = new SubTable('circle', '#@__circlelist');
            // $break_table = $sub->getSubTableById($id);
            $archives = $dsql->SetQuery("UPDATE `#@__circle_dynamic_all` SET `del` = 0 WHERE `id` = " . $val);
            $results = $dsql->dsqlOper($archives, "update");
            if ($results != "ok") {
                $error[] = $val;
            }
...
//添加属性
    ...
    if ($id != "") {
        foreach ($each as $val) {
            // $sub = new SubTable('circle', '#@__circlelist');
            // $break_table = $sub->getSubTableById($id);
            $archives = $dsql->SetQuery("SELECT `flag` FROM `#@__circle_dynamic_all` WHERE `id` = " . $val);
            $results = $dsql->dsqlOper($archives, "results");
            ...
            $archives = $dsql->SetQuery("UPDATE `#@__circle_dynamic_all` SET `flag` = '$flag' ".$flag_sql_val." WHERE `id` = " . $val);
            $results = $dsql->dsqlOper($archives, "update");
...
//移动分类
    ...
    if ($id != "") {
        foreach ($each as $val) {
            // $sub = new SubTable('circle', '#@__circlelist');
            // $break_table = $sub->getSubTableById($id);
            $archives = $dsql->SetQuery("UPDATE `#@__circle_dynamic_all` SET `typeid` = $typeid WHERE `id` = " . $val);
            $results = $dsql->dsqlOper($archives, "update");
            if ($results != "ok") {
                $error[] = $val;
            }
    ...
    if ($id != "") {
        ...
        if($auditConfig['switch']){
            ...
            if(!preg_match('/founder/i', $purview)){
                ...
                // 判断信息中是否有审核数据
                $sql = $dsql->SetQuery("SELECT `audit_log` FROM `#@__circle_dynamic_all` WHERE `id` = $id");
                $ret = $dsql->dsqlOper($sql, "results");
                if($ret){
                    if($ret[0]['audit_log']){
        ...
        foreach ($each as $val) {
            ...
            // $break_table = $sub->getSubTableById($id);
            //更新记录状态
            $archives = $dsql->SetQuery("UPDATE `#@__circle_dynamic_all` SET `arcrank` = $arcrank WHERE `id` = " . $val);
            $results = $dsql->dsqlOper($archives, "update");
            if ($results != "ok") {
                $error[] = $val;
            }
...
//获取指定ID信息详情
} else if ($action == "getcircleDetail") {
    if ($id != "") {
        $archives = $dsql->SetQuery("SELECT `typeid`, `title`, `subtitle`, `flag`, `arcrank` FROM `#@__circle_dynamic_all` WHERE `id` = " . $id);
        $results = $dsql->dsqlOper($archives, "results");
...
//更新快速编辑信息
    ...
    $archives = $dsql->SetQuery("UPDATE `#@__circle_dynamic_all` SET `typeid` = $typeid, `title` = '$title', `subtitle` = '$subtitle', `flag` = '$flags', `arcrank` = $arcrank  ".$flag_sql_val."WHERE `id` = " . $id);
    $results = $dsql->dsqlOper($archives, "update");
    if ($results != "ok") {
        echo $results;
    } else {
...
//更新自媒体领域
    ...
    if ($data != "") {
        ...
        $json = objtoarr($json);
        $json = typeAjax($json, 0, "circle_selfmedia_field");

        echo $json;
    }

//获取评论详细信息
} else if ($action == "getcircleCommonDetail") {
    if ($id != "") {
        ...
        if (count($results) > 0) {
            // $sub = new SubTable('circle', '#@__circlelist');
            // $break_table = $sub->getSubTableById($id);
            $archives = $dsql->SetQuery("SELECT `title` FROM `#@__circle_dynamic_all` WHERE `id` = " . $results[0]["aid"]);
...
//更新评论信息
    ...
    //会员通知
    $sql = $dsql->SetQuery("SELECT l.`title`, l.`admin`, l.`pubdate`, c.`aid`, c.`userid`, c.`ischeck`, c.`dtime`,c.`rid`,c.`pid` FROM `#@__" . $dopost . "common` c LEFT JOIN `#@__circle_dynamic_all` l ON l.`id` = c.`aid` WHERE c.`id` = $id");
    $ret = $dsql->dsqlOper($sql, "results");
    if ($ret) {
        $title = $ret[0]['title'];
        $admin = $ret[0]['admin'];
...
//更新评论状态
    ...
    if ($id != "") {
        foreach ($each as $val) {
            ...
            //会员通知
            $sql = $dsql->SetQuery("SELECT l.`title`, l.`admin`, l.`pubdate`, c.`aid`, c.`userid`, c.`ischeck`, c.`dtime`,c.`rid`,c.`pid` FROM `#@__" . $dopost . "common` c LEFT JOIN `#@__circle_dynamic_all` l ON l.`id` = c.`aid` WHERE c.`id` = $val");
            $ret = $dsql->dsqlOper($sql, "results");
            if ($ret) {
                $title   = $ret[0]['title'];
                $admin   = $ret[0]['admin'];
...
//删除评论
} else if ($action == "delcircleCommon") {
...
//获取评论列表
    ...
    if (count($results) > 0) {
        ...
        foreach ($results as $key => $value) {
            $list[$key]["id"] = $value["id"];
            $list[$key]["circleId"] = $value["aid"];

            // $tab = $sub->getSubTableById($value['aid']);
            $typeSql = $dsql->SetQuery("SELECT `id`, `title` FROM `#@__circle_dynamic_all` WHERE `id` = " . $value["aid"]);
            $typename = $dsql->getTypeName($typeSql);
            $list[$key]["circleTitle"] = $typename[0]['title'];
            ...
            $list[$key]["circleUrl"] = getUrlPath($param);
...
//操作审核流程
    ...
    $sql = $dsql->SetQuery("SELECT `audit_log`, `arcrank` FROM `#@__circle_dynamic_all` WHERE `id` = '$id'");
    $ret = $dsql->dsqlOper($sql, "results");
    if($ret){
        $audit_log = $ret[0]['audit_log'];
        ...
        if($audit_log){
            ...
            // 更新审核记录和状态
            $sql = $dsql->SetQuery("UPDATE `#@__circle_dynamic_all` SET `audit_state` = '$audit_state', `audit_log` = '$audit_log', `arcrank` = '$arcrank' WHERE `id` = $id");
...
//获取自媒体
    ...
    include_once HUONIAOROOT."/api/handlers/circle.class.php";
    $circle = new circle();
    $typeAll = $circle->selfmedia_type();
    $typeList = array();
    ...
    // 总数
    $sql = $dsql->SetQuery("SELECT count(`id`) as c FROM `#@__circle_selfmedia` WHERE 1 = 1" . getCityFilter('`cityid`') . $where);
    $res = $dsql->dsqlOper($sql, "results");
    $totalCount = $res[0]['c'];

    // 入驻待审核
    $sql = $dsql->SetQuery("SELECT count(`id`) as c FROM `#@__circle_selfmedia` WHERE (`state` = 0)" . getCityFilter('`cityid`') . $where);
    $res = $dsql->dsqlOper($sql, "results");
    $totalGray_join = $res[0]['c'];

    // 资料更新待审核
    $sql = $dsql->SetQuery("SELECT count(`id`) as c FROM `#@__circle_selfmedia` WHERE (`state` = 1 AND `editstate` = 0)" . getCityFilter('`cityid`') . $where);
    $res = $dsql->dsqlOper($sql, "results");
    ...
    // 入驻已审核
    $sql = $dsql->SetQuery("SELECT count(`id`) as c FROM `#@__circle_selfmedia` WHERE (`state` = 1)" . getCityFilter('`cityid`') . $where);
    $res = $dsql->dsqlOper($sql, "results");
    $totalAudit_join = $res[0]['c'];

    // 入驻拒绝审核
    $sql = $dsql->SetQuery("SELECT count(`id`) as c FROM `#@__circle_selfmedia` WHERE (`state` = 2)" . getCityFilter('`cityid`') . $where);
    $res = $dsql->dsqlOper($sql, "results");
    $totalRefuse_join = $res[0]['c'];

    // 资料更新拒绝审核
    $sql = $dsql->SetQuery("SELECT count(`id`) as c FROM `#@__circle_selfmedia` WHERE (`state` = 1 && `editstate` = 2)" . getCityFilter('`cityid`') . $where);
    $res = $dsql->dsqlOper($sql, "results");
    ...
    $archives = $dsql->SetQuery("SELECT `id`, `cityid`, `type`, `userid`, `ac_name`, `ac_photo`, `state`, `editstate`, `pubdate` FROM `#@__circle_selfmedia` WHERE 1 = 1" . getCityFilter('`cityid`') . $where);
    $results = $dsql->dsqlOper($archives, "results");
    // echo $archives;die;
    if($results){
        $list = array();
...
//更新自媒体状态
    ...
    foreach ($each as $val) {
        ...
        $field = "";
        $userid = 0;
        $sql = $dsql->SetQuery("SELECT `id`, `state`, `editstate`, `editlog`, `userid` FROM `#@__circle_selfmedia` WHERE `id` = $val".$where);
        $res = $dsql->dsqlOper($sql, "results");
        if($res){
            $state_  = $res[0]['state'];
        ...
        if($field == ""){
            ...
            $sql = $dsql->SetQuery("UPDATE `#@__circle_selfmedia` SET {$field} WHERE `id` = $val");
            ...
            if($res == "ok"){
                ...
                if($type == "join"){
                    ...
                    $sql = $dsql->SetQuery("UPDATE `#@__circle_dynamic_all` SET `media_state` = $state WHERE `admin` = $userid AND `media_state` != $state");
...
//删除自媒体
    ...
    if ($id != "") {
        ...
        foreach ($each as $val) {
            ...
            $archives = $dsql->SetQuery("SELECT * FROM `#@__circle_selfmedia` WHERE `id` = " . $val);
            $results = $dsql->dsqlOper($archives, "results");
            $detail = $results[0];
            ...
            //删除缩略图
            delPicFile($results[0]['ac_photo'], "delThumb", "circle");
            delPicFile($results[0]['mb_license'], "delThumb", "circle");
            delPicFile($results[0]['op_idcardfront'], "delThumb", "circle");
            delPicFile($results[0]['org_major_license'], "delThumb", "circle");
            if($results[0]['op_authorize']){
                $prove = explode(",", $results[0]['prove']);
                foreach ($prove as $key => $value) {
                    $info = explode("|", $value);
                    delPicFile($info[0], "delAtlas", "circle");
                }
            }
            if($results[0]['prove']){
                $prove = explode(",", $results[0]['prove']);
            ...
            //删除表
            $archives = $dsql->SetQuery("DELETE FROM `#@__circle_selfmedia` WHERE `id` = " . $val);
            $results = $dsql->dsqlOper($archives, "update");
            if ($results != "ok") {
                $error[] = $val;
...
} else if ($action == "getcircleType"){
    $mold = (int)$mold;
    echo json_encode($dsql->getTypeList(0, "circletype", true, 1, 100, " AND `mold`=".$mold));
    die;

// 获取没有视频时长和封面的信息
} else if ($action == "checkVideotime_face"){
    $pageSize = (int)$pageSize ? (int)$pageSize: 1;
    $sql = $dsql->SetQuery(
        "SELECT `id`, `type`, `thumbnail`, `videotime`,`videoadr` FROM `#@__circle_dynamic_all`
        WHERE
        `type` = 1
        AND `videotime` = 0
        AND `thumbnail` = ''
    ...
    if(is_array($res)){
        foreach ($res as $key => $value) {
            array_push($list, array(
                'id' => $value['id'],
                'type' => 0,
                'videoadr' => getFilePath($value['videoadr']),
...
// 更新视频时长
    ...
    if($id && $type){
        if($type == 'time'){
            ...
            $sql = $dsql->SetQuery("UPDATE `#@__circle_dynamic_all` SET `videotime` = '$videotime' WHERE `id` = $id AND `videotime` = ''");
        }
        if($type == 'face'){
            if(empty($litpic)) return;
            $sql = $dsql->SetQuery("SELECT `thumbnail` FROM `#@__circle_dynamic_all` WHERE `id` = $id");
            $res = $dsql->dsqlOper($sql, "results");
            if($res && $res[0]['thumbnail']){
                return;
                delPicFile($res[0]['thumbnail'], "delThumb", "circle");
            }
            $sql = $dsql->SetQuery("UPDATE `#@__circle_dynamic_all` SET `thumbnail` = '$litpic' WHERE `id` = $id");
        }
        $res = $dsql->dsqlOper($sql, "update");
        if ($res == "ok") {
...
//模糊匹配会员
  ...
  if(!empty($key)){
    ...
    $userSql = $dsql->SetQuery("SELECT m.`id`, m.`username`, m.`company`, m.`nickname`, s.`ac_name` FROM `#@__member` m LEFT JOIN `#@__circle_selfmedia` s ON s.`userid` = m.`id` WHERE (m.`username` like '%$key%' OR m.`company` like '%$key%' OR m.`nickname` like '%$key%' OR s.`ac_name` LIKE '%$key%')".$where." LIMIT 0, 10");
    // echo $userSql;die;
  ...
  die;
    ...
    if($title){
        $where = $id ? " AND `id` <> $id" : "";
        $sql = $dsql->SetQuery("SELECT `id` FROM `#@__circle_dynamic_all` WHERE `title` = '$title' AND `del` = 0".$where);
        $ret = $dsql->dsqlOper($sql, "results");
        if($ret){
            echo $ret[0]['id'];
        }else{
    ...
    $type = (int)$_POST['type'];
    echo json_encode($dsql->getTypeList(0, "circle_selfmedia_field", true, 1, 100));
    die;
// 获取专题分类
}else if($action == 'getZhuantiType'){
    $type = (int)$_POST['type'];
    echo json_encode($dsql->getTypeList($type, "circle_zhuanti", true, 1, 100));
    die;
...
//更新专题分类
    ...
    $where .= " AND `path` regexp '/circle/editor/video/(.*).mp4' ORDER BY `id` LIMIT 10";
...
function memberNotice($id, $arcrank)
{
    ...
    //查询信息之前的状态
    $sql = $dsql->SetQuery("SELECT `title`, `arcrank`, `admin`, `pubdate` FROM `#@__circle_dynamic_all` WHERE `id` = $id");
    $ret = $dsql->dsqlOper($sql, "results");
    if ($ret) {
...
// 检查缓存
function checkcircleCache($id){
    checkCache("circle_list", $id);
    clearCache("circle_detail", $id);
}
function checkMediaCache($id, $type = "", $userid = 0, $state = 0){
    checkCache("circle_media_list", $id);
    clearCache("circle_media_detail", $id);
    // 删除或者取消审核时
    if($type == "del" || ($type == "join" && $state != 1) ){
        global $dsql;
        $sql = $dsql->SetQuery("SELECT `id` FROM `#@__circle_dynamic_all` WHERE `admin` = $userid");
        $ids = $dsql->dsqlOper($sql, "results");
        if($ids){
            checkcircleCache(array_column($ids, "id"));
        }
    }
}
function clearcircleTypeCache(){
    for($i = 1; $i < 200; $i++){
        clearCache("circletype_par", $i);
    }
}
