...
$content = '';
if ($reportData != null && is_array($reportData) && isset($reportData['data'])) {
    $content = json_decode($reportData['data'], true);
}
...
if ($content != null && is_array($content)) {
    ...
    $dataid = trim($content['printId']);
    $status = (int)$content['status'];
    //打印成功才进行处理
    if ($dataid != null && $status == 2) {
