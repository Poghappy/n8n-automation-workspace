 ...
 * XorPay 微信
 *
 * @version        $Id: xorpay_wxpay.php $v1.0 2024-4-10 下午17:54:31 $
...
if(isset($set_modules) && $set_modules == TRUE){
    ...
    /* 代码 */
    $payment[$i]['pay_code'] = "xorpay_wxpay";

	/* 名称 */
    $payment[$i]['pay_name'] = "微信个人收款";

    /* 版本号 */
    $payment[$i]['version']  = '1.0.0';

    /* 描述 */
    $payment[$i]['pay_desc'] = '「XorPay」作为 微信支付服务商 和 支付宝系统服务商ISV 专业为开发者 / 个体户 / 小微企业提供正规、安全、稳定的创收支持，该支付方式支持【PC电脑端、微信h5端】，不支持【微信小程序端、APP端、手机浏览器端】，官网：https://xorpay.com';
...
class xorpay_wxpay {
    ...
    function get_code($order, $payment){
		...
		$redirect_url = return_url("xorpay_wxpay", $paramUrl);
		$notify_url = notify_url("xorpay_wxpay", $paramUrl);
        ...
        $pay_type = 'jsapi';  //支付方式  native  jsapi
		...
		$_yabandPay = new CLogFileHandler(HUONIAOROOT . '/log/xorpay_wxpay/'.date('Y-m-d').'-create.log', true);
		$_yabandPay->DEBUG("报文：" . json_encode($params, JSON_UNESCAPED_UNICODE));

        $httpUrl = 'https://xorpay.com/api/cashier/' . aid . '?' . hn_build_query($params);
    ...
    function respond(){
        ...
        // 加载支付方式操作函数
        loadPlug("payment");
        $payment  = get_payment("xorpay_wxpay");
        ...
        //初始化日志
		require_once dirname(__FILE__)."/../log.php";
		$_yabandPay = new CLogFileHandler(HUONIAOROOT . '/log/xorpay_wxpay/'.date('Y-m-d').'-respond.log', true);
