var page = new Vue({
	el:'#page',
	data:{
		navList:navList,  //左侧导航
		currid:currid, //左侧导航当前高亮
		hoverid:'',
		list:[],  //数据
		editArr:[],  //批量编辑
		shaixuanid:0,
		loading:false,
	},
	mounted(){
		var tt = this;
		tt.showPageInfo();
		tt.getList();


	},
  methods:{
    // 显示切换账户
		show_change:function(){
			$(".change_account").show()
		},

		// 隐藏切换账户
		hide_change:function(){
			$(".change_account").hide()
		},

		// 获取列表
		getList:function(){
			var tt = this;
			// atpage  页码  pageSize 单页加载数据条数      totalCount  数据总数  需在加载之后复制
			if(tt.loading) return false;
			tt.loading = true;
			tt.showPageInfo();
			var keywords = $("#search").val();
			axios({
				method: 'post',
				url:'/include/ajax.php?service=house&action=loupanNewsList&loupanid='+loupanid+"&page="+atpage+"&pageSize=8&keywords="+keywords,
			  })
			  .then((response)=>{
					tt.loading = false;
					var data = response.data;
					if(data.state == 100){
						tt.list = data.info.list
						totalCount = data.info.pageInfo.totalCount;
						if(atpage==1){
							tt.showPageInfo(currid);
						}
					}else{
						totalCount = 0;
						tt.list = [];
						$(".noData").html(data.info)
					}
			  });
		},
		// 搜索
		search:function(){
			var tt = this;
			atpage = 1;
			tt.getList();
		},
		// 批量选择
		edit_pl:function(id,i){
			var tt = this;
			if(tt.editArr.indexOf(id)>-1){
				tt.editArr.splice(tt.editArr.indexOf(id),1)
			}else{
				tt.editArr.push(id)
			}
		},
		del_article:function(){
			var tt = this;
			var id = '';
			if(tt.editArr.length==0){ //非批量操作
				id = $(".Popbox").attr('data-id')
				index = $(".Popbox").attr('data-index');
			}else{
				id = tt.editArr.join(',');
			}

			if(tt.loading) return false;
			tt.loading = true;
			axios({
				method: 'post',
				url:'/include/ajax.php?service=house&action=loupanDel&dopost=delAlbum&deltype=5&loupanid='+loupanid+"&delid="+id,
			  })
			  .then((response)=>{
				var data = response.data;
				tt.loading = false;
				if(data.state == 100){
					alert('删除成功');
					tt.close_pop();
					atpage = 1;
					tt.getList();
				}else{
					alert(data.info)
				}
			 });
		},
		showPop:function(id,index){
			var tt = this;
			if(id == '' && tt.editArr.length == 0){
				alert('请至少选择1个');
				return false;
			}
			$(".Popbox").attr('data-id',id)
									.attr('data-index',index);
			$(".Popbox,.pop_mask").show();
		},
		close_pop(){
			$(".Popbox,.pop_mask").hide();
		},
		// 分页
		showPageInfo:function() {
			var tt = this;
			var info = $(".pagination");
			var nowPageNum = atpage;
			var allPageNum = Math.ceil(totalCount/pageSize);
			var pageArr = [];
			info.html("").hide();
			var pages = document.createElement("div");
			pages.className = "pagination-pages";
			info.append(pages);
			//拼接所有分页
			if (allPageNum > 1) {

				//上一页
				if (nowPageNum > 1) {
					var prev = document.createElement("a");
					prev.className = "prev";
					prev.innerHTML = langData['siteConfig'][6][33];//上一页
					prev.onclick = function () {
						atpage = nowPageNum - 1;
						tt.getList();
					}
					info.find(".pagination-pages").append(prev);
				}

				//分页列表
				if (allPageNum - 2 < 1) {
					for (var i = 1; i <= allPageNum; i++) {
						if (nowPageNum == i) {
							var page = document.createElement("span");
							page.className = "curr";
							page.innerHTML = i;
						} else {
							var page = document.createElement("a");
							page.innerHTML = i;
							page.onclick = function () {
								atpage = Number($(this).text());
								tt.getList();
							}
						}
						info.find(".pagination-pages").append(page);
					}
				} else {
					for (var i = 1; i <= 2; i++) {
						if (nowPageNum == i) {
							var page = document.createElement("span");
							page.className = "curr";
							page.innerHTML = i;
						}
						else {
							var page = document.createElement("a");
							page.innerHTML = i;
							page.onclick = function () {
								atpage = Number($(this).text());
								tt.getList();
							}
						}
						info.find(".pagination-pages").append(page);
					}
					var addNum = nowPageNum - 4;
					if (addNum > 0) {
						var em = document.createElement("span");
						em.className = "interim";
						em.innerHTML = "...";
						info.find(".pagination-pages").append(em);
					}
					for (var i = nowPageNum - 1; i <= nowPageNum + 1; i++) {
						if (i > allPageNum) {
							break;
						}
						else {
							if (i <= 2) {
								continue;
							}
							else {
								if (nowPageNum == i) {
									var page = document.createElement("span");
									page.className = "curr";
									page.innerHTML = i;
								}
								else {
									var page = document.createElement("a");
									page.innerHTML = i;
									page.onclick = function () {
										atpage = Number($(this).text());
										tt.getList();
									}
								}
								info.find(".pagination-pages").append(page);
							}
						}
					}
					var addNum = nowPageNum + 2;
					if (addNum < allPageNum - 1) {
						var em = document.createElement("span");
						em.className = "interim";
						em.innerHTML = "...";
						info.find(".pagination-pages").append(em);
					}
					for (var i = allPageNum - 1; i <= allPageNum; i++) {
						if (i <= nowPageNum + 1) {
							continue;
						}
						else {
							var page = document.createElement("a");
							page.innerHTML = i;
							page.onclick = function () {
								atpage = Number($(this).text());
								tt.getList();
							}
							info.find(".pagination-pages").append(page);
						}
					}
				}

				//下一页
				if (nowPageNum < allPageNum) {
					var next = document.createElement("a");
					next.className = "next";
					next.innerHTML = langData['siteConfig'][6][34];//下一页
					next.onclick = function () {
						atpage = nowPageNum + 1;
						tt.getList();
					}
					info.find(".pagination-pages").append(next);
				}

				info.show();

			}else{
				info.hide();
			}
		},

  }
})
