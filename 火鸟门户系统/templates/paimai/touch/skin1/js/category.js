            ...
            if(data.state == 100){
                ...
                if(list.length > 0){
                    for(var i = 0; i < list.length; i++){//一级分类
                        ...
                        if(list[i].lower && list[i].lower!='' ){//有二级分类
                            var dlist = list[i].lower;
                            for(var j = 0;j < dlist.length; j++){
                                var sub = dlist[j].sublower;
                                ...
                                if(sub){//有三级分类
                                    ...
                                    for(var m = 0;m < sub.length; m++){
                                        html.push('<li>');
                                        html.push('<a href="'+channelDomain+'/list.html?typeid='+sub[m].id+'&pid='+list[i].id+'" class="toMini" data-module="shop" data-temp="list" data-id="'+sub[m].id+'" data-typename="'+sub[m].typename+'"> ');
                                    ...
                                    html2.push('<li>');
                                    html2.push('<a href="'+channelDomain+'/list.html?typeid='+dlist[j].id+'&pid='+list[i].id+'" class="toMini" data-module="shop" data-temp="list" data-id="'+dlist[j].id+'" data-typename="'+dlist[j].typename+'">');
                          ...
                          if(html2!=''){
                            ...
                            html.push('<ul class="fn-clear">')
                            html.push('<li>');
                            html.push('<a href="'+channelDomain+'/list.html?typeid='+list[i].id+'" class="toMini" data-module="shop" data-temp="list" data-id="'+list[i].id+'" data-typename="'+list[i].typename+'">');
                            html.push('<div class="imgbox">');
