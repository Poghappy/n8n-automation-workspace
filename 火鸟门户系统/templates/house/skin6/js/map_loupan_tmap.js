$(function(){
	...
	var map, filterData, list = $(".list"), init = {
		...
		,tilesloaded: function(){
			...
			//楼盘
				...
				newView.lng && newView.lat ? (map.centerAndZoom(new T.LngLat(newView.lng, newView.lat), newView.typ), t.addClass("clicked")) : map.setZoom(newView.typ);
			...
			list.delegate("dl", "click", function(){
				...
				isClickHx = true;
				t.lng && t.lat ? map.centerAndZoom(new T.LngLat(t.lng, t.lat), t.typ) : map.setZoom(t.typ);
			...
			$.ajax({
				...
				"success": function(data){
					...
					if(data && data.state == 100){
						...
						init.loupanNearbyInfo(ret, loupanChooseData);
		...
		,closeHx: function(){
			...
			isClickHx = false;
			$(".map-os").show();
			loupanChooseData.longitude && loupanChooseData.latitude ? (map.centerAndZoom(new T.LngLat(loupanChooseData.longitude, loupanChooseData.latitude), 14)) : map.setZoom(14);
			init.updateOverlays();
		...
		//附近信息
		,loupanNearbyInfo: function(hxData, loupanChooseData){
            ...
            init.createBubble(loupanChooseDataArr, bubbleTemplate[4], 2, bubbleTemplate.moreTpl);
			map.centerAndZoom(new T.LngLat(loupanChooseData.longitude, loupanChooseData.latitude), 15);
	...
	var bubbleMapSize = {
		...
		,bubbleTemplate = {
			...
			//区域
			1 : '<div style="text-align: center;" class="bubble bubble-1" data-longitude="${longitude}" data-latitude="${latitude}" data-id="${loupan_id}"><p class="name" title="${district_name}区">${district_name}区</p><p><span class="count">${count}</span>个楼盘</p></div>',
			...
			//周边信息
			4 : '<div class="bubble bubble-4 clicked" data-longitude="${longitude}" data-latitude="${latitude}" data-id="${loupan_id}"><div class="bubble-wrap"><div class="bubble-inner"><span class="close">&times;</span><a href="${url}" target="_blank"><p class="name" title="${resblock_name}">${resblock_name}</p>${moreTpl}</a></div><i class="arrow"><i class="arrow-i"></i></i></div><p class="cycle"></p></div>',
