    ...
    markerTpl = $.template("<i class='item <%=itemIcon%>' data-label='<%=itemIndex%>' title='<%=title%>'></i>"),
        ...
        mapInit = function () {
            t = new T.Map("map", {
                ...
                n = new T.LngLat(longitude, latitude),
                !0 && t.addControl(new T.Control.Zoom({ position: T_ANCHOR_TOP_LEFT })),
        ...
        renderTagBox = function () {
            ...
            $.each(e, function (e, i) {
                ...
                $(".itemTagBox").html(a),
                liClick(),
                $('.selectTag').size() == 0 ? i.first().trigger("click") : null;
        }
        ,
        liClick = function () {
            i.on("click", function() {
        ...
        searchDeal = function (e) {
            var i = e;
            bdLocalSearch = new T.LocalSearch(t),
                bdLocalSearch.searchNearby(i, n, 2e3),
                bdLocalSearch.setSearchCompleteCallback(function (e) {
                    var t = [], pois = e.getPois();
                    for (var n = 0; n < pois.length; n++){
                        var stationData = [];
                        if(pois[n].stationData && pois[n].stationData != undefined && pois[n].stationData.length > 0){
                            for(var i = 0; i < pois[n].stationData.length; i++){
                                stationData.push(pois[n].stationData[i].lineName);
                            }
                        }
                        if(stationData.length > 0){
                            pois[n].address = stationData.join(';');
                        }
                        t.push(pois[n]);
                    }
        ...
        createCircle = function() {
            ...
            //定义该矩形的显示区域
            var circle = new T.Circle(n, 2000, config);
            t.addOverLay(circle);
        },
        render = function () {
            var e = a[c]
            ...
            if (t.clearOverLays(),
                ...
                $.each(e, function (e, t) {
                    var n = searchItemTpl.render({
                        keyword: c,
                        title: t.name.replace('-' + $('.tagStyle.select').text(), ''),
                        distance: t.distance + '米',
                        address: t.address
                    });
                    var lonlat = t.lonlat;
                    var lonlatArr = lonlat.split(' ');
                    t.lnglat = new T.LngLat(lonlatArr[0], lonlatArr[1]);
                    i += "<li data-index=" + c + e + " data-address=" + lonlat[0] + "," + lonlat[1] + " title=" + t.name + "><div class='contentBox'>" + n + "</div></li>",
        ...
        addItemOverlays = function (e, n, a) {
            var i = markerTpl.render({
                ...
                , o = new T.Label({
                    text: i,
                    position: a.lnglat,
                    offset: new T.Point(-30, -25)
                });
                o.setBackgroundColor("transparent"),
                o.setBorderLine(0),
                t.addOverLay(o),
                $(".tdt-label").eq(0).css("z-index", 2),
                labelClick(o, n, a)
        }
        ,
        renderMarkerDetail = function (e, n) {
            var a = searchItemTpl.render({
                keyword: c,
                title: n.name.replace('-' + $('.tagStyle.select').text(), ''),
                distance: n.distance + '米',
                address: n.address
            })
        ...
        cancelBlue = function(e) {
            "click" == e ? ($(".contentBox").removeClass("contentActive"),
                ...
                $(".tdt-label").removeClass("labelUp"),
                $(".tdt-label .item").removeClass("blueLabel"),
            f && setBlue(f, "click")
        }
        ,
