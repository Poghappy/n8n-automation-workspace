...
function getList(is,orderby){
	...
	$(".pagination").hide();
	$.ajax({
		url: "/include/ajax.php?service=live&action=chatRoomPaySeeList&chatid="+chatid+"&page="+atpage+"&pageSize="+pageSize+"&keywords="+keywords+"&orderby="+(orderby?orderby:''),
		type: "GET",
		dataType: "json",
				...
				if(data.state == 101){
					...
					if(list.length > 0){
						...
						for(var i = 0; i < list.length; i++){
							...
							html.push('<h4>'+userinfo.nickname+'</h4>');
							html.push('<p>ID：'+userid+'</p>');
							html.push('<p>'+(userinfo.sex == '0' ? '女' : '男')+'</p></div></div>');
							html.push('<div class="ic_time"><p>'+date+'</p></div>');
							html.push('<div class="ic_money">'+echoCurrency('symbol')+'<em>'+amount+'</em></div>');
							html.push('<div class="ic_money" style="font-weight: 500;">'+seetype+'</div></div></li>');
						}
