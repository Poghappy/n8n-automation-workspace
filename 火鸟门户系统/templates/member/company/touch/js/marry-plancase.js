$(function () {
    ...
    function getList(tr){
        ...
        $.ajax({
            url: masterDomain+"/include/ajax.php?service=marry&action=plancaseList&u=1&orderby=5&stypeid="+typeid+"&state="+state+"&page="+atpage+"&pageSize="+pageSize,
            type: "GET",
            dataType: "jsonp",
            success: function (data) {
                isload = false;
                if(data && data.state == 100){
                    ...
                    for (var i = 0; i < list.length; i++) {
                        ...
                        html.push('<dd class="item-area"><span>'+huoniao_.transTimes(list[i].holdingtime, 2)+'</span></dd>');
                        html.push('</a></dl>');
                        html.push('</div>');
