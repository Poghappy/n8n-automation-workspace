...
function getList(is){
  ...
  isload = true;
	...
	$.ajax({
		url: masterDomain+"/include/ajax.php?service=paimai&action=orderList&store=1&state="+state+"&page="+atpage+"&pageSize="+pageSize,
		type: "GET",
		dataType: "jsonp",
		success: function (data) {
			if(data && data.state != 200){
				if(data.state == 101){
					...
					if(state == ''){
						$("#all").html(pageInfo.totalCount)
						$("#toPay").html(pageInfo.state7);
						$("#end").html(pageInfo.state4);
						$("#tuikuan").html(pageInfo.state5);
						$("#toFa").html(pageInfo.state1);
						$("#toFahuo").html(pageInfo.state6);
						// $("#guoqi").html(pageInfo.state2);
						$("#shouhuo").html(pageInfo.state3);
					}
					...
					if(list.length > 0){
						for(var i = 0; i < list.length; i++){
								var item       = [],
										id         = list[i].id,
										company    = list[i].store.company,
										ordernum   = list[i].ordernum,
										proid      = list[i].proid,
										procount   = list[i].procount,
										orderprice = list[i].amount,
								...
								var stateInfo = btn = "";
								var urlString = paimaiEditUrl.replace("%id%", id);
								...
								switch(orderstate){
									...
									case "6":
										...
										break;
									case "7":
										orderprice = list[i].product.money,
								...
								html.push('<p class="mprice">'+(list[i].type == 'regist' && list[i].orderstate != 7 ? "保证金" : "竞拍价")+'</p>');
								html.push('</div>');
								html.push('</div>');
								html.push('</a>');
