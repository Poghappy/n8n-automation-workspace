...
$(function () {
    ...
    function getList(tr) {
        ...
        $.ajax({
            url: "/include/ajax.php?service=" + module + "&action=getrefund&page=" + orderpage + "&pageSize=" + pageSize + "&id=" + id,
            type: "GET",
            dataType: "json",
                ...
                if (data && data.state == 100) {
                    ...
                    if (list.length > 0) {
                        ...
                        for (var i = 0; i < list.length; i++) {
                            ...
                            html.push('<div class="peoimg ' + (list[i].status == 3 ? 'spe' : '') + '">');
                            if (list[i].status == 3) {
                            ...
                            if(list[i].service == 1){
                                ...
                                if (list[i].status == 1 && list[i].type == 3) {
                                    html.push('<div class="infoBot">');
                                    html.push('<p>您已经同意退款 </p>');
                                    html.push('<p>退款成功 </p>');
                                    html.push('</div>');
                                } else if (list[i].status == 2 && list[i].type == 3) {
                                    html.push('<div class="infoBot">');
                                    html.push('<p>您拒绝了退款 </p>');
                                    html.push('<p>原因：'+list[i]['retnote']+' </p>');
                                    html.push('</div>');
                                } else if (list[i].type == 2 && list[i].status == 1) {
                                    html.push('<div class="infoBot">');
                                    html.push('<p>平台客服介入处理完成：已退款￥' + list[i].price + '</p>');
                                    html.push('<p>退回至买家的账户余额</p>');
                                    html.push('</div>');
                                } else {
                                    html.push('<div class="infoBot">');
                                    html.push('<p>买家提交了退款申请 </p>');
