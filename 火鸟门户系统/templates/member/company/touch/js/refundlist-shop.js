                    ...
                    if(list.length > 0){
                        ...
                        for(var i = 0; i < list.length; i++){
                            ...
                            switch(orderstate){
                                ...
                                case "6":
                                    ...
                                    if(retState == 1){
                                        ...
                                        if(list[i].ret_audittype == 0){
                                            ...
                                            stateInfo = '<p class="order-state"><span>您已拒绝退款，等待买家处理中</span></p>';
                                        }
                                ...
                                // html.push('<dt><a href="'+store.domain+'"><p class="shop_name"><i></i><span>'+member.nickname+'</span></p></a></dt>')
                                var chatUrl = imUrl.replace('chat-0','chat-'+list[i].userid)
                                console.log(imUrl,chatUrl)
                                 html.push('<dt><a href="'+chatUrl+'"><p class="shop_name"><i></i><span>'+member.nickname+'</span></p></a></dt>')
                                html.push('<dd class="order-content">');
                                var totalCount = 0;
                                for(var p = 0; p < product.length; p++){
                                    html.push('<a href="'+detailUrl+'"><div class="fn-clear">');
                                    html.push('<div class="imgbox-l"><img src="'+product[p].litpic+'" onerror="this.src=\'/static/images/404.jpg\'" alt="" /></div>');
                                    var specTxt = ''
                                    ...
                                    html.push('<div class="txtbox-c"><p>'+product[p].title+'</p><h3 class="spec">1111</h3>'+specTxt+'<h4 class="tkp">退款:'+(echoCurrency('symbol'))+totalPayPrice+'</h4></div>');
                                    html.push('</div></a>');
                                }
                                html.push('<div class="tk-type"><strong>【'+(list[i].user_refundtype == '0' ? "仅退款" : "退货退款")+'】</strong>'+stateInfo+'</div>')
