$(function(){
     if($(".inp.radio .mtype.curr").attr('data-id')=='0'){
		 var h = $(window).height() - $(".inpbox.singel_inp.coninp").offset().top;
		 $(".inpbox.singel_inp.coninp").css("min-height",h);
	 }else{
		 var h = $(window).height() - $(".inpbox.singel_inp.linkinp").offset().top;
		 $(".inpbox.singel_inp.coninp").css("min-height",h);
	 }
	 // 切换类型
	 $(".inp.radio .mtype").click(function(){
		 var t = $(this),tid = t.attr('data-id');
		 t.addClass('curr').siblings('span').removeClass('curr');
		 if(tid=='0'){
			 $(".singel_inp").addClass('fn-hide')
			 $(".singel_inp.coninp").removeClass('fn-hide').show();
		 }else{
			 $(".singel_inp").addClass('fn-hide')
			 $(".singel_inp:not(.coninp)").removeClass('fn-hide').show();
		 }
		 $("#menutype").val(tid)
	 });
	...
	var upvideoShow = new Upload({
		...
		fileQueued: function(file) {
			...
			$(".singel_inp.coninp .textarea").append('<div class="video_li" id="' + file.id + '"><span class="load">'+langData['business'][9][39]+'~</span></div>');   //视频上传中~
		},
		uploadSuccess: function(file, response) {

			if (response.state == "SUCCESS") {console.log(response)
				$('#' + file.id).html('<div class="img_show"><img style="display: none;" class="vposter fn-hide" src="'+response.poster+'"><video src="' + response.turl + '" data-url="' + response.url +'" /></div><div class="btns-group"><span style="100%" class="btn_del">'+langData['siteConfig'][6][8]+'</span></div>');  //删除
			}
		},
	...
	// 删除视频
	$('.coninp').delegate('.video_li', 'click', function() {
		var t = $(this);
	...
	$('.coninp').delegate('.video_li .btn_del', 'click', function() {
		...
		showErrAlert(langData['business'][9][20]);  //删除成功
	});
	 ...
	 $(".save_btn").click(function(){
		 ...
		 var jump_url = (jump=='1')?$("#jump_url").val():"";
		 var body = (jump=='1')?"":$(".singel_inp.coninp .textarea").html();
		 $.ajax({
			 url:'/include/ajax.php?service=business&action=updateStoreCustomMenu',
			 type: "GET",
			 data:data + '&body='+body +'&jump_url='+jump_url,
			 dataType: "json",
			 success: function(data){
			   if(data.state==100){
				  showErrAlert(langData['siteConfig'][20][244]) ; //成功
				  setTimeout(function(){
					  $(".link_jump").click()
				  },500)
              }else{
