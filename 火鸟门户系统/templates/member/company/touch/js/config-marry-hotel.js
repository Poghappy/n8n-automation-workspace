...
$(function () {
    ...
    function po_Last(obj) {
        ...
        if (window.getSelection) {//ie11 10 9 ff safari
            var max_Len=obj.value.leFngth;//text字符数
   ...
   // 酒店类别筛选
    function getType(){
        $.ajax({
            type: "POST",
            url:  "/include/ajax.php?service=marry&action=type&value=1",
            dataType: "jsonp",
    ...
    $('.category-box').delegate('li','click',function(){
      if($(this).hasClass('active')){
        ...
        if($('.category-box li.active').size() < 5){
            $(this).toggleClass('active');
        }else{
                  showErr(langData['marry'][7][34]);//最多可选五个
    ...
    $('#btn-keep').click(function (e) {
        ...
        if(!hotelname){
            ...
            r = false;
            showErr(langData['marry'][7][37]); //请选择酒店类型！
            return;
        }else if($('.category-list li.active').length == 0){
            r = false;
            showErr(langData['marry'][7][38]);//请选择酒店特色！
            return;
        ...
        var tags = [];
        $('.category-list').find('li.active').each(function(){
