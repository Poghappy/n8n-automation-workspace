...
//                                $money      = sprintf('%.2f',($usermoney-$totalPrice));
    ...
    public function orderList(){
        ...
        //待抢单
        $state10 = $dsql->dsqlOper($grabarchives." AND `orderstate` != 0", "totalCount");
        ...
        //已退款
        $state9 = $dsql->dsqlOper($archives." AND (o.`orderstate` = 8  or o.`orderstate` = 9 or `ret-state` = 1)", "totalCount");

        //结单
        $state11 = $dsql->dsqlOper($archives." AND `orderstate` = 11", "totalCount");
        $pageinfo = array(
            "page" => $page,
            "pageSize" => $pageSize,
            "totalPage" => $totalPage,
        ...
        if($state==10){
            $results = $dsql->dsqlOper($grabarchives.$where, "results");
        }else{
