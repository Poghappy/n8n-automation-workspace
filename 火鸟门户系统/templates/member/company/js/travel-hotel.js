	...
	objId.delegate(".del", "click", function(){
		...
		if(id){
			$.dialog.confirm(langData['siteConfig'][20][211], function(){
				...
				$.ajax({
					url: "/include/ajax.php?service=travel&action=operHotel&id="+id+"&oper=del",
					type: "GET",
					dataType: "jsonp",
...
function getList(is){
	...
	$.ajax({
		url: "/include/ajax.php?service=travel&action=hotelList&u=1&orderby=2&state="+state+"&page="+atpage+"&pageSize="+pageSize,
		type: "GET",
		dataType: "jsonp",
		success: function (data) {
			if(data && data.state != 200){
					...
					if(list.length > 0){
						...
						for(var i = 0; i < list.length; i++){
							var item        = [],
									...
									address     = list[i].addrname[list[i].addrname.length-1],
									style       = list[i].style,
									price       = list[i].price,
							...
							html.push('<p>'+typename+'&nbsp;&nbsp;·&nbsp;&nbsp;'+address+'&nbsp;&nbsp;·&nbsp;&nbsp;'+langData['siteConfig'][55][60]+'：'+pubdate+arcrank+'</p>');//发布时间
							html.push('<h5><a href="'+url+'" target="_blank" title="'+title+'">'+title+'</a></h5>');

							html.push('<p>'+langData['siteConfig'][19][394]+'：'+click+'&nbsp;&nbsp;·&nbsp;&nbsp;'+langData['siteConfig'][55][61]+'：'+ sale+'</p>');//浏览--预定
							html.push('</div>');
							html.push('</div>');
