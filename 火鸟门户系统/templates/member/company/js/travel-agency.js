	...
	objId.delegate(".del", "click", function(){
		...
		if(id){
			$.dialog.confirm(langData['siteConfig'][20][211], function(){
				...
				$.ajax({
					url: "/include/ajax.php?service=travel&action=operAgency&oper=del&id="+id,
					type: "GET",
					dataType: "jsonp",
...
function getList(is){
	...
	$.ajax({
		url: "/include/ajax.php?service=travel&action=agencyList&u=1&orderby=2&state="+state+"&page="+atpage+"&pageSize="+pageSize,
		type: "GET",
		dataType: "jsonp",
		success: function (data) {
			if(data && data.state != 200){
                  ...
                  totalCount = pageInfo.totalCount;
					...
					if(list.length > 0){
						...
						for(var i = 0; i < list.length; i++){
							...
							html.push('<p>'+typename+'&nbsp;&nbsp;·&nbsp;&nbsp;'+langData['siteConfig'][55][60]+'：'+pubdate+arcrank+'</p>');//发布时间
