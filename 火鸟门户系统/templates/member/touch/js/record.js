					...
					if(list.length > 0){
						...
						for(var i = 0; i < list.length; i++){
							...
							if(flag ==1){//月份已存在
								...
								html2.push('<h3>'+info+'</h3><strong class="number'+(type == 1 ? " add" : " less")+'">'+(type == 1 ? "+" : "-")+Number(amount).toFixed(2)+(judgeTime(time) ? '<p>'+langData['siteConfig'][19][363]+':'+list[i].balance+'</p>' : '')+'</strong>');  //交易后余额
								html2.push('</div>');
								...
								html.push('<h3>'+info+'</h3><strong class="number'+(type == 1 ? " add" : " less")+'">'+(type == 1 ? "+" : "-")+Number(amount).toFixed(2)+(judgeTime(time) ? '<p>'+langData['siteConfig'][19][363]+':'+list[i].balance+'</p>' : '')+'</strong>');
								html.push('</div>');
