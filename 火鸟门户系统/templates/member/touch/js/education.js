var pageVue = new Vue({
    ...
    methods: {
        ...
        backFn() {
            if (history.length == 1) {
                location.href = `${masterDomain}/education.html`;
            } else {
                history.go(-1);
            }
        },
        ...
        async detailData() { //详情信息
            ...
            if (result.state == 100) {
                ...
                if (info.id == 0) { //店铺信息未填写
                    alert('请先提交公司资料！')
                    location.href = `${memberUrl}/config-education.html`;
                } else {
                    this.detail = info;
                }
            } else {
        ...
        async courseData() { //课程信息
            ...
            let result = await this.ajax(data);
            if (result.state == 100) {
                this.courseInfo = result.info.pageInfo;
            }
        },
        async applyData() { //报名管理
            let data = {
                service: 'education',
                action: 'orderList',
                store: 1,
            ...
            let result = await this.ajax(data);
            if (result.state == 100) {
                this.applInfo = result.info.pageInfo.state1;
            }
        }
    }
})