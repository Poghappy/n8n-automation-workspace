$(function(){
  ...
  $('body').off('click').delegate('.nofollow', 'click', function(){
    ...
    if (x.hasClass('follow')) {
      ...
      follow(x, function(){
        x.addClass('follow').text(langData['siteConfig'][19][845]);  //互相关注
      });
    }
  });
          ...
          if(data.state == 101){
            ...
            if(list.length > 0){
              for(var i = 0; i < list.length; i++){
                var isfollow = list[i].isfollow, state = list[i].state, luid = list[i].uid,
                    url = state == 1 ? 'javascript:;' : masterDomain + '/user/' + luid;
                ...
                if(!state){
                    ...
                    if (!state) {
                      ...
                      html.push('<p class="nickname"><a href="'+url+'">'+list[i].nickname+'</a></p><p class="gray"></p><p class="fans_num gray">'+langData['siteConfig'][19][847]+'：'+list[i].fansall+'</p>');
                      html.push('</div>');
                      if(luid != loginId){
                        if (isfollow) {
                        html.push('<div class="fn-right"><a href="javascript:;" class="nofollow follow " data-id="'+luid+'">'+langData['siteConfig'][19][845]+'</a></div>');  //已关注
                        }else {
                        html.push('<div class="fn-right"><a href="javascript:;" class="nofollow" data-id="'+luid+'">'+langData['siteConfig'][19][846]+'</a></div>'); //关注
                        }
                      }
                    }else {
                      html.push('<div class="fn-left txtbox">'+langData['siteConfig'][19][850]+'</div>') ;  //已注销
                    }
                    html.push('</div>');
                }
