...
$(function(){
                    ...
                    success: function (data) {
                        if(data && data.state == 100){
                            ...
                            for (var i = 0, area, lower; i < list.length; i++) {
                                ...
                                areaList.push('<li data-id="'+area.id+'" data-lng="'+area.longitude+'" data-lat="'+area.latitude+'" data-lower="'+lower+'"'+(!lower ? 'class="n"' : '')+'>'+area.typename+'</li>');
							...
							if(list.length > 20 && id == 0){
							  ...
							  for (var i = 0; i < szmArr.length; i++) {
							    ...
							    for (var j = 0; j < cityArr[szmArr[i]].length; j++) {
							      ...
							      areaList.push('<li data-id="'+cityArr[szmArr[i]][j].id+'" data-lng="'+cityArr[szmArr[i]][j].longitude+'" data-lat="'+cityArr[szmArr[i]][j].latitude+'" data-lower="'+lower+'"'+(cla != "" ? 'class="'+cla+'"' : '')+'>'+cityArr[szmArr[i]][j].typename+'</li>');
							
							      if(cityArr[szmArr[i]][j].hot == 1){
							        hotList.push('<li data-id="'+cityArr[szmArr[i]][j].id+'" data-lng="'+cityArr[szmArr[i]][j].longitude+'" data-lat="'+cityArr[szmArr[i]][j].latitude+'" data-lower="'+lower+'">'+cityArr[szmArr[i]][j].typename+'</li>');
							      }
							    }
							  }
                    ...
                    success: function (data) {
                        if(data && data.state == 100){
                            ...
                            for (var i = 0, area, cla, lower; i < list.length; i++) {
                                ...
                                areaList.push('<li data-id="'+area.id+'" data-lng="'+area.longitude+'" data-lat="'+area.latitude+'" data-lower="'+lower+'"'+(cla != "" ? 'class="'+cla+'"' : '')+'>'+area.typename+'</li>');
                            }
    ...
    gzSelAddrList.delegate("li", "click", function(){
        var t = $(this), id = t.attr("data-id"), addr = t.text(), lng = t.attr('data-lng'), lat = t.attr('data-lat'), lower = t.attr("data-lower"), par = t.closest("ul"), index = par.index();
        $('.' + gzSelAddrSzm).hide();
        if(id && addr){

            t.addClass("gz-curr").siblings("li").removeClass("gz-curr");
            gzSelAddrNav.find("li:eq("+index+")").attr("data-id", id).attr("data-lng", lng).attr("data-lat", lat).html("<span>"+addr+"</span>");
            ...
            //没有下级
                ...
                var lastAddrLi = gzSelAddrNav.find("li:last");
    ...
    gzSelAddrNav.delegate("li", "touchend", function(){
        ...
        if(gzSelAddrList.find('ul:eq('+index+')').find('.gz-curr').length){
    ...
    gzSelAddr.delegate("." + gzSelAddrSzm, "touchstart", function (e) {
        ...
        $(this).find("li").each(function (i, item) {
            ...
            if (x > left && x < (left + width) && y > top && y < (top + lheight)) {
    ...
    //自动定位
    if(typeof HN_Location == 'object' && gzAddrSeladdr.attr('data-ids') == '' && gzAddrSeladdr.attr('data-action') != 'type' && (typeof(autoLocation) == 'undefined' ||  (typeof(autoLocation) != 'undefined' && autoLocation))){
