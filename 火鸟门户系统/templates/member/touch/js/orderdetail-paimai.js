$(function(){
	// if(orState ==0){
 //      var nopayTimer = setInterval(function(){
	// 		getnopayDetail()
	// 	},2000)
 //    }
  	function getnopayDetail(){
		$.ajax({
			type: "GET",
			url: "/include/ajax.php?service=paimai&action=orderDetail&id="+id,
			dataType: "jsonp",
			success: function(data) {
              if(data.state == 100){
                if(data.info.orderstate != '0' ){
                  window.clearInterval(nopayTimer);
                  location.reload();
                }
  ...
  //提交申请退款
	$("#submit").bind("click", function(){
		...
		$.ajax({
			url: masterDomain+"/include/ajax.php?service=paimai&action=refund",
			data: data,
			type: "POST",
			dataType: "jsonp",
			success: function (data) {
	...
	var isBytemini = device.toLowerCase().includes("toutiaomicroapp");
  	...
  	if(isBytemini){
  		$('.shop-pro .imgbox a,.shop-pro .txtbox').click(function(){
  			var thref = $(this).attr('href');
  			var ttid = (thref.split('-')[1]).split('.')[0];
