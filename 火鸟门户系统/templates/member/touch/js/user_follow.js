...
// 关注按钮
  ...
  function getList(is){
    isload = true;
  	...
  	$.ajax({
  		url: masterDomain+"/include/ajax.php?service=member&action=follow&type=follow&fortype=&uid="+uid+"&page="+atpage+"&pageSize="+pageSize,
  		type: "GET",
  		dataType: "jsonp",
  		success: function (data) {
            ...
            var msg = pageInfo.totalCount == 0 ? langData['siteConfig'][20][126] : langData['siteConfig'][20][185];
                ...
                if (!state) {
								...
								html.push('<p class="nickname"><a href="'+url+'">'+list[i].nickname+'</a></p><p class="gray">我是一个动态或者其他</p><p class="fans_num gray">粉丝：'+list[i].fansall+'</p>');
    							html.push('</div>');
