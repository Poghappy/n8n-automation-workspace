$(function(){
	...
	var map, filterData, infoWindow, isload, markersArr = [], markerHx, list = $(".yl_list"), init = {
  ...
      });
      ...
      isload = true;
		...
		,getLoupanData: function(type){
			...
			$.ajax({
				"url":"/include/ajax.php?service=pension&action=storeList&pageSize=9999999",
				"data": data,
				"dataType": "JSONP",
				"async": false,
				"success": function(data){
	...
	var bubbleMapSize = {
		...
		,bubbleTemplate = {
			...
			//区域
			1 : '<div class="bubble bubble-1" data-longitude="${longitude}" data-latitude="${latitude}" data-id="${id}"><p class="name" title="${district_name}区">${district_name}区</p><p><span class="count">${count}</span>个养老</p></div>',
		...
		,listTemplate = {
			...
			//养老列表1 最开始左侧边栏
			building: '<dl class="fn-clear"data-id="${id}"data-lng="${longitude}"data-lat="${latitude}"title="${resblock_name}"><dt><a href="${url}"target="_blank"><img src="${cover_pic}"/></a></dt><dd><h2><a href="${url}"target="_blank">${resblock_name}</a></h2><p>${loupan_addr}</p><p>${store_tel}</p><p class="price">￥${priceTpl}</p></dd></dl>',
