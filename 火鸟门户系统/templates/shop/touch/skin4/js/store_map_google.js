$(function(){
  console.log(siteCity)
   ...
   function  mapDistance(lat_a,lng_a,lat_b,lng_b){
    ...
    //获取当前位置
    var oldlng,oldlat,markersArr = [],infoWindow ;
    var isload = false;
  ...
  // var panWidth = $('#map').width()/2
  	var init = {
        ...
        if(site_map == 'google'){
          ...
          geocoder.geocode({'address': g_conf.cityName}, function(results, status) {
            ...
            if(status == google.maps.GeocoderStatus.OK) {
              ...
              // var latLng = (oldlat && oldllng) ? (oldlat +','+oldlng) : (g_conf.lat +','+g_conf.lng)
              ...
              var icon = {
                url: templets+"/images/map/circle.png?v=1",
                ...
                if(data.state == 100){
                  ...
                  var addrData = data.info;
                   addrData.unshift({
                    id:'0',
                    typename: '不限',
                    latitude:  oldlat?oldlat:g_conf.lat,
                    longitude: oldlng?oldlng:g_conf.lng,
                    lowerarr:''
                  })
        ...
        $('.quyuAlert .chooseLeft').delegate('a','click',function(){
            ...
            if(id == 0){//不限 没有二级
              ...
              if(onelng&&onelat){
                var pt = new google.maps.LatLng(onelat, onelng);
                map.setCenter(pt);
                map.setZoom(13);
              }else{
                var pt = new google.maps.LatLng(g_conf.lat, g_conf.lng);
                map.setCenter(pt);
                map.setZoom(13);
              }
        ...
        data.push(moreid);
              ...
              if(keywords !=""){
                if(list.length > 0){
                  ...
                  var pt = new google.maps.LatLng(selng, selat);
                  map.setCenter(pt);
                  map.setZoom(13);
      ...
      ,getSearch_posi:function(){
          ...
          success: function(data){
            ...
            if(data.state == 100){
              ...
              if(list.length > 0){
                ...
                //没搜到结果 则回到定位的位置
                var pt = new google.maps.LatLng(oldlng, oldlat);
                  map.setCenter(pt);
                  map.setZoom(13);
              }
        ...
        //   var pt = new google.maps.LatLng(oldlng, oldlat);
        //   if(Boolean(oldlng&&oldlat)){//定位成功 绘制当前位置
        //           map.setCenter(pt);
        //           map.setZoom(13);
        //   }
          // else{//定位失败时就画苏州市;
          //   var pt = new google.maps.LatLng(g_conf.lng, g_conf.lat);
          //   map.setZoom(13);
          //         map.setCenter(pt);
          // }
  			}
        ...
        if(site_map != 'google'){
          ...
          var icon = {
            url: templets+"/images/map/shop.png",
          ...
          google.maps.event.addListener(markers, 'click', function() { 
            console.log(data)
            var infowincontent = '<div style="font-weight: 700; line-height: 2.5em; font-size: 16px;"><span style="display:inline-block; max-width:3rem; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; vertical-align: middle">'+data[0].store_name+'&nbsp;&nbsp;</span><a style="font-size: 12px; color: #de1e30; font-weight: 500;" href="'+data[0].store_url+'" target="_blank">详细>></a></div>';
        ...
        //     labelContent: init.replaceTpl(temp, o),
    ...
    $('.zoom-local').click(function(){
      ...
      console.log(oldlng, oldlat);
        if(oldlng&&oldlat){//定位成功 绘制当前位置
          ...
          var pt = new google.maps.LatLng(oldlat, oldlng);
          map.setCenter(pt);
          map.setZoom(13);
        }else{//定位失败时就画苏州市
