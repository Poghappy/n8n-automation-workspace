...
$(function(){
    ...
    gzAddrSeladdr.bind("click", function(){
        ...
        //2021-8-12特改
        var t = $(this), ids = t.attr("data-ids"), id = t.attr("data-id"), addrname = t.attr("data-addrname");
    ...
    gzSelAddrList.delegate("li", "click", function(){
        ...
        if(id && addr){
            ...
            if(just == "1"){
                ...
                //gzAddrSeladdrActive.removeClass("gz-no-sel").addClass('hasChose').attr("data-ids", ids.join(" ")).attr("data-id", id).find("dd p").html(addrname.join(" "));
                //2021-8-12 只取最后一个地址
                if(gzAddrSeladdrActive.hasClass('addrcart')){
                  gzAddrSeladdrActive.removeClass("gz-no-sel").addClass('hasChose').attr("data-ids", ids.join(" ")).attr("data-id", id).find("dd p").html(addr);
                }else{
                  gzAddrSeladdrActive.removeClass("gz-no-sel").addClass('hasChose').attr("data-ids", ids.join(" ")).attr("data-id", id).find("dd p").html(addrname.join(" "));
                }
                
                gzAddrSeladdrActive.attr("data-addrname", addrname.join(" "))
                gzAddrInit.hideNewAddrMask();

                gzAddrSeladdrActive.find('input').val(id);
                gzAddrSeladdrActive.find('input').trigger('change');
            ...
            gzAddrSeladdrActive.removeClass("gz-no-sel").addClass('hasChose').attr("data-ids", '').attr("data-id", 0).find("dd p").html('不限');
            gzAddrInit.hideNewAddrMask();
    ...
    //2021-05-08 新增 确定区域
    gzSelAddr.delegate(".sureAddrid", "click", function(){
      var addrname = [], ids = [];
      var par = gzSelAddrList.find('ul:last-child');
      var id;
      gzSelAddrNav.find("li").each(function(){
      ...
      if(ids && id){
        if(gzAddrSeladdrActive.hasClass('addrcart')){
          gzAddrSeladdrActive.removeClass("gz-no-sel").addClass('hasChose').attr("data-ids", ids.join(" ")).attr("data-id", id).find("dd p").html(addrname[addrname.length-1]);
        }else{
          gzAddrSeladdrActive.removeClass("gz-no-sel").addClass('hasChose').attr("data-ids", ids.join(" ")).attr("data-id", id).find("dd p").html(addrname.join(" "));
        }
