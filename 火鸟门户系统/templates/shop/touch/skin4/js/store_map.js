  ...
  // var panWidth = $('#map').width()/2
  	var init = {
        ...
        if(site_map == 'baidu'){
          ...
          map = new BMap.Map(g_conf.mapWrapper,{enableMapClick: false});
          ...
          var newStyle = [{//自定义的地图样式 去掉了 地图自带的商家标注
          ...
          map.setMapStyle({ styleJson:newStyle});//自定义地图样式
      ...
      ,storeClick: function(e){
        ...
        location.href=url;
  		...
  		,cleanBubble: function(){
  			map.clearOverlays();
        //创建自定义中心点图标
        var localPoint = new BMap.Point(oldlng, oldlat)
    ...
    $('.zoom-local').click(function(){
      ...
      console.log(oldlng, oldlat);
        ...
        if(oldlng&&oldlat){//定位成功 绘制当前位置
          ...
          var mPoint = new BMap.Point(g_conf.lng, g_conf.lat)
