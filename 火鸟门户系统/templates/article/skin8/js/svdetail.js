...
//关注
    ...
    function getCommentList(list){
        ...
        for(var i = 0; i < list.length; i++){
            ...
            html.push('      <span>'+list[i]['ftime']+(list[i].iphome ? ' · ' + list[i].iphome : '')+'</span>');
            html.push('    </div>');
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if(data && data.state == 100){
                    ...
                    list.push('      <span>'+info['ftime']+(list[i].iphome ? ' · ' + list[i].iphome : '')+'</span>');
                    list.push('    </div>');
    ...
    $(".zan").bind("click", function(){
        ...
        $.post("/include/ajax.php?service=member&action=getZan&module=article&temp=detail&title="+zanTitle+"&id="+aid+"&uid="+admin, function(data){
            if(data.state == 100){
    ...
    $(".report").bind("click", function(){
        ...
        complain = $.dialog({
            fixed: false,
            title: "信息举报",
            content: 'url:'+domainUrl+'/complain-article-detail-'+aid+'.html',
            width: 460,
            height: 280
        });
    });
