$(function(){
    ...
    $('.dfs-left ul').delegate('li','click',function(){
        ...
        let data = ['min_salary', 'max_salary','salary_type','scroll'];
        let value;
        if ($(this).closest('.df-salary')[0].className.indexOf('time')==-1) { //月薪
            value = [min ? min : '', max ? max : '',1,''];
        } else { //时薪
            value = [min ? min : '', max ? max : '',2,''];
        }  
    ...
    $('.fsf-items').delegate('.item','click',function(){
        ...
        switch(name){
            case 'item':{ //薪资(月/时薪)
                let min = $(this).attr('data-min');
                let max = $(this).attr('data-max');
                if ($(this).parent()[0].className == 'msalary') { //月薪
                    replaceFn(['min_salary', 'max_salary','salary_type','scroll'], [min, max,1,1]);
                } else { //时薪
                    replaceFn(['min_salary', 'max_salary','salary_type','scroll'], [min, max,2,1]);
                }
                break;
            };
            case 'salary':{ //薪资(不限)
            ...
            case 'experience':{ //经验
                let id=$(this).attr('data-id');
                replaceFn(['experience','scroll'],[id,1]);
                break;
            };
            case 'industry':{ //行业
                let id=$(this).attr('data-id');
    ...
    { //期望薪资文本
        ...
        if(target.text()=='不限'&&target.length!=0){
            ...
            target.closest('.salary').find('span').eq(0).text(target.text());
        }else{ //自定义搜素的文本
            let min;
            let max;
    ...
    { //工作经验文本
        let target = $('.fs-filter .experience .fsf-items  .active');
        if(target.text()=='不限'){
            target.closest('.experience').find('span').text(target.siblings().eq(0).text())
        }else{
            target.closest('.experience').find('span').text(target.text());
        }
    }
    // 自定义薪资筛选
    ...
    ajax(data).then(res=>{
        ...
        $('.fs-filter .nature').find('.fsf-items').html(str1);
        // 吸顶选中文本
    ...
    ajax(data).then(res=>{
        ...
        $('.fs-filter .scale').find('.fsf-items').html(str1);
        // 吸顶选中文本
    ...
    $('.df-experience ul').delegate('li','click',function(){
        let id=$(this).attr('data-id');
        replaceFn(['experience','scroll'],[id,'']);
    });
