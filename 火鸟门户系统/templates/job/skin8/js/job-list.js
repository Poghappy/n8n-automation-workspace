$(function(){
    ...
    ajax(data).then(res=>{
        ...
        $('.fs-filter li').eq(5).find('.fsf-items').html(str1);
        // 吸顶选中文本
    ...
    $('.clt-all .btn').click(function(){
        ...
        if(!userid){ //未登录
            location.href=`${masterDomain}/login.html`
        }else if(rdetail.certifyState==0&&postCertificate){ //已登录，没有认证
            $('.sendpop').show();
            $('.s-certificate').show();
