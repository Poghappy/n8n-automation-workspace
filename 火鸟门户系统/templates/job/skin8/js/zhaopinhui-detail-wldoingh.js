...
$(function () {
    ...
    scrollTab = $(".js-header").offset().top
    ...
    $('.js-header form').submit(function () {
        ...
        if (index == 0) { //职位搜索
            ...
            page1 = 1
            // $.ajax({
            //     url: '/include/ajax.php?action=fairsJoinCJ_company&service=job',
            //     data: {
            //         fid: fid,
    ...
    // 打开报名窗口
    let company='';
    $('.bottom-signup .company,.jch-right span').click(function () {
        if (!userid) { //未登录
    ...
    $('.js-header li').click(function () {
        ...
        if (index == 0) { //招聘职位
            $('.js-position').show();
            $('.js-company').hide();
            atpage = page0,totalCount = totalCount0,pageSize = pageSize0
        } else { //参会企业
            $('.js-position').hide();
            $('.js-company').show();
            if(totalCount1 == 0){
            ...
            atpage = page1,totalCount = totalCount1,pageSize = pageSize1
