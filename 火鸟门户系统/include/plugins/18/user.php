...
class User{
    ...
    public function getList(){
        ...
        $prePage = $page-1;
        $action_url = $page_url."?action=getList";
        // 上一页
        $prePageUrl = "";
        if($prePage>=1){
            $prePageUrl = "$action_url&page=$prePage";
        }
        // 下一页和尾页
        $endPage = $totalPage;
        $nextPage = $page+1;
        $nextPageUrl = "";
        if($page<$endPage){
            $nextPageUrl = "$action_url&page=$nextPage";
        }
        // 取得本页数据
        $record_Start = ($page-1)*$pageSize;
        $array_slice = array_slice($data,$record_Start,$pageSize);
        ...
        $huoniaoTag->assign("page_url",$page_url);
        $huoniaoTag->assign("action_url",$action_url);
        $huoniaoTag->assign("prePageUrl",$prePageUrl);
        $huoniaoTag->assign("nextPageUrl",$nextPageUrl);
        $huoniaoTag->assign("page",$page);
        $huoniaoTag->assign("allCount",$allCount);
        $huoniaoTag->assign("totalPage",$totalPage);
        $huoniaoTag->assign("endPage",$endPage);
        $huoniaoTag->assign("data",$array_slice);
    ...
    public function cat(){
        ...
        // 2. 请求信息
        $num = $_GET['num'];
        $action_url = $page_url."?action=cat";
        // 设置抓取数量（默认使用GET请求的数量，如果不存在则使用配置文件的、如果没有配置文件，则默认50条）
        if(!isset($num)){
            if(!empty($config) && is_array($config)){
                $num = $config['default_num'];
                $default_num = $config['default_num'];
            }else{
                $num = 50;
                $default_num = 50;
            }
        }
        // 设置抓取数量（默认100条，如果大于100，也只能100条）
        // if($num>100){
        //     $num = 100;
        // }
        // 2.开始抓取
        $time_start = time();  // 开始时间
        for($j=1; $j<=$num; $j++){ // 循环抓取
            // 生成8-11位随机数
            $rand_str = "";
            $wei = rand(8,11);
            for($i=0;$i<$wei;$i++){
            ...
            $qq = rand(100000,3000000000);
            $qq_nick = $this->nickname(2);
            if($qq_nick){  // 对用户名筛选，如果通过则合法
                $img = "https://q1.qlogo.cn/g?b=qq&nk=".$qq."&s=100&t=".time();
                $data[] = [
                    'qq' => $qq,
                    'name' => $qq_nick,
                    'img' => $img
        ...
        foreach ($tongji as $k=>$v){
            $totalSum += $tongji[$k]['num'];
            $totalTime += $tongji[$k]['time'];
        }
        // $avgTime = number_format($totalSum/$totalTime,2);  // 平均每条/耗时
        $avgTime = 0;  //不需要远程抓取了，这里时间为0
        // 由于数据会删除，在进行抓取操作时，对数据进行重新排序
        $new_json_arr = [];
        $k = 0;
        for($i=0;$i<count($data);$i++){
            if($data[$i]){
                $new_json_arr[$k]  = $data[$i];
                $k++;
            }
        }
        // 对重新排序后的数据进行保存
        global $plugin_18_data;
        putSession($plugin_18_data, $new_json_arr);  // 注意、先存入session
        $text='<?php return '.var_export($new_json_arr,true).';';
        file_put_contents($data_file,$text);

        // 取得本次数据内容，用于渲染到页面上
        $array_slice = array_slice($data,-$num,$num);


        if($num > 0){
            build_success("成功生成" . $num . "个账号");
        }else{
            build_success("生成失败，请稍等重试！");
        }
        die;
        ...
        global $tpl;
        $huoniaoTag->template_dir = $tpl; //设置后台模板目录
        $templates = "user_cat.html";

        $huoniaoTag->assign("page_url",$page_url);
        $huoniaoTag->assign("action_url",$action_url);
        $huoniaoTag->assign("num",$num);
        $huoniaoTag->assign("default_num",$default_num);
        $huoniaoTag->assign("avgTime",$avgTime);
        $huoniaoTag->assign("current",$current);
        $huoniaoTag->assign("totalSum",$totalSum);
        $huoniaoTag->assign("totalCat",count($tongji));
        $huoniaoTag->assign("data",$array_slice);
    ...
    public function setData(){
        ...
        // 保存上传的文件
        $temp_uri = "./data/json_cache.php";
        move_uploaded_file($_FILES["file"]["tmp_name"],$temp_uri);

        $str = "";
        if(file_exists($temp_uri)){
            $str = file_get_contents($temp_uri);
        }
        $arr = json_decode($str,true);  // 把json解析的数据追加到data中
        foreach ($arr as $k=>$v){
            $data[] = $arr[$k];
        }
        $data2 = assoc_unique($data,"qq"); // 根据qq号去重后的结果
        global $plugin_18_data;
        putSession($plugin_18_data, $data2);  // 注意、先存入session
        $text='<?php return '.var_export($data2,true).';';
        file_put_contents($data_file,$text);
        build_success("导入成功");
