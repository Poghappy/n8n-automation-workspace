...
class UyghurCharUtilities {
    ...
    public  function getPFRevGb($str){
        $str = $this->getUyPFStr($str);
        ...
        foreach ($array as $k=>$v) {
            if (preg_match('/[\x{4e00}-\x{9fa5}]/u', $array[$k]) || preg_match("/^[a-zA-Z]+$/",$array[$k]) || preg_match("/^[0-9]+$/",$array[$k])) {
                $cn[]=$array[$k];
            }else {
                $uy[]=$array[$k];
            }
            /*$wc = hexdec(json_encode($array[$k]));
            if ($wc < 255 || preg_match('/[\x{4e00}-\x{9fa5}]/u', $array[$k])) {
                $cn[]=$array[$k];
            }else {
                $uy[]= $array[$k];
            }*/
