...
class Smarty_Internal_Data
{
     ...
     */
    public function assign($tpl_var, $value = null, $nocache = false, $conver_html = false)
    {
        // echo $tpl_var . '___' . gettype($value) . '___' . (is_string($value) && is_array(json_decode($value)) ? '是' : '否') . '___' . $value . "\r\n\r\n";
        //向页面显示数据时，清除掉通过addslashes添加的反斜杠，并将&"'<>符号转为HTML实体
        if($value && is_string($value) && !is_array(json_decode($value, true)) && $tpl_var != 'cfg_encryptPubkey' && $tpl_var != 'rsaScript'){
