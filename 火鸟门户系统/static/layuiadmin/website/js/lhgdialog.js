...
//提示文字 图标样式  显示时间 回调函数
function showTip(text, icon, timer, callback) {
    var icont = "";
    var st = icon | 0;
    timer < 1 ? timer = 1 : timer = timer;
    if (st == 0) {
        icont = "fail.png";//错误
    } else if (st == 1) {
        icont = "succ.png";//正确
    ...
    var stmp = "<p style='font-size:16px;line-height:30px;height:30px;font-family:Microsoft Yahei;padding:5px 15px;color:#333;display:inline-block;'>" + text + "</p>";
    
    $.dialog({ content: stmp, id: 'oneID', icon: icont, lock: true, time: timer, fixed: true, max: false,zIndex:21000, min: false, title: false, close: function () {
            var duration = 300,
                api = this,
                opt = api.config,
                wrap = api.DOM.wrap,
