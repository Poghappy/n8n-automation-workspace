...
;(function( $, window, undefined ){
    ...
    _path = (typeof adminPath !== 'undefined' ? adminPath + "../static/" : (typeof adminPath !== 'undefined' ? staticPath : '/static/')) + 'js/ui/',
    ...
    {   //config中新增配置 fn，对象格式  key对应点击对象设置的name 点击之后调用fn中对应的方法
        // 例如 <div name="click"></div> ====> fn:{'click':function(){},}  点击div执行fn中click方法
        config = config || {};
    ...
    lhgdialog.fn = lhgdialog.prototype =
    {
        ...
        _init: function( config )
        {
            ...
            if(config.id){
                DOM.border.addClass(config.id);
            }
            // if(config.content)
            if(content.indexOf('complain') > -1){
                DOM.border.addClass('JubaoPop');
                DOM.wrap.addClass('JubaoPopBox')
            }
            ...
            if(content.indexOf('url') > -1 && content.indexOf('complain') >-1){
                config.width = 670;
                config.height = 430;
            }
        ...
        button: function()
        {
            var that = this, DOM = that.DOM,
                ...
                focusButton = 'btn-primary ui_state_highlight',
        ...
        _addEvent: function()
        {
            ...
            DOM.wrap.bind('click', function(event){
                ...
                {   
                    if($(target).attr('name') ){ //新增自定义时间 非按钮类型
                        let don_name = $(target).attr('name')
                        if(config.fn && config.fn[don_name]){
                            config.fn[don_name]();
                        }
                    }
