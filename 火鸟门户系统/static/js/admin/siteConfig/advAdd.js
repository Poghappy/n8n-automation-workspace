var mue = UE.getEditor('notifyAdv', {"term": "mobile"});
...
$(function () {
	...
	if($("#dopost").val() == "edit"){
		if(classid != ""){
			...
			//弹窗公告
				...
				mue.ready(function() {
					mue.setContent(adbody[0]);
				})
				$("#notifyLink").val(adbody[1]);
			}
		}
	}
	...
	$("#btnSubmit").bind("click", function(event){
		...
		//多图广告
			...
			$('#imglist1').val(imglist1.replace(/,/g, "||").replace(/resize\|\|/g, "resize,").replace(/m_fill\|\|/g, "m_fill,").replace(/w_4096\|\|/g, "w_4096,"));
		...
		//弹窗公告
			...
			mue.sync();
			content = mue.getContent() + '$$' + $("#notifyLink").val();
		}
...
function fileQueuedList_(file) {
	...
	var pli = $('<li class="clearfix" id="'+file.id+'"></li>'),
		...
		lid = $('<div class="li-input"><input style="margin:0 10px 10px 0!important; width:47%; float:left;" class="i-name" placeholder="请输入图片名称" value="" /><input style="margin:0 0 10px 10px!important; width:44%; float:left;" class="i-link" placeholder="请输入图片链接" value="" /><input class="i-desc" placeholder="请输入图片介绍" value="" /></div>');
