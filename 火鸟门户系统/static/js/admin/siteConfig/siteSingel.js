	...
	huoniao.operaJson("siteSingel.php?dopost=getList&action="+action, data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < siteSingelList.length; i++){
				...
				list.push('  <td class="row60 left">【'+siteSingelList[i].id+'】<a href="'+siteSingelList[i].url+'" target="_blank">'+siteSingelList[i].title+'</a></td>');
				list.push('  <td class="row25">'+siteSingelList[i].date+'</td>');
				list.push('  <td class="row12"><a data-id="'+siteSingelList[i].id+'" data-title="'+siteSingelList[i].title+'" href="siteSingel.php?dopost=Edit&id='+siteSingelList[i].id+'&action='+action+'" title="修改" class="edit">修改</a><a href="javascript:;" title="删除" class="del">删除</a></td>');
				list.push('</tr>');
			}
