	...
	huoniao.operaJson("?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < data.length; i++){
				...
				list.push('  <td class="row12 left">'+data[i].time1+'<br />'+data[i].time2+'</td>');
				list.push('  <td class="row12 left">'+data[i].time3+'<br />'+data[i].time4+'</td>');
				list.push('  <td class="row7">'+(data[i].record ? '<a href="'+data[i].record+'" target="_blank" class="link">录音</a>' : '无')+'</td>');
				list.push('</tr>');
			}
