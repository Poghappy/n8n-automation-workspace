$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("privatenumberList.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							$("#selectBtn a:eq(1)").click();
							setTimeout(function() {
    ...
    //更新单条状态
	$("#list tbody").delegate(".audit, .refuse", "click", function(event){
    ...
    $("#Filedata").bind("change", function(){
        if($(this).val() == ""){
			...
			if($.inArray(ext,typeArr) == -1){
				$.dialog.alert("请从华为云隐私保护通话号码管理页面导出数据后，选择保存的csv格式文件进行上传！");
                return;
			}
		}
        mysub();
...
function getList(){
	...
	huoniao.operaJson("privatenumberList.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < data.length; i++){
				...
				list.push('  <td class="row15 left">'+data[i].number+'</td>');
				list.push('  <td class="row15 left">'+data[i].cityName+' - <code>'+data[i].cityCode+'</code></td>');
				list.push('  <td class="row15 left">'+data[i].carrier+'</td>');
				list.push('  <td class="row15 left">'+data[i].bindCount+'</td>');
				list.push('  <td class="row15 left">'+data[i].useCount+'</td>');
				var state = "";
				switch(data[i].state){
					case "1":
						state = "<a href='javascript:;' data-text='锁定' class='audit' title='点击锁定该号码'>正常</a>";
						break;
					case "2":
						state = "<a href='javascript:;' data-text='正常' class='refuse' title='点击解锁该号码'>锁定</a>";
						break;
				}
				list.push('  <td class="row15 left">'+state+'</td>');
				list.push('  <td class="row7"><a href="javascript:;" class="del">删除</a></td>');
				list.push('</tr>');
			}
