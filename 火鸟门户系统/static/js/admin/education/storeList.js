		...
		init = {
			...
			,updateState: function(type, refuse = ''){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("educationstoreList.php?dopost=updateState", "id="+id+"&state="+state+"&refuse=" + encodeURIComponent(refuse), function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	huoniao.operaJson("educationstoreList.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < storeList.length; i++){
				...
				list.push('  <td class="row15 left">'+storeList[i].tel+'<br />'+storeList[i].address+'</td>');
				var flag = "";
