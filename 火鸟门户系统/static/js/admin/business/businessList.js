$(function(){
		...
		init = {
			...
			,updateState: function(type, refuse = ''){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("businessList.php?dopost=updateState", "id="+id+"&arcrank="+arcrank+"&refuse=" + encodeURIComponent(refuse), function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	$("#searchBtn").bind("click", function(){
		...
		$("#sPackage").html($("#package").val());
		$("#sItems").html($("#items").val());
	...
	huoniao.operaJson("businessList.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < businessList.length; i++){
				...
				var img = '<a href="'+businessList[i].url+'" target="_blank"><img onerror="this.src=\'/static/images/shop.png\'" style="width: 50px; height: 50px;" src="'+cfg_attachment+businessList[i].logo+'" class="litpic" /></a>';

				var autharr = businessList[i].auth, auth = [];

				if(businessList[i].isbid){
					auth.push('<span class="label label-success" style="margin-right:3px;" title="置顶">顶</span>');
				}
				...
				list.push('  <td class="row35 left">'+img+'<span><a href="'+businessList[i].url+'" target="_blank">'+businessList[i].title+'</a><br />'+(businessList[i].promotion > 0 ? '<code>保障金：'+businessList[i].promotion+echoCurrency('short')+'</code><br />' : '')+auth.join("")+'</span></td>');
				...
				switch (businessList[i].state) {
					...
					case "1":
						...
						break;
					case "2":
						state = '<span class="refuse statusTips1" data-toggle="tooltip" data-placement="bottom" data-original-title="'+businessList[i].refuse+'"><i class="icon-question-sign" style="margin-top: 3px;"></i>拒绝</span>';
						break;
				}
