$(function(){
		...
		printTypeTree: function(){
			...
			for(var i = 0; i < l; i++){
				(function(){
					...
					typeList.push('  <div class="row20 left"><input type="text" data-type="typename" class="input-medium typename" data-id="'+jsonArray["id"]+'" value="'+jsonArray["typename"]+'" placeholder="次数"></div>');
					typeList.push('  <div class="row40 left"><input type="text" data-type="price" class="input-medium price" data-id="'+jsonArray["id"]+'" value="'+jsonArray["price"]+'" placeholder="价格"></div>');
	...
	$("#addNew").bind("click", function(){
		...
		html.push('    <div class="row20 left"><input data-id="0" type="text" data-type="typename" class="input-medium typename" placeholder="次数"></div>');
		html.push('    <div class="row40 left"><input data-id="0" type="text" data-type="price" class="input-medium price" placeholder="价格"></div>');
	...
	$("#list").delegate("input", "blur", function(){
		...
		if(id != "" && id != 0){
			huoniao.operaJson("?dopost=updateType&id="+id+"&type="+type, "value="+value, function(data){
				if(data.state == 100){
...
function saveOpera(type){
	...
	for(var i = 0; i < first.length; i++){
		(function(){
			var html =arguments[0], count = 0, input = $(html).find(".tr .typename"), id = input.attr("data-id"), val = input.val(), price = $(html).find(".tr .price").val();
			if(val != ""){
				json = json + '{"id": "'+id+'", "name": "'+encodeURIComponent(val)+'", "price": "'+price+'"},';
			}
		})(first[i]);
	}
	...
	huoniao.operaJson("?dopost=typeAjax&pid="+pid+"&cid="+cid+"&did="+did, "data="+json, function(data){
		if(data.state == 100){
			huoniao.showTip("success", data.info, "auto");
			if(type == ""){
				//window.scroll(0, 0);
