$(function(){
		...
		printTypeTree: function(){
			...
			for(var i = 0; i < l; i++){
				(function(){
					...
					if(jsonArray["parentid"] == id){
						typeList.push('  <div class="row3">&nbsp;</div>');
						typeList.push('  <div class="row20 left"><input'+style+' class="typename" data-type="typename" type="text" data-id="'+jsonArray["id"]+'" value="'+jsonArray["typename"]+'"></div>');
						typeList.push('  <div class="row20 left"><input'+style+' class="note" data-type="note" type="text" data-id="'+jsonArray["id"]+'" value="'+jsonArray["note"]+'"></div>');
					}else{
						typeList.push('  <div class="row3"></div>');
						typeList.push('  <div class="row20 left"><span class="plus-icon" style="margin-left:'+cl+'px;"></span><input'+style+' type="text" data-id="'+jsonArray["id"]+'" value="'+jsonArray["typename"]+'"></div>');
					}
	...
	$("#addNew_").bind("click", function(){
		...
		html.push('    <div class="row20 left"><input class="typename" data-id="0" type="text" value=""></div>');
		html.push('    <div class="row20 left"><input class="note" data-id="0" type="text" value=""></div>');
	...
	$("#list").delegate("input", "blur", function(){
		...
		if(id != "" && id != 0){
			huoniao.operaJson("?dopost=updateType&id="+id+"&action=single", "type="+type+"&value="+value, function(data){
				if(data.state == 100){
	...
	$("#batch").bind("click", function(){
		$.dialog({
			fixed: false,
			title: "批量删除",
			content: '<div class="batch-data"><p class="muted">选择要删除的权益，多个按【ctrl+点击】选择</p><select id="category" multiple>'+init.treeTypeList_()+'</select></div>',
			width: 310,
			ok: function(){
				var ids = [];
...
function saveOpera(type){
	...
	for(var i = 0; i < first.length; i++){
		(function(){
			var html =arguments[0], count = 0, jArray = $(html).find(">ul>li"), tr = $(html).find(".typename"), id = tr.attr("data-id"), val = tr.val(), note = $(html).find('.note').val(), icon = $(html).find(".tr .icon").val();

			if(jArray.length > 0 && val != ""){
				json = json + '{"id": "'+id+'", "typename": "'+encodeURIComponent(val)+'", "note": "'+encodeURIComponent(note)+'", "icon": "'+icon+'", "lower": [';
				for(var k = 0; k < jArray.length; k++){
				...
				if(val != ""){
					json = json + '{"id": "'+id+'", "typename": "'+encodeURIComponent(val)+'", "note": "'+encodeURIComponent(note)+'", "icon": "'+icon+'", "lower": null},';
				}
			}
		})(first[i]);
	}
	...
	huoniao.operaJson("?dopost=typeAjax&id=" + id, "data="+json, function(data){
		if(data.state == 100){
			huoniao.showTip("success", data.info, "auto");
			if(type == ""){
				//window.scroll(0, 0);
