		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("awardlegouProList.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("awardlegouProList.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
   ...
   huoniao.choseCity($(".choseCity"),$("#cityList"));  //城市分站选择初始化
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			...
			parent.addPage("productEdit"+id, "awardlegou", title, "awardlegou/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("awardlegouProList.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < productList.length; i++){
				...
				list.push('  <td class="row20 left"><img src="'+cfg_attachment+productList[i].litpic+'" class="litpic" /><a href="'+productList[i].linkurl+'" target="_blank">'+productList[i].title+'</a></td>');
				list.push('  <td class="row12 left">'+productList[i].prizetypename+'</td>');
				...
				list.push('  <td class="row7 left">'+productList[i].price+'/'+productList[i].usepoint+'</td>');
				list.push('  <td class="row7 left"><span>每人每天'+productList[i].onenumber+'次</span><br><small>'+productList[i].numbe+'人</small></td>');
				list.push('  <td class="row5 left"><a href="javascript:;" title="删除" class="tuikuanlogtic" style="color:#2672ec;">'+productList[i].tuikuanlogtic+'<i class="icon-pencil"  style="margin-left: 6px;vertical-align: middle; margin-top: -8px"></i></a></td>');
				var state = "";
...
$("#list").delegate(".tuikuanlogtic", "click", function(){
	var t = $(this), id = t.closest('tr').attr('data-id'), txt = t.text();
	if(t.hasClass('disabled')) return;
	var id = t.closest('tr').attr('data-id');
	info ='<p style="font-weight:bold;font-size:14px;color:#f60;">商品退货邮费</p><p>请正确填写！';
	$.dialog.prompt(info, function(amount){
		huoniao.showTip("loading", "操作中···");
		$.ajax({
			url: 'awardlegouProList.php?dopost=updateLogtic&tuikuanlogtic='+amount+'&id='+id,
			type: 'post',
			dataType: 'json',
