$(function () {
    ...
    var init = {
        ...
        , quickEdit: function (id, rank) {
            if (id == "") {
                ...
                huoniao.operaJson("awardlegouType.php?dopost=getTypeDetail&action=" + action, "id=" + id, function (data) {
                    if (data != null) {
                        var retData = data;
                        data = retData[0];
                        huoniao.hideTip();
                        ...
                        $.dialog({
                            ...
                            ok: function () {
                                ...
                                huoniao.operaJson("awardlegouType.php?dopost=updateType&action=" + action, "id=" + id + "&" + serialize + "&type1=1", function (data) {
                                    if (data.state == 100) {
    ...
    $(".root").delegate(".add-type", "click", function () {
        var parent = $(this).parent().parent(), level = Number($(this).attr("data-level")),
            m = $(this).siblings(".plus-icon").css("margin-left"),
            margin = Number(m == undefined ? -45 : m.replace("px", "")), html = [], addType = '';
    ...
    $("#list").delegate("input", "blur", function () {
        ...
        if (id != "" && id != 0) {
            huoniao.operaJson("awardlegouType.php?dopost=updateType&id=" + id + "&action=" + action, "type=single&typename=" + value, function (data) {
                if (data.state == 100) {
                    huoniao.showTip("success", data.info, "auto");
                } else if (data.state == 101) {
    ...
    $(".root").delegate(".up", "click", function () {
        var t = $(this), parent = t.parent().parent().parent(), index = parent.index(),
            length = parent.siblings("li").length;
        if (index != 0) {
    ...
    $(".root").delegate(".del", "click", function (event) {
        ...
        if (t.parent().parent().next("ul").html() != undefined && t.parent().parent().next("ul").html() != "") {
            ...
            if (type.indexOf("编辑") > -1) {
                huoniao.operaJson("awardlegouType.php?dopost=del&action=" + action, "id=" + id, function (data) {
                    if (data.state == 100) {
                        huoniao.showTip("success", data.info, "auto");
                        setTimeout(function () {
                            location.reload();
            ...
            ok: function () {
                ...
                if (ids.length <= 0) {
                    ...
                    $.dialog.confirm("确定后，此分类下的子级和帖子信息也将同时删除！<br />删除后无法恢复，请谨慎操作！！！", function () {
                        ...
                        huoniao.operaJson("awardlegouType.php?dopost=del&action=" + action, "id=" + ids.join(","), function (data) {
                            if (data.state == 100) {
    ...
    $(".root").delegate(".edit", "click", function () {
        var id = $(this).parent().parent().find("input").attr("data-id"),
            rank = $(this).parent().parent().find(".add-type").attr("data-level");
...
function saveOpera(type) {
    ...
    for (var i = 0; i < first.length; i++) {
        (function () {
            var html = arguments[0], count = 0, jArray = $(html).find(">ul>li"), tr = $(html).find(".tr input"),
                id = tr.attr("data-id"), val = tr.val(), icon = $(html).find(".tr .icon").val();
    ...
    huoniao.operaJson("awardlegouType.php?dopost=typeAjax", "data=" + json, function (data) {
        if (data.state == 100) {
            huoniao.showTip("success", data.info, "auto");
            if (type == "") {
                location.href = href;
