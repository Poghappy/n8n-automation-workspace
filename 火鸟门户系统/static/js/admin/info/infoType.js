$(function(){
			...
			for(var i = 0; i < l; i++){
				(function(){
					...
					if(jsonArray["parentid"] == 0){
						...
						typeList.push('  <div class="row10">&nbsp;</div>');
						typeList.push('  <div class="row10">&nbsp;</div>');
					}else{
						...
						typeList.push('  <div class="row15">'+(jsonArray["icon"] != '' ? '<img src="'+jsonArray["iconturl"]+'" class="img" alt="" style="height:40px;">' : '')+'<a href="javascript:;" class="upfile" title="删除">上传图标</a><input type="file" name="Filedata" value="" class="imglist-hidden Filedata hide" id="Filedata_'+iconIdx+'"><input type="hidden" name="icon" class="icon" value="'+jsonArray["icon"]+'"></div>');
						typeList.push('  <div class="row10"><a href="javascript:;" class="btn btn-small manageItem">管理参数</a></div>');
						typeList.push('  <div class="row10"><a href="javascript:;" class="btn btn-small manageType">管理特色</a></div>');
					}
					typeList.push('  <div class="row12"><a href="javascript:;" class="up">向上</a><a href="javascript:;" class="down">向下</a></div>');
					typeList.push('  <div class="row15 left"><a href="javascript:;" class="link modify" title="编辑">编辑</a>&nbsp;&nbsp;&nbsp;<a href="javascript:;" class="link advanced" title="高级设置">高级设置</a>&nbsp;&nbsp;&nbsp;<a href="javascript:;" class="link delete" title="删除">删除</a></div>');
					typeList.push('</div>');
			...
			if(id == ""){
				...
				huoniao.operaJson("infoType.php?dopost=getTypeDetail&action="+action, "id="+id, function(data){
					if(data != null){
						...
						self.parent.$("#redirect").val(data.redirect);

                        self.parent.$("#style"+data.style).prop("checked",true);
						self.parent.$("#searchall"+data.searchall).prop("checked",true);

                        if(t.closest('.subnav').size() > 0){
                            self.parent.$("#style"+data.style).closest('dl').show();
                            self.parent.$("#searchall"+data.searchall).closest('dl').hide();
                        }else{
                            self.parent.$("#style"+data.style).closest('dl').hide();
                            self.parent.$("#searchall"+data.searchall).closest('dl').show();
                        }
                        self.parent.$('.statusTips').tooltip();

                        if(data.parentid != 0){
                            self.parent.$("#freecall").parent().parent().show();
                            if(data.freecall == '1'){
                                self.parent.$("#freecall").prop("checked",true);
                            }
                        }else{
                            self.parent.$("#freecall").parent().parent().parent().hide();
                        }
	...
	//管理应用分类
	$("body").delegate(".manageType","click", function(){
		var sid = $(this).closest('.tr').data('id')
		$.ajax({
			url: "infoType.php?dopost=manageType&tid="+sid,
			type: "POST",
			dataType: "json",
			success: function(data){
				...
				if(data){
					...
					content += '<a href="javascript:;" id="addNewManageType"><i class="icon-plus"></i>新增特色</a>';
				}
				...
				$.dialog({
					...
					ok: function(){
						...
						for(var i = 0; i < itemList.length; i++){
							var obj = itemList.eq(i), id = obj.attr("data-id"),tid=sid, weight = obj.index(), val = obj.find("input").val();
							data.push('{"id": "'+id+'","tid": "'+tid+'", "weight": "'+weight+'", "val": "'+val+'"}');
						}

						$.ajax({
							url: "infoType.php?dopost=saveManageType",
							data: "data=["+data.join(",")+"]",
							type: "POST",
							dataType: "json",
							success: function(data){
				...
				parent.$('.menu-itemlist').delegate("a", "click", function(){
					...
					if(id != ""){
						$.dialog.confirm("确定要删除吗？<br />此操作将同时删除分类下的应用，请谨慎操作！", function(){
							...
							$.ajax({
								url: "infoType.php?dopost=delManageType",
								data: "id="+id,
								type: "POST"
							});
	...
	$("#addNew_").bind("click", function(){
		...
		html.push('    <div class="row70 left"><input data-id="0" type="text" value=""><a href="javascript:;" class="add-type" data-level="0">添加下级分类</a></div>');
		html.push('    <div class="row12"><a href="javascript:;" class="up">向上</a><a href="javascript:;" class="down">向下</a></div>');
		html.push('    <div class="row15 left"><a href="javascript:;" class="del">删除</a></div>');
		html.push('  </div>');
		html.push('</li>');
	...
	$(".root").delegate(".add-type", "click", function(){
		...
		html.push('    <div class="row70 left"><span class="plus-icon" style="margin-left:'+Number(margin+45)+'px"></span><input data-id="" type="text" value="">'+addType+'</div>');
		html.push('    <div class="row12"><a href="javascript:;" class="up">向上</a><a href="javascript:;" class="down">向下</a></div>');
		html.push('    <div class="row15 left"><a href="javascript:;" class="del">删除</a></div>');
		html.push('  </div>');
		html.push('</li>');
	...
	$(".root").delegate(".modify", "click", function(){
		...
		init.quickEdit(id, rank, $(this));
	});

	//字段
	$(".root").delegate(".manageItem", "click", function(){
	...
	//高级设置
	$(".root").delegate(".advanced", "click", function(){
		var t = $(this), id = t.parent().parent().find("input").attr("data-id"), title = t.parent().parent().find("input").val();
		try {
			parent.addPage("infoTypeAdvanced"+id, "info", title+"高级设置", "info/infoTypeAdvanced.php?tid="+id);
		} catch(e) {
			location.href = "infoTypeAdvanced.php?tid="+id;
		}
	});

    //恢复默认配置
    $('.resetAllAdvanced').bind('click', function(){
        $.dialog.confirm('确认要清除所有分类的高级设置吗？', function(){
            $.ajax({
                type: "POST",
                url: "infoTypeAdvanced.php",
                data: "dopost=resetAllAdvanced",
                dataType: "json",
                success: function(data){
                    if(data.state == 100){
                        $.dialog({
	...
	//删除
	$(".root").delegate(".delete", "click", function(event){
