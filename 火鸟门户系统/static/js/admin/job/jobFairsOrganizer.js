		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("jobFairsOrganizer.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
   ...
   huoniao.choseCity($(".choseCity"),$("#cityid"));  //城市分站选择初始化
	...
	$("#addNew").bind("click", function(event){
		...
		try {
			...
			parent.addPage("jobFairsOrganizerAdd", "job", "新增主办单位", "job/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("jobFairsOrganizer.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < jobFairsCenter.length; i++){
				...
				list.push('  <td class="row15 left">'+jobFairsCenter[i].phone+'</td>');
				list.push('  <td class="row15 left">'+jobFairsCenter[i].email+'</td>');
				list.push('  <td class="row15 left"><a href="javascript:;" data-id="'+jobFairsCenter[i].addrid+'" class="addr">'+jobFairsCenter[i].addr+'</a></td>');
				list.push('  <td class="row12">'+jobFairsCenter[i].pubdate+'</td>');
				list.push('  <td class="row15">');
				list.push('<a data-id="'+jobFairsCenter[i].id+'" data-title="'+jobFairsCenter[i].title+'" href="jobFairsOrganizer.php?dopost=edit&id='+jobFairsCenter[i].id+'" title="修改" class="edit">修改</a>');
