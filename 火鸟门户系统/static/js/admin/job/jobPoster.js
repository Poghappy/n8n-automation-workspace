$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("jobPoster.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
	...
	$("#addNew").bind("click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("jobPosterAdd", "job", "添加新海报", "job/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("editjobPosterAdd"+id, "job", title, "job/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("jobPoster.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < mytagTemp.length; i++){
				...
				list.push('  <td class="row30 left"><a target="_blank" href="'+cfg_attachment+mytagTemp[i].litpic+'"><img src="'+cfg_attachment+mytagTemp[i].litpic+'&type=small" class="litpic" /></a>'+mytagTemp[i].title+'</td>');
				list.push('  <td class="row20 left">'+mytagTemp[i].type+'</td>');
				list.push('  <td class="row25 left">'+mytagTemp[i].pubdate+'</td>');
				list.push('  <td class="row22 left">');
				list.push('<a data-id="'+mytagTemp[i].id+'" data-title="'+mytagTemp[i].title+'" href="jobPoster.php?dopost=edit&id='+mytagTemp[i].id+'" title="修改" class="edit">修改</a>');
...
function importDefaultData(){
	$.dialog.confirm("确定要导入吗？", function(){
		...
		huoniao.operaJson("jobPoster.php?dopost=importDefaultData",{}, function(data){
			if(data.state == 100){
				huoniao.showTip("success", data.info, "auto");
				setTimeout(function() {
					location.reload();
