		...
		init = {
			...
			,updateState: function(type, refuse = ''){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("jobResume.php?dopost=updateState", "id="+id+"&state="+state+"&refuse=" + encodeURIComponent(refuse), function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	huoniao.operaJson("jobResume.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < jobResume.length; i++){
				...
				list.push('  <td class="row20 left"><a class="link" target="_blank" href="'+jobResume[i].url+'">'+jobResume[i].name+'</a>&nbsp;&nbsp;<font style="font-size: 85%; color: #666">['+jobResume[i].alias+']</font>'+(jobResume[i].default ? '&nbsp;<span class="label label-success" style="margin-right:3px;">默认</span>' : '')+'<br /><code>'+jobResume[i].cityName+'</code> <a href="javascript:;" class="userinfo" data-id="'+jobResume[i].userid+'">'+jobResume[i].nickname+'</a></td>');
				var items = jobResume[i].sex+'/'+jobResume[i].age+ '岁/'+jobResume[i].work_jy;
				list.push('  <td class="row12 left">'+items+'</td>');
                
				list.push('  <td class="row15 left">'+jobResume[i].phone+'<br /><small>'+jobResume[i].pubdate+'</small></td>');
				list.push('  <td class="row15 left">'+jobResume[i].min_salary+' - '+jobResume[i].max_salary+'<br/>'+jobResume[i].workState+'</td>');
                var items = '';
                for (let j = 0; j < jobResume[i].job.length; j++) {
					items += (j+1) + "." + jobResume[i].job[j].name+"<br>";
				}
				list.push('  <td class="row15 left"><small>'+items+'</small></td>');
				var state = "";
				switch (jobResume[i].state) {
					case "0":
						state = jobResume[i].del==1 ? '<span class="refuse">已删除</span>' : '<span class="gray">待审核</span>';
						break;
					case "1":
						state = '<span class="audit">已审核</span>';
						break;
					case "2":
                        state = '<span class="refuse statusTips1" data-toggle="tooltip" data-placement="bottom" data-original-title="'+jobResume[i].refuse+'"><i class="icon-question-sign" style="margin-top: 3px;"></i>拒绝</span>';
						break;
				}
				if(jobResume[i].private==1){
					state += '<code style="padding: 1px 4px; margin-left: 3px; display: inline-block; vertical-align: baseline; text-indent: 0; line-height: 16px; background: #fff!important; border: 1px solid #D4E9FF!important; color: #409EFF!important;" title="该简历已设置不公开">私</code>';
				}
				if(jobResume[i].del==1){
					state += '<span class="label label-warning" style="margin-left:3px; background: #FFF0F2!important; text-indent: 0; border: 1px solid #FFDEE3!important; color: #FF4C6A!important;" title="该简历已被用户删除">删</span>';
				}
				state += "</span>";
				list.push('  <td class="row15 state">'+state+(jobResume[i].state == '0' && jobResume[i].del == 1 ? '' : '<span class="more"><s></s></span>')+'</td>');
