...
function getList(){
	...
	huoniao.operaJson("jobTop.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < jobPost.length; i++){
				...
				list.push('  <td class="row20 left"><a href="'+jobPost[i].postUrl+'" class="link" target="_blank">'+jobPost[i].postTitle+'</a></td>');
				list.push('  <td class="row15 left">'+jobPost[i].ordernum+"<br>"+jobPost[i].pubdate+'</td>');
				list.push('  <td class="row13 left">'+jobPost[i].noTopZh+'</td>');
				list.push('  <td class="row17 left">'+jobPost[i].top_start+"<br>"+jobPost[i].top_end+'</td>');
				list.push('  <td class="row15 ">'+jobPost[i].topState+"<br>"+(jobPost[i].amount==0?"<code>免费</code>":'<span style="color: #0BB20C;">'+jobPost[i].amount+'元</span>')+'</td>');
				list.push('</tr>');
			}
