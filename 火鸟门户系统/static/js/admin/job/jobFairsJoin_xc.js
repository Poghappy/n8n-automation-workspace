		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("jobFairsJoin_xc.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
  ...
  $("#keyword").keyup(function (e) {
	...
	$("#addNew").bind("click", function(event){
		...
		try {
			...
			parent.addPage("jobFairsJoin_xcAdd" + $('#fid').val(), "job", "新增参会信息", "job/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("jobFairsJoin_xc.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < jobFairs.length; i++){
				...
				list.push('  <td class="row25 left"><a href="'+jobFairs[i].url+'" target="_blank">'+jobFairs[i].title+'</a></td>');
				list.push('  <td class="row20 left">'+jobFairs[i].company+'</td>');
				...
				list.push(jobFairs[i].seat);
				list.push('</td>');
				list.push('  <td class="row12">');
				// console.log(read);
				if(!read){
					list.push('<a data-id="'+jobFairs[i].id+'" data-title="'+jobFairs[i].title+'" href="jobFairsJoin_xc.php?dopost=edit&id='+jobFairs[i].id+'" title="修改" class="edit">修改</a>');
				}
