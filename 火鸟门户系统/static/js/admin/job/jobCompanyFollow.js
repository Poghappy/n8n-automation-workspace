$(function(){
	...
	//填充操作人列表
	huoniao.buildAdminList($("#cadmin"), adminList, '请选择销售');
		...
		huoniao.operaJson("jobCompany.php?dopost=companyFillList", {"cid":id},function (r){
			...
			if(r.state==100){
				...
				for (let i = 0; i < r.list.length; i++) {
					html += `<div><code>id：${r.list[i].id}</code> ${r.list[i].adminName}<br>时间：${r.list[i].showTime}<br>类型：${r.list[i].typeName}<br>内容：<br><textarea readonly rows="5">${r.list[i].content}</textarea><hr></div>`;
				}
				if(html.length===0){
					html = "暂无相关数据";
				}
...
function getList(){
	...
	huoniao.operaJson("jobCompanyFollow.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < jobCompany.length; i++){
				...
				list.push('<td class="row21 left"><code>'+jobCompany[i].cityName+'</code> <a href="'+jobCompany[i].url+'" target="_blank" class="link">'+jobCompany[i].title+'</a><br/><a class="userinfo" href="javascript:;" data-id='+jobCompany[i].userid+'>'+jobCompany[i].username+'</a></td>');
				list.push('<td class="row15 left">'+(jobCompany[i].people ? jobCompany[i].people : '无')+'<br/>'+(jobCompany[i].contact ? jobCompany[i].contact : '无')+'</td>');
				list.push('<td class="row10 left">'+jobCompany[i].admin+'</td>');
				list.push('<td class="row14 left">'+jobCompany[i].show_time+'</td>');
				list.push('<td class="row10 left">'+jobCompany[i].typeName+'</td>');
				list.push('<td class="row30 left">'+jobCompany[i].content+'</td>');
				list.push('</tr>');
			}
