$(function(){
	...
	//顾问
	$("#list").delegate(".guwen", "click", function(event){
		var id = $(this).attr("data-id"),
			title = $(this).attr("data-title"),
			url = $(this).attr("data-url");
		...
		try {
			event.preventDefault();
			parent.addPage("gwList"+id, "house", title + "顾问", "house/"+url);
		} catch(e) {}
	});
	...
	huoniao.operaJson("loupanList.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < loupanList.length; i++){
				...
				list.push('  <td class="row10 left"><a href="javascript:;" data-id="'+loupanList[i].addrid+'" class="addr">'+loupanList[i].addrname+'</a></td>');
				// var username = [];
				...
				list.push('  <td class="row10 left" title="'+loupanList[i].username+'"><a href="javascript:;" data-id="'+loupanList[i].manageuid+'" class="userinfo">'+(loupanList[i].username ? loupanList[i].username : '无')+'</a></td>');
				...
				list.push('<li><a href="javascript:;" class="guwen" data-id="'+loupanList[i].id+'" data-url="loupanGw.php?loupanid='+loupanList[i].id+'" data-title="'+loupanList[i].title+'">顾问</a></li>');
				list.push('</ul></span>');
				list.push('</td>');
				list.push('</tr>');
			}
