$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("houseCooperation.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("houseCooperation.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	$("#list").delegate(".more", "click", function(event){
		...
		var htmlCreateStateMenu = function(){
			...
			htmlMenu.push('      <li class="smart_menu_li"><a href="javascript:" class="smart_menu_a">未联系</a></li>');
...
function getList(){
	...
	huoniao.operaJson("houseCooperation.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < tuanList.length; i++){
				...
				list.push('  <td class="row30 left">'+tuanList[i].title+'</td>');
				list.push('  <td class="row20 left">'+tuanList[i].usename+'</td>');
				list.push('  <td class="row10 left">'+tuanList[i].tel+'</td>');
				list.push('  <td class="row15">'+tuanList[i].pubdate+'</td>');
				list.push('  <td class="row12 state">'+state+'<span class="more"><s></s></span></td>');
				list.push('  <td class="row10"><a href="javascript:;" data-id='+tuanList[i].id+'" title="删除" class="del">删除</a></td>');
				list.push('</tr>');
			}
