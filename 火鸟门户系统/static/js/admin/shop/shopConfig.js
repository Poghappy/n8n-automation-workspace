...
$(function(){
	$("#delivery_fee_mode").change(function (){
		...
		var val = $(this).val();
		$("#delivery_fee_mode"+val).show().siblings('.delivery_fee_mode').hide()
	});

	var val2 = $("#delivery_fee_mode").val();
	$("#delivery_fee_mode"+val2).show().siblings('.delivery_fee_mode').hide()

	$('.addrangedeliveryfee').click(function (){
		var t = $(this), par = t.closest('.lievf');
		var temp = '<div class="rangedeliveryfee rangedeliveryfeeblock" >\n' +
			'                  配送距离<input type="text" style="width:80px;" class="juliSt"  >公里至<input type="text" style="width:80px;" class="juliEd" >公里，外送费<input type="text" style="width:80px;" class="ranfee" >元，起送价<input type="text" style="width:80px;" class="ranbasic" >元<div class="deleterangedeliveryfee" title="删除自定义显示项">删除</div>\n' +
			'                </div>'
		par.after(temp)
	})

	$('body').delegate('.deleterangedeliveryfee','click',function (){
		var t = $(this),par = t.closest('.rangedeliveryfeeblock');
		par.remove()
	})

	//切换免配送费时骑手补贴
	changeFreeDeliverModer();
	function changeFreeDeliverModer(){
		//隐藏
		$(".free_deliver_mode_item").hide();
		//找出当前选中下标，并显示对应选项
		let index = $("#free_delivery_mode>option:selected").index();
		$(".free_deliver_mode"+index).show();
	}
	$("#free_delivery_mode").change(changeFreeDeliverModer);

	// $("#btnSubmit").click(function (){
	// 	//		range_delivery_fee_value
	// 	var logtitle = $("#logtitle")
	// 	var content = $("#content")
	// 	var basicprice = $("#fixbasic")
	// 	var express_postage = $("#fixfee")
	// 	var preferentialMoney = $("#fixover")
	// 	var openFree = $("input[name='openFree'].ace").val()
	// 	var range_delivery_fee_value = [];
	// 	$('.rangedeliveryfeeblock').each(function (){
	// 		var t =$(this),arr = [];
	// 		t.find('input').each(function (){
	// 			arr.push($(this).val());
	// 		})
	// 		range_delivery_fee_value.push(arr)
	// 	});
	// 	var dataArr = [];
	// 	if(logtitle == ''){
	// 		$.dialog.alert('请输入名称');
	...
	// 		return false;
	// 	}
	// 	dataArr.push('logtitle='+logtitle)
	// 	dataArr.push('content='+content)
	// 	dataArr.push('basicprice='+basicprice)
	// 	dataArr.push('express_postage='+express_postage)
	// 	dataArr.push('preferentialMoney='+preferentialMoney)
	// 	dataArr.push('openFree='+openFree)
	// 	dataArr.push('range_delivery_fee_value='+range_delivery_fee_value)
	// 	'&'+dataArr.join("&")
	//
	// })
	...
	if(showType){
		$(".config-nav button[data-type='"+showType+"']").click()
	}

	$(".huodongOpen label").click(function(){
		var t = $(this),id = t.find('input').val();
		var checked = t.find('input').attr('checked') == 'checked'
		if(checked){
			$(".huodongOpenShow label[data-id='"+id+"']").removeClass('fn-hide')
		}else{
			$(".huodongOpenShow label[data-id='"+id+"']").addClass('fn-hide')
		}
	});
	...
	$(".huodongOpenShow label input[name='shopopen[]']").click(function(){
		var t =$(this)
		if(t.attr('checked') == 'checked'){
			$(".shopopenBox").removeClass('fn-hide')
		}else{
			$(".shopopenBox").addClass('fn-hide')
		}
	})
    ...
    $('.themeList').delegate('input[type=checkbox]', 'click', function(){
        var t = $(this), li = t.closest('li'), sid = parseInt(li.find('.s_id').val());
        if(t.is(':checked')){
            t.parent().siblings('.s_show').val(1);
        }else{
            t.parent().siblings('.s_show').val(0);
        }
        
        //开启到店优惠，需要同步开启销售类型中的到店消费
        if(sid == 1 && t.is(':checked')){
            $('input[name="saleType[]"][value=1]').prop("checked", true);
        }
    });


	//前往活动管理页面
	$('.shopHuodongConfig').bind('click', function(){
		var href = $(this).attr("href");
	  	try {
	  		event.preventDefault();
            parent.addPage("shopHuodongConfigphp", "shop", "活动促销管理", href);
	  	} catch(e) {}
	});
	...
	$("#btnSubmit").bind("click", function(event){
		...
		if(type == "site"){
			...
			mue.sync();
			var body = "&body="+encodeURIComponent(mue.getContent());
			if(mue.getContent() == ""){
			...
			dataArr.push('range_delivery_fee_value='+JSON.stringify(range_delivery_fee_value))
		...
		huoniao.operaJson(formAction+"?action="+action+"&type="+type, post+body+ "&token="+$("#token").val()+'&'+dataArr.join("&"), function(data){
			var state = "success";
			if(data.state != 100){
				state = "error";
			}

            if(type == 'display' && data.state != 100){
