	...
	huoniao.operaJson("tiebaCommon.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < commonList.length; i++){
				...
				if(commonList[i].top == 1){
					list.push('  <td class="row35 title left">标题：<a href="'+commonList[i].storeUrl+'" target="_blank">'+commonList[i].storeTitle+'</a><br />内容：<code style=" display: inline-block; vertical-align: bottom; line-height: 16px;">置顶</code>'+commonList[i].content);
				}else{
