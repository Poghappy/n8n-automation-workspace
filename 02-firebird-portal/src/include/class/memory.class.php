...
class memory {
    ...
    public function llen($key) {
        ...
        $ret = false;
        if($this->enable) {
            $ret = $this->memory->llen($this->_key($key));
        }
        return $ret;
    }

    public function lpush($key, $value, $ttl = 0, $prefix = '') {

        $ret = false;
        if($value === false) $value = '';
        if($this->enable) {
            $this->userprefix = $prefix;
            $ret = $this->memory->lpush($this->_key($key), $value, $ttl);
        }
        return $ret;
    }

    public function rpush($key, $value, $ttl = 0, $prefix = '') {

        $ret = false;
        if($value === false) $value = '';
        if($this->enable) {
            $this->userprefix = $prefix;
            $ret = $this->memory->rpush($this->_key($key), $value, $ttl);
        }
        return $ret;
    }
    ...
    public function lpop($key) {
        ...
        $ret = false;
        if($this->enable) {
            $ret = $this->memory->lpop($this->_key($key));
        }
        return $ret;
    }
