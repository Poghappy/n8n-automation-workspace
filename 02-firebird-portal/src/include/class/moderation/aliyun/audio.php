...
class moderation_audio {
    ...
    public static function createApiInfo($action = ""){
        $params = new Params([
            // 接口名称
            "action" => $action ? $action : "VoiceModeration",
            // 接口版本
            "version" => "2022-03-02",
            // 接口协议
            "protocol" => "HTTPS",
    ...
    public static function main($config = array()){
        ...
        $params = self::createApiInfo($config['action']);
        // body params
        $body = [];
        $body["Service"] = "audio_media_detection";

        //查询
        if($config['action'] == "VoiceModerationResult"){
            $body["ServiceParameters"] = "{\"taskId\":\"".$config['taskId']."\"}";
        }else{
            $seed = create_ordernum();
            $body["ServiceParameters"] = "{\"url\":\"".$config['url']."\", \"callback\": \"".$config['callback']."\", \"seed\": \"".$seed."\"}";
        }
        // runtime options
