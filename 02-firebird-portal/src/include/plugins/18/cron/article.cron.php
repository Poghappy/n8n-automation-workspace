...
class article{
    ...
    public function read($tid){
        ...
        $cur = $maxRand-$minRand;
        $sql = $dsql->SetQuery("update `#@__articlelist_all` set click=click+ceil($minRand+rand()*$cur) where `arcrank`=1 and `pubdate`>$limit_time");
    ...
    public function dianzan($tid){
        ...
        $sql_a = $dsql->SetQuery("select id,admin from `#@__articlelist_all` where `arcrank`=1 and `pubdate`>$limit_time");
        ...
        foreach ($res_a as $k => $v) {
            ...
            if($robots_nin_length>0){  // 机器人数据必须大于0才执行
                ...
                if (!is_numeric($res_up)) {  // 插入public_up失败
                    ...
                    $zan_sql = $dsql->SetQuery("UPDATE `#@__articlelist_all` SET  `zan` = zan + $robots_nin_length WHERE `id` = '$tid'");
