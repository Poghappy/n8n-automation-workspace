...
class tieba{
    ...
    public function read($tid){
        ...
        $cur = $maxRand-$minRand;
        $sql = $dsql->SetQuery("update `#@__tieba_list` set click=click+ceil($minRand+rand()*$cur) where `state`=1 and `pubdate`>$limit_time");
//        die($sql);
    ...
    public function dianzan($tid)
    {
        ...
        $sql_a = $dsql->SetQuery("select id,uid 'admin' from `#@__tieba_list` where `state`=1 and `pubdate`>$limit_time");
//        die($sql_a);
        ...
        foreach ($res_a as $k => $v) {
            ...
            if($robots_nin_length>0){  // 机器人数据必须大于0才执行
                ...
                if (!is_numeric($res_up)) {  // 插入public_up失败
                    ...
                    $zan_sql = $dsql->SetQuery("UPDATE `#@__tieba_list` SET  `up` = up + $robots_nin_length WHERE `id` = '$tid'");
