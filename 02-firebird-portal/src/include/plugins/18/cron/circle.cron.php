...
class circle{
    ...
    public function read($tid){
        ...
        $cur = $maxRand-$minRand;
        $sql = $dsql->SetQuery("update `#@__circle_dynamic_all` set browse=browse+ceil($minRand+rand()*$cur)  where `state`=1 and `addtime`>$limit_time");
    ...
    public function dianzan($tid)
    {
        ...
        $sql_a = $dsql->SetQuery("select id,userid 'admin' from `#@__circle_dynamic_all` where `state`=1 and `addtime`>$limit_time");
        ...
        foreach ($res_a as $k => $v) {
            ...
            if($robots_nin_length>0){  // 机器人数据必须大于0才执行
                ...
                for ($i = 0; $i < $robots_nin_length; $i++) {
                    ...
                    $where_public_sql .= "'circle',";
                    $where_public_sql .= "'circle-blogdetail',";
                ...
                if (!is_numeric($res_up)) {  // 插入public_up失败
                    ...
                    $zan_sql = $dsql->SetQuery("UPDATE `#@__circle_dynamic_all` SET  `zan` = zan + $robots_nin_length WHERE `id` = '$tid'");
                    $zan_res = $dsql->dsqlOper($zan_sql, "update");
                    if ($zan_res != "ok") {  // 更新zan失败
                        $result = 0;
                    }
                }
            }
