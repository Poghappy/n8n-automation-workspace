$(function (){
    ...
    $("#addNew").bind("click", function(event){
        try {
            ...
            parent.addPage("sitePlugin18_dianzan_add", "plugins", "新增虚拟点赞量任务", "../include/plugins/18/" + href);
        } catch(e) {}
    });
    ...
    $("#delBtn").bind("click", function(){
        $.dialog.confirm('确定要删除吗？', function(){
            ...
            if(checked.length < 1){
                ...
                $.post("ajax.php?action=dianzan_del","tids="+id.join(","),function (res){
                    if(res.errno==0){
    ...
    $("#openBtn").bind("click", function(){
        $.dialog.confirm('确定要启用吗？', function(){
            ...
            if(checked.length < 1){
                ...
                $.post("ajax.php?action=dianzan_changeState","tids="+id.join(",")+"&state=1",function (res){
                    if(res.errno==0){
    ...
    $("#closeBtn").bind("click", function(){
        $.dialog.confirm('确定要停用吗？', function(){
            ...
            if(checked.length < 1){
                ...
                $.post("ajax.php?action=dianzan_changeState","tids="+id.join(",")+"&state=0",function (res){
                    if(res.errno==0){
                        huoniao.showTip('success', '操作成功', "auto");
                        setTimeout(function(){
                            location.reload();
    ...
    $("#list").delegate(".changeState", "click", function(){
        ...
        $.dialog.confirm('确定要'+changeText+'吗？', function(){
            $.post("ajax.php?action=dianzan_changeState",{"tids":tid,"state":changeState},function (res){
                if(res.errno==0){
    ...
    $("#list").delegate(".del", "click", function(){
        let tid   =$(this).closest('tr').attr("data-id");
        $.dialog.confirm('确定要删除吗？', function(){
            $.post("ajax.php?action=dianzan_del",{"tids":tid},function (res){
                if(res.errno==0){
                    huoniao.showTip('success', "删除成功", "auto");
                    setTimeout(function(){
                        location.reload();
    ...
    $("#list").delegate(".run", "click", function(){
        ...
        $.dialog.confirm('确定要执行吗？', function(){
            $.post("ajax.php?action=dianzan_runTask",{"tid":tid},function (res){
                if(res.errno==0){
    ...
    $("#list").delegate(".edit", "click", function(){
        ...
        try {
            event.preventDefault();
            var href = $(this).attr("href");
            parent.addPage("sitePlugin18_dianzan_edit_"+tid, "plugins", "修改虚拟点赞量任务", "../include/plugins/18/" + href);
        } catch(e) {}
    });
