...
<html>
<head>
<title>外卖订单分表同步</title>
<meta http-equiv="Content-Type" content="text/html;charset=utf-8">
</head>
<body>
<h1 style="padding:50px 0;text-align:center;">外卖订单分表同步</h1>
...
class checkArticleTable{
    ...
    public function run($page = 1){
        ...
        $sql = $dsql->SetQuery("SELECT * FROM `#@__site_sub_tablelist` WHERE `service` = 'waimai_order'");
        ...
        // 更新主表总表关联表
        $sql = $dsql->SetQuery("SELECT * FROM `#@__site_sub_tablelist` WHERE `service` = 'waimai_order' ORDER BY `begin_id` LIMIT $atpage, $pageSize");
        ...
        foreach ($tabs as $key => $value) {
            ...
            echo '<center style="padding-top:100px;color:red;" id="progress">正在同步外卖订单分表 '.$value['table_name'].'，请耐心等待 ······</center>';
