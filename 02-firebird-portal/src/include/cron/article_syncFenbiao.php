...
class checkArticleTable{
    ...
    public function run($page = 1){
        ...
        $sql = $dsql->SetQuery("SELECT * FROM `#@__site_sub_tablelist` WHERE `service` = 'articlelist'");
        ...
        // 更新主表总表关联表
        $sql = $dsql->SetQuery("SELECT * FROM `#@__site_sub_tablelist` WHERE `service` = 'articlelist' ORDER BY `begin_id` LIMIT $atpage, $pageSize");
...
if($user > 0){
  ...
  if(!$check && !isset($_GET['confirm'])){
    $sql = $dsql->SetQuery("SELECT COUNT(*) total FROM `#@__site_sub_tablelist` WHERE `service` = 'articlelist'");
    $res = $dsql->dsqlOper($sql, "results");
    $count = $res[0]['total'];
    // if(!$count){
