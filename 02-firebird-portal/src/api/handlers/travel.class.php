...
class travel {
    ...
    public function countryDetail(){
        ...
        $storeDetail = array();
        $id = $this->param;
        $id = is_numeric($id) ? $id : (int)$id['id'];
        $uid = $userLogin->getMemberID();

        if(!$id) return array("state" => 200, "info" => $langData['travel'][12][13]);//格式错误
...
//                            $money    = sprintf('%.2f',($usermoney-$totalPrice));
    ...
    public function orderList(){
        ...
        if($title){
            if($store){
                $where .= " AND (o.`ordernum` LIKE '%$title%' OR s.`title` LIKE '%$title%' OR m.`nickname` LIKE '%$title%')";
            }else{
                $where .= " AND (o.`ordernum` LIKE '%$title%' OR s.`title` LIKE '%$title%')";
            }
        }
        ...
        if(empty($store)){
            ...
            $archives = $dsql->SetQuery("SELECT o.`id`, o.`ordernum`, o.`store`, o.`proid`, o.`userid`, o.`procount`, o.`orderprice`, o.`orderstate`, o.`orderdate`, o.`paydate`, o.`paytype`, s.`title`, o.`ret-state`, o.`type`, o.`walktime` FROM `#@__travel_order` o  LEFT JOIN `#@__travel_store` s ON o.`store` = s.`id` LEFT JOIN `#@__member` m ON m.`id` = s.`userid` WHERE 1 = 1" . $where);
        ...
        //商家订单列表
            ...
            $where .= " AND s.`userid` = '$userid' AND o.`orderstate` != 0 ";
            if($type){
                $where .= " AND o.`type` = '$type' ";
            }
            ...
            $archives = $dsql->SetQuery("SELECT o.`id`, o.`ordernum`, o.`store`, o.`proid`, o.`userid`, o.`procount`, o.`orderprice`, o.`orderstate`, o.`orderdate`, o.`paydate`, o.`paytype`, o.`ret-state`, o.`type`, o.`walktime` FROM `#@__travel_order` o LEFT JOIN `#@__travel_store` s ON o.`store` = s.`id` LEFT JOIN `#@__member` m ON m.`id` = s.`userid` WHERE 1 = 1" . $where);
        ...
        if($results){
            ...
            foreach($results as $key => $val){
                ...
                $list[$key]['walktime']    = (int)$val['walktime'];  //出行时间
                ...
                if($val['type'] == 1 || $val['type'] == 2){//景点门票 周边游
                    ...
                    if(!empty($ret)){
                        ...
                        if($val['type'] == 1){
                            ...
                            $proDetail = $this->agencyDetail();
                            $list[$key]['title'] = $proDetail['title'];
                            $list[$key]['subtype'] = (int)$proDetail['typeid'];
                        }
                    }
                    if($val['type'] == 1){
                        $param = array(
...
//                        $list[$key]['title']       = $ret[0]['title'];
                    ...
                    $this->param = $val['proid'];
                    $proDetail = $this->visaDetail();
                    $unitprice = (float)$proDetail['price'];
                    $list[$key]['title']  = $proDetail['title'];
                ...
                $list[$key]['unitprice'] = $unitprice;  //单价
...
//        }
        ...
        if(!empty($results)){
            ...
            $people = aesDecrypt($results["people"]);
            if(is_array(json_decode($people))){
                $people = json_decode($people,true);
            }
            $orderDetail["people"]  = $people;
            $orderDetail["contact"]  = aesDecrypt($results["contact"]);
            $orderDetail["note"]  = $results["note"];
            $orderDetail["idcard"]  = aesDecrypt($results["idcard"]);
            $orderDetail["backpeople"]  = aesDecrypt($results["backpeople"]);
            $orderDetail["backcontact"]  = aesDecrypt($results["backcontact"]);
            $orderDetail["backaddress"]  = aesDecrypt($results["backaddress"]);
            $orderDetail["email"]  = aesDecrypt($results["email"]);
