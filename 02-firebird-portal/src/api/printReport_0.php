...
//查询打印机配置
$sql = $dsql->SetQuery("SELECT `print_config` FROM `#@__business_print_config` WHERE `print_code` = 'yilianyun' ORDER BY `id` DESC LIMIT 1");
$ret = $dsql->dsqlOper($sql, "results");
if ($ret) {
    $print_config = $ret[0]['print_config'] ? unserialize($ret[0]['print_config']) : array();
    if($print_config){
        $customClientId = $customClient_secret = $customPrintKey = '';
        foreach ($print_config as $key => $value) {
            if ($value['name'] == 'clientId') {
                $customClientId = $value['value'];
            } elseif ($value['name'] == 'client_secret') {
                $customClient_secret = $value['value'];
            } elseif ($value['name'] == 'signkey') {
                $customPrintKey = $value['value'];
            }
        }
    }else{
        die('打印机配置错误！');
...
if ($cmd == 'oauth_finish') {
    ...
    $signCode = md5($customClientId . $push_time . $customClient_secret);
...
if ($cmd == "print_status") {
    ...
    $_printReport->DEBUG("query:" . join("&", $data) . "\r\n");
    ...
    if ($installWaimai) {
        ...
        $sql = $dsql->SetQuery("SELECT * FROM `#@__waimai_shopprint` WHERE `id` != ''");
        $ret = $dsql->dsqlOper($sql, "results");
        if ($ret) {
            foreach ($ret as $key => $value) {
                ...
                if ($mcode == $machine_code) {
                    $sql = $dsql->SetQuery("UPDATE `#@__waimai_shopprint` SET `print_state` = $online WHERE `id` = $id");
                    $_printReport->DEBUG("waimai_sql:" . $sql . "\r\n");
    ...
    if ($installBusiness) {
        //查询数据库中的打印机
        $sql = $dsql->SetQuery("SELECT * FROM `#@__business_shopprint` WHERE `id` != ''");
        $ret = $dsql->dsqlOper($sql, "results");
        if ($ret) {
            foreach ($ret as $key => $value) {
                // $shopid       = $value['sid'];
                ...
                //如果两个终端号相同，则更新打印机状态
                if ($mcode == $machine_code) {
                    $sql = $dsql->SetQuery("UPDATE `#@__business_shopprint` SET `print_state` = $online WHERE `id` = $id");
                    $_printReport->DEBUG("business_sql:" . $sql . "\r\n");
                    $dsql->dsqlOper($sql, "update");
                }
            }
        }
    ...
    if ($installShop) {
        //查询数据库中的打印机
        $sql = $dsql->SetQuery("SELECT * FROM `#@__shop_shopprint` WHERE `id` != ''");
        $ret = $dsql->dsqlOper($sql, "results");
        if ($ret) {
            foreach ($ret as $key => $value) {
                // $shopid       = $value['sid'];
                ...
                //如果两个终端号相同，则更新打印机状态
                if ($mcode == $machine_code) {
                    $sql = $dsql->SetQuery("UPDATE `#@__shop_shopprint` SET `print_state` = $online WHERE `id` = $id");
                    $_printReport->DEBUG("shop_sql:" . $sql . "\r\n");
                    $dsql->dsqlOper($sql, "update");
                }
            }
        }

        // $sql = $dsql->SetQuery("SELECT `id`, `print_config` FROM `#@__shop_branch_store` WHERE `print_config` != ''");
        // $ret = $dsql->dsqlOper($sql, "results");
        // if($ret){
        //   foreach ($ret as $key => $value) {
        //       $shopid       = $value['id'];
        ...
        //       //如果两个终端号相同，则更新打印机状态
        //       if($mcode == $machine_code){
        //           $sql = $dsql->SetQuery("UPDATE `#@__shop_branch_store` SET `print_state` = $online WHERE `id` = $shopid");
        //           $_printReport->DEBUG("shop_branch_sql:" . $sql . "\r\n");
        //           $dsql->dsqlOper($sql, "update");
        //       }
        //   }
        // }
...
if ($cmd == $acceptTypeed) {
    ...
    if (!empty($dataid) && $state == 1) {
        ...
        if ($installWaimai) {
            ...
            /*$sql = $dsql->SetQuery("SELECT o.`id`, o.`uid`, o.`ordernumstore`, o.`pubdate`, o.`food`, o.`amount`,o.`selftime`,o.`ordertype`,o.`person`,o.`tel`, s.`shopname`,s.`address`,s.`coordX`,s.`coordY`,o.`lng`,o.`lat`,o.`address` useraddress,o.`otherparam`,s.`phone`  FROM `#@__waimai_order_all` o LEFT JOIN `#@__waimai_shop` s ON s.`id` = o.`sid` WHERE o.`state` != 3 AND o.`print_dataid` = '$dataid'");*/
            ...
            $_printReport->DEBUG("waimai sql:" . $sql . "\r\n");
            $_printReport->DEBUG("waimai ret:" . json_encode($ret, JSON_UNESCAPED_UNICODE) . "\r\n");
...
//                $pluginsres = $dsql->dsqlOper($pluginssql, "results");
    ...
    //                            $backurl = 'https://ihuoniao.cn/include/plugins/19/maiyatian.callback.php';
        ...
        if ($installShop) {
            ...
            $_printReport->DEBUG("shop sql:" . $sql . "\r\n");
            $_printReport->DEBUG("shop ret:" . json_encode($ret, JSON_UNESCAPED_UNICODE) . "\r\n");
            if ($ret) {
                $data = $ret[0];
