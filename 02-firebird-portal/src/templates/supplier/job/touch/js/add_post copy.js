...
var pageVue = new Vue({
    ...
    methods:{
        ...
        resolveInfoData(d){
            ...
            for(var i = 0; i < d.length; i++){
                ...
                }else if(d[i].name == 'address'){
                    that.newAddr['address'] = d[i].value
				...
				success: function (data) {
					if(data.state == 100){
						if(type == 'all'){
							...
							if(!that.formData.id && that.jobAddrList.length){
        ...
        checkFormData(step,noTip){
            ...
            const checkArr = ['type','title','nature','job_addr_id','min_salary','max_salary','number','valid','education','experience','note','claim']
            ...
            for(let i = 0; i < checkArr.length; i++){
                if(that.formData[checkArr[i]] === ''){
                    if(checkArr[i] == 'number' && !noTip){
                        that.formData[checkArr[i]] = 1; //招聘人数至少1人
                    }
                    else if(checkArr[i] == 'job_addr_id' && that.formData[checkArr[i]] === 0 ){
                        tip = ('请选择工作地点');
                        stop = true;
                        break;
                    }
                ...
                }else if((checkArr[i] == 'job_addr_id' && !that.formData[checkArr[i]])){
                    if(!noTip && that.newAddr && that.newAddr.lng && that.newAddr.lat){
                        that.getAllAddrList('add');
                    }
                    stop = true;
                    break;
        ...
        checkNumber(type){
            ...
            if(type == 'min' && that.formData[type + '_salary'] >= that.formData.max_salary && that.formData.max_salary){
                that.formData[type + '_salary'] = that.formData.max_salary - 1000;
            }
    ...
    watch:{
        addWorkPosi:function(val){
            const that = this;
            console.log(val,that.lng,that.lat);
