$(function(){
  ...
  if(localData || waimaiData){
    ...
    if (waimaiData) { //（1）
      ...
      if(quAdrrData){//跑腿index 过来的取货地址数据
        ...
        console.log(quAdrrData)
        $('.gz-addr-seladdr #addr').val(quAdrrData.addr);
        $('#address').val(quAdrrData.address);
        $('#people').val(quAdrrData.people);
        $('#mobile').val(quAdrrData.mobile);
      }else{
  ...
  $(".popl_box.anum_box .back,.popl_mask").click(function(){
    ...
    gzAddrSeladdr.bind("click", function(){
      if (!dingwei) return false; //没有定位成功，不可以点击
      let data=[ //定位页面会读取waimaiData，并且将定位成功的数据填充好，然后回到该页面读取填表即可
        {name:'lnglat',value:''},
        {name:'addr',value:''},
        {name:'address',value:''},
        {name:'people',value:$('#people').val()},//联系人名字
        {name:'mobile',value:$('#mobile').val()},//电话
          ...
          $.ajax({
            ...
            "success": function(data){
              if(data && data.state == 100){
                ...
                if((info.juli <= maxJuli && info.juli != 0) || stype == 'q'){
                  saveFlag = true;
                }else{
          ...
          $.ajax({
            ...
            success: function (res) {
                if(res && res.state == 100){
                    comTiao(res.info,lng,lat,people,tel,areaCode,street,address);
                }else{
                    gzAddrInit.showErr(res.info);
                }
