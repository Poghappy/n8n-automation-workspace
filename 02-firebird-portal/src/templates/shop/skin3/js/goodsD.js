...
$(function () {
			...
			success: function (data) {
				...
				if (data.state == 100 && data.info.list.length > 0) {
					...
					for (var i = 0; i < list.length; i++) {
						tcNewsHtml.push('<li><div class="orderInfo fn-clear">');
						...
						tcNewsHtml.push('<p class="cuttime" data-time="' + list[i].enddate + '" data-now="' + list[i].now + '">剩余20:25:15</p>');
						tcNewsHtml.push('</div></div><a href="javascript:;" class="pindan_btn">立即拼单</a></li>');
					}
	...
	var init = {
		...
		defautx: function () {
				...
				if (vipyouhui) {
					$(".vipyouhui .vipJian").text((maxPrice > minPrice ? minPrice : maxPrice) * vipyouhui)
				}
				var pointGet = $(".point").attr('data-point');
				if (pointGet) {
					$(".point .pointGet").text((maxPrice > minPrice ? minPrice : maxPrice) * pointGet)
				}
	...
	$('.sku').each(function () {
		...
		if (selectedObjs.length) {
			...
			mpriceObj.html((echoCurrency('symbol')) + (maxPrice > minPrice ? minPrice.toFixed(2) + "-" + maxPrice.toFixed(2) : maxPrice.toFixed(2)));
			var choseStock = SKUResult[selectedIds.join(';')].stock
			stockObj.text(choseStock);
			//获取input的值
			var inputValue = parseInt($(".singleGoods dd .num input").val());
			var inputTip = $(".singleGoods dd cite");
			if (maxCount * 1 == 0) { //表示不限购
				var canbuycount = choseStock;
			} else {
				var canbuycount = Math.min.apply(null, [choseStock, maxCount * 1]); //库存和限购的最小值为 最大购买数量
			}


			if (inputValue >= canbuycount) {
				if (canbuycount > choseStock) { //原先是等于
					inputTip.html('超过当前库存！')
				} else if (canbuycount < choseStock) {
					inputTip.html('限购' + maxCount + detailShopunit + '!');
				}
				$('.singleGoods dd .num .up').addClass('noclick');
				inputTip.show();
	...
	// 拼团购买
	$('.pindanListBox').delegate('.pindan_btn', 'click', function () {
		var $buy = $(this),
			$li = $(".sys_item_specpara"),
			$ul = $(".singleGoods dd.info"),
			n = $li.length;
	...
	$(".searchbox a.search").click(function () {
		var t = $(this),
			form = $(".searchbox");
		if (t.hasClass('search_grey')) {
			form.attr('action', shopAction)
		} else {
