$(function(){
	...
	var bubbleMapSize = {
		...
		,bubbleTemplate = {
			...
			//缩小时 只显示图标
			2 : '<div class="bubble bubble-2" data-longitude="${longitude}" data-latitude="${latitude}" data-id="${store_id}" data-url="${store_url}"><div class="map_win"><s class="arr"></s><div class="shop_detl fn-clear"> <div class="shop_logo"><img src="${store_logo}" onerror="this.src=\'/static/images/404.jpg\'" /></div> <div class="shop_info"> <h4>${store_name}</h4> <p class="starInfo"><span class="shop_star"><s></s>${store_rating}好评率</span> <span class="shop_gz">${store_collectnum}人关注</span></p> <p class="shop_addr"><s></s>${store_address}</p> </div> </div> </div><span class="${typeModule}"><a href="${store_url}" data-url="${store_url}">${store_index}</a></span></div>'
    ...
    ,getStoreData: function(type){
        ...
        $('.loading').show()
            ...
            //       map.setZoomAndCenter(new BMap.Point(selng, selat), 13);
    ...
    ,getSearch_posi:function(){
      ...
      sp_ajax = $.ajax({
        ...
        success: function(data){
          ...
          //     //没搜到结果 则回到定位的位置
          //     map.setZoomAndCenter(new BMap.Point(oldlng, oldlat), 13);
          //   }
          //
          // }else{
          //   if(directory !="")
