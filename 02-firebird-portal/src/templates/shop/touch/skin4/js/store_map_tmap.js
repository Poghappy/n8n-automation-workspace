$(function () {
    ...
    var init = {
        ...
        createMap: function () {
            ...
            map = new T.Map(g_conf.mapWrapper, { enableMapClick: false });
            if (oldlng && oldlat) {//定位成功 绘制当前位置
                map.centerAndZoom(new T.LngLat(oldlng, oldlat), 13);
            }
        ...
        , tilesloaded: function () {
            ...
            $('.quyuAlert .chooseLeft').delegate('a', 'click', function () {
                ...
                if (id == 0) {//不限 没有二级
                    ...
                    if (onelng && onelat) {
                        map.centerAndZoom(new BMap.Point(onelng, onelat), 13)
                    } else {
                        map.centerAndZoom(g_conf.cityName, 13);
                    }
        ...
        , cleanBubble: function () {
            map.clearOverLays();
            var localPoint = new T.LngLat(oldlng, oldlat)
            //创建个人图标
            var myIcon = new T.Icon({
                "iconUrl": templets+"images/map/circle.png?v=1",
                "iconSize": new T.Point(50, 50),
            });
            var marker2 = new T.Marker(localPoint,{icon:myIcon});  // 创建标注
    ...
    HN_Location.init(function (data) {
        if (data == undefined || data.address == "" || data.name == "" || data.lat == "" || data.lng == "") {
            ...
            oldlng = siteCityInfo_lng;
            oldlat = siteCityInfo_lat;
            init.createMap();
        } else {
            oldlng = data.lng;
            oldlat = data.lat;
    ...
    var bubbleMapSize = {
        1: function () {
            return new T.Point(-80, -40)
        },
        2: function () {
            return new T.Point(-30, -10)
        },
        3: function () {
    ...
    $('.zoom-local').click(function () {
        ...
        var mPoint = new T.LngLat(oldlng, oldlat)
        if (oldlng && oldlat) {//定位成功 绘制当前位置
            map.centerAndZoom(mPoint, 15);
        } else {//定位失败时就画苏州市
            var mPoint = new T.LngLat(g_conf.lng, g_conf.lat)
