...
function map_url(pageData){
	...
	if (ua.match(/MicroMessenger/i) == "micromessenger") {
        ...
        var result = gcoord.transform([pageData.lng, pageData.lat], gcoord.WGS84, gcoord.BD09);
        var tmap_lng = result[0];
        var tmap_lat = result[1];
    
        OpenMap_URL = "https://api.map.baidu.com/marker?location="+tmap_lat+","+tmap_lng+"&title="+pageData.title+"&content="+pageData.address+"&output=html"
	}
...
$(function(){
  $('body').delegate('.appMapBtn', 'tap', function(e){
    ...
    if (pageData.lat != "" && pageData.lng != "") {
      if (ua.match(/MicroMessenger/i) == "micromessenger") {
        ...
        if (site_map == "tmap"){
            var result = gcoord.transform([pageData.lng, pageData.lat], gcoord.WGS84, gcoord.GCJ02);
            pageData.lng = result[0];
            pageData.lat = result[1];
        }
        ...
        if(site_map == "baidu" || site_map == "google"){
  				e.preventDefault();
