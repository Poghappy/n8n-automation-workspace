$(function(){
	...
	if (ua.match(/MicroMessenger/i) == "micromessenger") {
		...
		wx.ready(function() {
			$('#filePicker').off('click').bind('click', function(){
	...
	//默认上传
		...
		uploader.on('uploadSuccess', function(file, response){
			...
			if(response.state == "SUCCESS"){
				$li.find("img").attr("data-val", response.url).attr("data-src", response.turl);
			}else{
				removeFile(file);
		...
		uploader.on('error', function(code){
			...
			switch(code){
				case "Q_EXCEED_NUM_LIMIT":
					txt = langData['sfcar'][1][7].replace('1',atlasMax);//图片数量已达上限
					break;
				case "F_EXCEED_SIZE":
