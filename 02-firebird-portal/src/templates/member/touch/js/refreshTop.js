...
$(function(){
  ...
  refreshTopFunc = {
    ...
    update_zjuser_btn: function(type, need){
      ...
      var btn = $('#zjuser_refresh'), href = btn.data('url1');
      ...
      if(refreshTopModule == 'house' && rtConfig.zjuserMeal.iszjuser == "1" && rtConfig.zjuserMeal.meal_check.state != 101){
        ...
        if(type == "refresh"){
          ...
          $('.house_zjuser_choose').show().children('li').click(function(){
            ...
            $('.rtSmartPackage li').removeClass('curr');            
            $('.paySubmit').addClass('paySubmit1');
            if(index == 0){
        ...
        //普通刷新单价
        $('.refreshNormalPrice').html(refreshNormalPrice + echoCurrency('short'));  //元
            ...
            smartHtml.push('<div class="pri"><small>'+echoCurrency("symbol")+'</small>'+refreshSmart[i].price+'</div>');
          ...
          if(smartHtml.length == 0){
            $('.house_zjuser_choose li:eq(0)').addClass('curr').siblings().hide();
            that_.update_zjuser_btn(type, 1);
          }else{
            $('.rtSmartPackage li.curr').click();
          }
          ...
          //如果还有免费次数
            ...
            if(topNormal[i].offer > 0){
              topNormalHtml.push('<em>'+topNormal[i].discount+'</em><span>'+langData['siteConfig'][45][51].replace('1',(topNormal[i].offer + echoCurrency("short")))+'</span>');//省1元
            }
            topNormalHtml.push('</div>');
						topNormalHtml.push('<div class="pri"><small>'+echoCurrency("symbol")+'</small>'+topNormal[i].price+'</div>');
  ...
  //选择智能刷新方案
    ...
    if(check_zjuser){
      ...
      $('.house_zjuser_choose li').removeClass('curr');
      $('#refreshTopForm #type').val('smartRefresh');
      $('.paySubmit').removeClass('paySubmit1');
  ...
  //购买并支付
  var click = false;  //是否点击刷新
  $('.paySubmit, .paySubmit1').unbind('click').bind('click', function(){
    var t = $(this);
    if(click) return false;
    if(t.hasClass('disabled')) return false;
    t.addClass('disabled');
    click = true;
    if($('.rtTopping').is(":hidden")){

	    if(t.hasClass('paySubmit1')){
	    	$('#refreshTopForm #type').val('refresh');
	    	$('#refreshTopForm #amount').val(refreshNormalPrice);
	    }else{
	    	$('.rtSmartPackage li.curr').click();
    	...
    	if($(".rtToppingType .curr").attr('data-type') == 'normal'){
    		$(".rtToppingNormal li.curr").click()
    		$('#refreshTopForm #type').val('topping');
    	}else if($(".rtToppingType .curr").attr('data-type') == 'plan'){
    		$('#refreshTopForm #type').val('toppingPlan');
    		$('#refreshTopForm #amount').val($(".totalAmount").text());
    	}
    }
          ...
          if(data && data.state == 100){
            ...
            setTimeout(function(){
               if(navigator.userAgent.indexOf('huoniao_iOS') > -1) {
			     ...
			     if(urlLocal.indexOf(currentPageOpen) > -1){
			      location.href = urlLocal;
			     }else{
			      location.href = urlLocal+'?currentPageOpen=1';
			     }
			   }else{
                location.reload();
			   }
...
var timeComputeRefresh = function (a, b) {
    ...
    this.time = a,
        ...
        this.run = function(a) {
            ...
            // 清除之前的计时器
            if(i == 0){
            	for(var m = 1; m < this.countTimer; m++){
            		clearInterval(m)
            	}
            }
        }
...
function countDownRefreshSmart(){
    $('body').find('.refreshSmartTime').each(function(i){
        var t = $(this), stime = t.attr('data-time');
        var begin = stime - nowTime;
        var time = begin > 0 ? begin : 0;
        var countDown = new CountDownRefresh(time,i);
