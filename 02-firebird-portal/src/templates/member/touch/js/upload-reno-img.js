$(function(){
	...
	if (ua.match(/MicroMessenger/i) == "micromessenger") {
		atlasMax2 = atlasMax2 > 9 ? 9 : atlasMax2;
		...
		wx.ready(function() {
			$('#filePicker2').bind('click', function(){
								...
								$.ajax({
									...
									success: function (data) {
										if (data.state == 100) {
											...
											uploadbtn2.before('<li id="' + id + '" class="thumbnail imgshow_box"><div class="img_show"><img src="'+turl+'" data-val="'+fid+'"></div><i class="del_btn">+</i></li>');
										}else {
											alert(data.info);
										}
									},
...
//			$('#uploadbtn2').append('<li>1111<li>')
		...
		uploader.on('uploadComplete', function(file){
			$('#'+file.id+'_reno').find('.progress').remove();
		});
...
//		}
	...
	if(atlasMax2 > 1){
		$list.delegate(".item img", "click", function(){
			...
			if(atlasMax2 > 1 && !t.hasClass('litpic')){
	...
	function addFile(file) {
		var $li= $('<li id="' + file.id+'_reno' + '" class="thumbnail imgshow_box"><div class="img_show"><img></div></li>');
