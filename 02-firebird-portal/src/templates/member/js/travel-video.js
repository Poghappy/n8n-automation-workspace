	...
	objId.delegate(".del", "click", function(){
		...
		if(id){
			$.dialog.confirm(langData['siteConfig'][20][543], function(){   //你确定要删除这条信息吗？
				...
				$.ajax({
					url: "/include/ajax.php?service=travel&action=operVideo&oper=del&id="+id,
					type: "GET",
					dataType: "jsonp",
...
function getList(is){
	...
	$.ajax({
		url: "/include/ajax.php?service=travel&action=videoList&u=1&orderby=2&state="+state+"&page="+atpage+"&pageSize="+pageSize,
		type: "GET",
		dataType: "jsonp",
		success: function (data) {
				...
				if(data.state == 101){
					...
					if(list.length > 0){
						...
						for(var i = 0; i < list.length; i++){
							...
							if(litpic){
								html.push('<div class="p"><a href="'+url+'" target="_blank"><s></s><img src="'+litpic+'" onerror="javascript:this.src=\''+staticPath+'images/noPhoto_100.jpg\';this.onerror=this.src=\''+staticPath+'images/404.jpg\';"></a></div>');
							}
							...
							html.push('<p>'+langData['travel'][11][12]+'：'+pubdate+arcrank+'</p>');   //发布时间
