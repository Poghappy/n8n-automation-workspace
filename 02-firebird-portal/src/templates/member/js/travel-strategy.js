	...
	objId.delegate(".del", "click", function(){
		...
		if(id){
			$.dialog.confirm(langData['siteConfig'][20][543], function(){   //你确定要删除这条信息吗？
				...
				$.ajax({
					url: "/include/ajax.php?service=travel&action=operStrategy&oper=del&id="+id,
					type: "GET",
					dataType: "jsonp",
...
function getList(is){
	...
	$.ajax({
		url: "/include/ajax.php?service=travel&action=strategyList&u=1&orderby=2&state="+state+"&page="+atpage+"&pageSize="+pageSize,
		type: "GET",
		dataType: "jsonp",
		success: function (data) {
			if(data && data.state != 200){
				if(data.state == 101){
					...
					if(list.length > 0){
						...
						for(var i = 0; i < list.length; i++){
							var item     = [],
								id       = list[i].id,
								title    = list[i].title,
								litpic   = huoniao.changeFileSize(list[i].litpic, "small"),
								typename = list[i].typename,
								url      = list[i].url,
								click    = list[i].click,
								reply    = list[i].countcomment,
								pubdate  = huoniao.transTimes(list[i].pubdate, 1);

							html.push('<div class="item fn-clear" data-id="'+id+'">');

							if(litpic){
								html.push('<div class="p"><a href="'+url+'" target="_blank"><i></i><img src="'+litpic+'" onerror="javascript:this.src=\''+staticPath+'images/noPhoto_100.jpg\';this.onerror=this.src=\''+staticPath+'images/404.jpg\';"></a></div>');
							}
							...
							html.push('<p>'+langData['siteConfig'][19][393]+'：'+typename+'&nbsp;&nbsp;·&nbsp;&nbsp;'+langData['travel'][11][12]+'：'+pubdate+arcrank+'</p>');   //分类------发布时间

							html.push('<h5><a href="'+url+'" target="_blank" title="'+title+'">'+title+'</a></h5>');

							html.push('<p>'+langData['siteConfig'][19][394]+'：'+click+'&nbsp;&nbsp;·&nbsp;&nbsp;'+langData['siteConfig'][6][114]+'：'+reply+'</p>');
							//浏览---评论
							html.push('</div>');
							html.push('</div>');
					...
					$("#audit").html(pageInfo.gray);
					$("#gray").html(pageInfo.audit);
