new Vue({
	...
	data() {
		return {
			...
			// 选择类型样式 账单类型
			isCTypeBox: false,
			isCTypeBoxCurr: false,
			currCType: '账单类型',
			// 选择类型样式 账单来源
			isOrderTypeBox: false,
			isOrderTypeBoxCurr: false,
			currOrderType: '账单来源',
			// 选择金额样式
			isAmountBox: false,
	...
	methods: {
		...
		getRecordData(){
			this.loading = true;
			axios.get( `/include/ajax.php?service=member&action=record${this.billType}${this.amountType}${this.ctype}${this.ordertype}${this.dateType}&page=${this.pageInfo.page}&pageSize=${this.pageInfo.pageSize}` ) 
			.then(response => {
					if (response.data.state == 100) { 
						if(response.data.info.pageInfo.totalPage <=1){
							...
							this.billData = response.data.info.list
							this.pageInfo=response.data.info.pageInfo;
							this.allIncome=(response.data.info.pageInfo.totalAdd).toFixed(2)
		...
		// 从接口中获取支出和收入总计
		getRecordSummary(){
			axios.get(`/include/ajax.php?service=member&action=recordSummary`)
				.then(response => {
		...
		showDateDiv(){
			...
			if(!this.isDateBox){
				...
				$('.el-picker-panel').css('opacity', '0')
			}else{
				console.log(2);
				$('.el-picker-panel').css('display', 'block')
				$('.el-picker-panel').css('opacity', '1')
			}
			if (this.isDateBox) {
				// 弹窗显示时监听点击事件
		...
		// 点击空白处 隐藏日期弹窗弹窗 
		hideDateBox(e) {
			if (!this.$refs.refDateBox.contains(e.target) ) {
		...
		// 点击空白处 隐藏账单类型弹窗弹窗 
		hideCTypeBox(e) {
			if (!this.$refs.refCTypeBox.contains(e.target) && !this.$refs.popupCTypeBox.contains(e.target)) {
				this.isCTypeBox = false
				this.isCTypeBoxCurr = false
				document.removeEventListener('click', this.hideHistory, false)
			}
		},
		// 显示账单类型弹窗
		showCTypeBox() {
			this.isCTypeBox = !this.isCTypeBox;
			this.isCTypeBoxCurr = !this.isCTypeBoxCurr;
			if (this.isCTypeBox) {
				// 弹窗显示时监听点击事件
				document.addEventListener('click', this.hideCTypeBox, false)
			} else {
				// 弹窗隐藏时撤销监听事件
				document.removeEventListener('click', this.hideCTypeBox, false)
			}
		},
		// 选择了哪个ctype,传到接口地址中进行筛选
		filterCType(item,index) {
			if (this.cTypeColor == index) {
				this.cTypeColor = '';
				this.ctype = '';
				this.pageInfo.page = 1
				this.currCType = '账单类型';
				...
				this.cTypeColor = index;
				this.ctype = `&ctype=${index}`;
				this.pageInfo.page = 1
				this.currCType = item;
				this.pagingChange()
				this.getRecordSummary();
		...
		// 点击空白处 账单来源弹窗弹窗 
		hideOrderTypeBox(e) {
			if (!this.$refs.refOrderTypeBox.contains(e.target) && !this.$refs.popupOrderTypeBox.contains(e.target)) {
				this.isOrderTypeBox = false
				this.isOrderTypeBoxCurr = false
				document.removeEventListener('click', this.hideHistory, false)
			}
		},
		// 显示账单来源弹窗
		showOrderTypeBox() {
			this.isOrderTypeBox = !this.isOrderTypeBox;
			this.isOrderTypeBoxCurr = !this.isOrderTypeBoxCurr;
			if (this.isOrderTypeBox) {
				// 弹窗显示时监听点击事件
				document.addEventListener('click', this.hideOrderTypeBox, false)
			} else {
				// 弹窗隐藏时撤销监听事件
				document.removeEventListener('click', this.hideOrderTypeBox, false)
			}
		},
		...
		filterMember(){
			if(this.orderTypeColor == '会员信息'){
		        ...
		        this.orderTypeColor = '会员信息';
		    	this.ordertype = `&ordertype=member`;
			    this.pageInfo.page = 1
			    this.currOrderType = '会员信息';
			    this.pagingChange()
			    this.getRecordSummary();
		    }
		},
	...
	mounted() {
		...
		// 初始化账单类型中的数据
		this.cTypeData = leimuarr.reduce((acc, obj) => {
			acc[obj.ctype] = obj.ctypename;
			return acc;
		}, {});
		// 初始化加载第一页数据
