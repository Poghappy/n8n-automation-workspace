	...
	objId.delegate(".del", "click", function(){
		...
		if(id){
			$.dialog.confirm(langData['siteConfig'][20][211], function(){
				...
				$.ajax({
					url: "/include/ajax.php?service=travel&action=operRentcar&oper=del&id="+id,
					type: "GET",
					dataType: "jsonp",
...
function getList(is){
	...
	$.ajax({
		url: "/include/ajax.php?service=travel&action=rentcarList&u=1&orderby=2&state="+state+"&page="+atpage+"&pageSize="+pageSize,
		type: "GET",
		dataType: "jsonp",
		success: function (data) {
			if(data && data.state != 200){
					...
					if(list.length > 0){
						...
						for(var i = 0; i < list.length; i++){
							var item        = [],
									...
									address     = list[i].addrname[list[i].addrname.length-2]+'-'+list[i].addrname[list[i].addrname.length-1],
							...
							html.push('<p>'+langData['siteConfig'][19][394]+'：'+click+'&nbsp;&nbsp;·&nbsp;&nbsp;'+typename+'&nbsp;&nbsp;·&nbsp;&nbsp;'+address+'</p>');//浏览--已售
							html.push('</div>');
							html.push('</div>');
