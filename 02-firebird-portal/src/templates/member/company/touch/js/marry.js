var pageVue = new Vue({
	el: "#vue",
	data: {
		masterDomain: masterDomain, //当前的域名
		userId: userId, //用户的id
		detail: {}, //详情信息
		hotelDetail: {},//酒店详情信息
		clientInfo: 0, //客户管理信息
		info: '', //接口错误提示
		infoError: '', //黑框提示
		warnb: false,
		loadingb: true, //页面加载
		imgError: imgError, //图片加载错误的地址
		mealList: [
			{
				id: '9',
				title: '婚礼策划',
				img: 'newDiamond',
				show: true
			},
			{
				id: '10',
				title: '租婚车',
				img: 'newCar',
				show: true
			},
			{
				id: '1',
				title: '婚纱摄影',
				img: 'newPhoto',
				show: true
			},
			{
				id: '2',
				title: '摄影跟拍',
				img: 'newCamera',
				show: true
			},
			{
				id: '7',
				title: '婚礼主持',
				img: 'newMicrophone',
				show: true
			},
			{
				id: '3',
				title: '珠宝首饰',
				img: 'newJewel',
				show: true
			},
			{
				id: '4',
				title: '摄像跟拍',
				img: 'newVideo',
				show: true
			},
			{
				id: '5',
				title: '新娘跟装',
				img: 'newLipstick',
				show: true
			},
			{
				id: '6',
				title: '婚纱礼服',
				img: 'newDress',
				show: true
			},
		],//套餐列表
		selectType: [], //选中的类别id
		mealActive: 0, //操作套餐下标
		popb: false,
		ajaxb: false,
		popType: 'warn',
		appBoolean: Boolean(navigator.userAgent.toLowerCase().match(/huoniao/)),
		reloadb: false
	},
	mounted() {
		this.detailData();
		this.typeData();
		this.clientData();
		this.onPageVisibility({
			show: res => {
				if (this.reloadb) {
					this.typeData();
				}
			},
		})
	},
	methods: {
		ajax(data, param = {}) { //接口请求
			return new Promise((resolve, reject) => {
				$.ajax({
					url: param.url || '/include/ajax.php?',
					data: data,
					type: param.type || 'POST',
					timeout: 5000, //超时时间
					success: (res) => {
						resolve(typeof res == 'object' ? res : JSON.parse(res));
					},
					error: error => {
						reject(JSON.parse(error));
					}
				})
			})
		},
		backFn() {
			if (history.length == 1) {
				location.href = `${masterDomain}/marry.html`;
			} else {
				history.go(-1);
			}
		},
		linkTo(url, reload = false,wxUrl) { //链接跳转
			this.reloadb = reload;
			if (!this.userId) { //登录验证
				url = `${this.masterDomain}/login.html`;
			} else if (this.appBoolean) { //app跳转
				setupWebViewJavascriptBridge(function (bridge) {
					bridge.callHandler('redirectNative', {
						'name': '',
						'code': '',
						'link': url
					}, function () { });
				})
			}else if(navigator.userAgent.toLowerCase().match(/micromessenger/)){ //微信环境
                wx.miniProgram.navigateTo({
                    url: wxUrl||`/pages/redirect/index?url=${encodeURIComponent(url)}`,
                    fail:res=>{ //微信浏览器  
                      location.href = url;
                    }
                  })
            } else {
				location.href = url;
			}
		},
		async detailData() { //详情信息
			let data = {
				service: 'member',
				action: 'storeCenterData',
				module: 'marry'
			};
			let result = await this.ajax(data);
			if (result.state == 100) {
				// 过期时间处理
				let info = result.info;
				let date = new Date(info.expired * 1000);
				let year = date.getFullYear();
				let month = date.getMonth() + 1;
				month = month > 9 ? month : ('0' + month);
				let day = date.getDate();
				day = day > 9 ? day : ('0' + day);
				info['endTime'] = `${year}.${month}.${day}`; //添加日期
				// 续期提醒
				info['continue'] = Boolean(info.expired - (+new Date() / 1000) < 864000); //添加续期提示(10天)
				if (info.id == 0) { //店铺信息未填写
                    alert('请先提交店铺资料！')
					location.href = `${memberUrl}/config-marry.html`;
				} else {
					this.detail = info;
				}
			} else {
				this.info = result.info;
			}
			this.loadingb = false;
		},
		async typeData() { //经营类别/酒店资料
			let data = {
				service: 'marry',
				action: 'storeDetail',
			};
			let result = await this.ajax(data);
			if (result.state == 100) {
				let selectArr = result.info.bind_moduleArr_;
				for (let i = 0; i < this.mealList.length; i++) {
					let item = this.mealList[i];
					item.show = selectArr.includes(item.id);
				}
				this.selectType = selectArr;
				this.hotelDetail = result.info;
				if (selectArr.length == 0) { //新店注册，没有经营类别，先选择
					// location.href = `${memberUrl}/marry-manage.html`;
				}
			}
		},
		async clientData() { //客户管理信息
			let data = {
				service: 'marry',
				action: 'getrese',
				page: 1,
				pageSize: 1
			};
			let result = await this.ajax(data);
			if (result.state == 100) {
				this.clientInfo = result.info.pageInfo.state0;
			}
		},
		async openFn(state) { //开启/关闭婚宴酒店
			if (this.ajaxb) {
				return false
			}
			this.ajaxb = true;
			let selectType = JSON.parse(JSON.stringify(this.selectType));
			if (state) { //添加婚宴酒店
				selectType.push('8');
			} else { //删除婚宴酒店
				let index = selectType.indexOf('8');
				selectType.splice(index, 1);
			}
			let data = {
				service: 'marry',
				action: 'storeConfigModule',
				bind_module: selectType.join(',')
			};
			let result = await this.ajax(data);
			if (result.state == 100) {
				this.selectType = selectType; //视图更新
				this.popFn(0, ''); //弹窗关闭
			}
			this.ajaxb = false;
		},
		popFn(state, type = '') {
			this.popb = state;
			this.popType = type;
		},
		selectMealFn(state) { //套餐选择跳走
			let item = this.mealList[this.mealActive];
			let url = '';
			let index=0;
			if (state) { //案例
				url = `${companyDomain}/manage-marry.html?typeid=${item.id}&filterActive=1`;
				index=1
			} else { //套餐
				url = `${companyDomain}/manage-marry.html?typeid=${item.id}&filterActive=0`;
			}
			this.linkTo(url, true,`/pages/packages/mcenter/bfabu/bfabu?mold=marry&typeid=${item.id}&filterActive=${index}`);
		},
		locationFn(url, fail) {
			if (this.hotelDetail.hoteltitle) {
				this.linkTo(url)
			} else {
				this.infoError = '请先完善酒店资料';
				this.warnb = true;
				setTimeout(res => {
					this.warnb = false;
					this.linkTo(fail);
				}, 2000)
			}
		},
		/* 监听页面显示隐藏 */
		onPageVisibility(functions) {
			var _t = {};

			var onShowCall = function () {
				if (!functions || !functions.show) {
					return;
				}
				window.clearTimeout(_t.showTime);
				_t.showTime = window.setTimeout(function () {
					functions.show();
				}, 100);
			}

			var onHideCall = function () {
				if (!functions || !functions.hide) {
					return;
				}
				window.clearTimeout(_t.hideTime);
				_t.hideTime = window.setTimeout(function () {
					functions.hide();
				}, 100);
			}

			document.addEventListener('visibilitychange', function () {
				var visibility = document.visibilityState;
				if (visibility == 'visible') {
					onShowCall();
				} else if (visibility == 'hidden') {
					onHideCall();
				}
			});

			window.addEventListener("pageshow", function () {
				onShowCall()
			}, false);

			window.addEventListener("pagehide", function () {
				onHideCall();
			}, false);
		}
	}
})