$(function(){
	...
	var delAtlasPic = function(b) {
	 ...
	 var conuploader = WebUploader.create({
		...
		window.onresize = function(){
			var changeH = $(window).height();
 			if(changeH == windoHeight){
	...
	var upvideoShow = new Upload({
		...
		fileQueued: function(file) {
			...
			$(".singel_inp.coninp .textarea").append('<div class="video_li" id="' + file.id + '"><span class="load">'+langData['business'][9][28]+'~</span></div>');  //视频上传中
		},
	...
	$('.coninp').delegate('.video_li .btn_del', 'click', function() {
		...
		showErrAlert(langData['business'][9][28]); //视频删除成功
	});
