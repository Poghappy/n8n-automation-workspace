...
function getList(tr) {
  ...
  var url ="/include/ajax.php?service=renovation&action=constructionList&u=1&sid="+Identity.store.id+"&orderby=1&state="+state+"&page="+orderpage +"&pageSize=5";
  $.ajax({
      url: url,
      type: "GET",
      dataType: "json",
          ...
          if(data && data.state == 100){
              ...
              if(list.length > 0){
                ...
                for(var i=0;i<list.length;i++){
                    ...
                    html.push('          <dd>');
                    html.push('             <span class="or_num">'+list[i].mqjd+'</span>');
                    html.push('          </dd>');
                    html.push('         </a>');
                    html.push('         </dl>');
                    html.push('        </div>');
