var pageVue = new Vue({
    ...
    methods: {
        ...
        backFn() {
            if (history.length == 1) {
                location.href = `${masterDomain}/renovation.html`;
            } else {
                history.go(-1);
            }
        },
        ...
        async detailData() { //详情信息
            ...
            if (result.state == 100) {
                ...
                if (info.id == 0) { //店铺信息未填写
                    alert('请先提交公司资料！')
                    location.href = `${memberUrl}/config-renovation.html`;
                } else {
                    this.detail = info;
                    this.bidsData();
                    this.clientData();
        ...
        async bidsData() { //招标信息
            let data = {
                ...
                u: 1,
                b: 1,
                company: this.detail.id,
                page: 1,
                pageSize: 1
            };
            let result = await this.ajax(data);
            if (result.state == 100) {
                this.bidsInfo = result.info.pageInfo.state0;
            }
        },
        clientData() { //客户信息
            let _this = this;
            ...
            async function design() { //免费设计
                ...
                let result = await _this.ajax(data);
                if (result.state == 100) {
                    _this.clientInfo += result.info.pageInfo.state0;
                }
            }
            async function offer() { //1:在线报价 0:在线预约
                let data = {
                    service: 'renovation',
                    action: 'rese',
                    company: _this.detail.id,
                    resetype: 1,
                    type: 0,
                    page: 1,
                    pageSize: 1
