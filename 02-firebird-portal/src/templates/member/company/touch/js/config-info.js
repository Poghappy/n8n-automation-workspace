$(function () {
    ...
    for (let key in formData_infoData) {
      ...
      if (key.includes('_val')) { //.val()操作
        ...
        switch (newKey) {
          case '#pics_wrap dt': {
            $(newKey).html(value);
            for (let i = 0; i < $('#pics_wrap .thumbnail').length; i++) {
              $('#pics_wrap .thumbnail').eq(i).attr('id', Math.random().toFixed(4) * 10000);
            }
            break;
          }
          case '#wechat_pic_wrap dt': {
            $(newKey).html(value);
            for (let i = 0; i < $('#wechat_pic_wrap .thumbnail').length; i++) {
              $('#wechat_pic_wrap .thumbnail').eq(i).attr('id', Math.random().toFixed(4) * 10000);
            }
            break;
          }
          default: {
  ...
  $(".LoTitle span").bind("click", function () {
    ...
    let formData = {
      ...
      '#pics_wrap dt_html': $('#pics_wrap dt').html(),
      '#wechat_pic_wrap dt_html': $('#wechat_pic_wrap dt').html(),
