	...
	MobileSelect.prototype = {
		...
		init: function(config){
			...
			_this.ensureBtn.addEventListener('click',function(){
				...
				//商城专改 -- 场次已满则重新选择
				if(_this.curValue[0].time.indexOf('red') > -1){//选择了已满场次
					showErr('该场次已满请重新选择');
					return false;
				}else{
					_this.hide();
			...
			for(var i=0; i<wheelsData.length; i++){
				...
				if(_this.jsonType){
					for(var j=0; j<wheelsData[i].data.length; j++){
						...
						//行
						tempHTML += '<li data-id="'+wheelsData[i].data[j][_this.keyMap.id]+'" class="'+cla+'">'+wheelsData[i].data[j][_this.keyMap.value]+'</li>';
					}
				}else{
					for(var j=0; j<wheelsData[i].data.length; j++){
						//行
		...
		updateWheel: function(sliderIndex, data){
			...
			if(_this.cascade){
				...
				for(var j=0; j<data.length; j++){
					...
					tempHTML += '<li data-id="'+data[j][_this.keyMap.id]+'" class="'+cla+'">'+data[j][_this.keyMap.value]+'</li>';
				}
			...
			switch(event.type){
				...
				case "touchend":
					...
					if(_this.offsetSum == 0){
						...
						_this.transitionEnd(_this.getIndexArr(),_this.getCurValue());
						//商城选择场次加特殊样式
						$('.selectContainer li').removeClass('curr');
						$('.selectContainer li').eq(_this.getIndexArr()[0]).addClass('curr')
					}
