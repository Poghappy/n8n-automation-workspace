$(function(){
    ...
    $('.tcr-search input').on({
        'focus':()=>{ //聚焦
            $('.tcr-search .inputs').css({
                ...
                'border':'2px solid #323999'
            });
            $('.tcr-search .inputs').find('img').hide();
            $('.tcr-search .btn').removeClass('jobtype');
            $('.tcr-search .btn').css({
                'border-radius':'0px 23px 23px 0px',
                'width':'108px'
            });
            $('.tcr-search .btn').find('img').show();
            $('.tcr-search .btn').find('span').text('搜索');
            // 点击搜索
            $('.tcr-search .btn').click(function(){
        ...
        'blur':()=>{ //失去焦点
            $('.tcr-search .inputs').css({
                'border-radius':'23px',
                'border':'0px solid #323999'
            });
            $('.tcr-search .inputs').find('img').show();
            $('.tcr-search .btn').addClass('jobtype');
            $('.tcr-search .btn').css({
                'border-radius':'23px',
                'width':'100px'
            });
            $('.tcr-search .btn').find('img').hide();
            $('.tcr-search .btn').find('span').text('全部职位');
            setTimeout(()=>{
                $('.tcr-search .btn').unbind();
            },500)
        }
    });
    $('.tcr-search .inputs img').click(function(){
        $('.tcr-search .inputs').submit();
    })
    ...
    let str = `
        <div>时间不限</div>
        <div data-pub=${(+new Date() - 2592000000) / 1000}>1个月</div>
        <div data-pub=${(+new Date() - 7776000000) / 1000}>3个月</div>
        <div data-pub=${(+new Date() - 15552000000) / 1000}>6个月</div>
        <div data-pub=${(+new Date() - 31104000000) / 1000}>12个月</div>
    `;
    $('.dfm-left .time').find('.fItem').html(str);
    for(let i=0;i<$('.dfm-left .time').find('.fItem div').length;i++){
        let val=$('.dfm-left .time').find('.fItem div').eq(i).attr('data-pub')-params.get('pubdate');
        if(-86400<val&&val<86400){  
            let text=$('.dfm-left .time').find('.fItem div').eq(i).text();
            $('.dfm-left .time').find('.fItem div').eq(i).css({
                'color':'#2278FF'
            })
            $('.dfm-left .time').find('span').text(text);
            break;
        }
    };
    // 筛选
    ...
    $('.df-welfare ul').delegate('li','click',function(){ //福利筛选
        let id=$(this).attr('data-id');
        replaceFn(['welfare'],[id])
    })
    $('.dfs-left ul').delegate('li','click',function(){ //薪资筛选
        let min=$(this).attr('data-min');
        let max=$(this).attr('data-max');
        replaceFn(['min_salary','max_salary'],[min,max])
    })
    $('.dfs-right .btn').click(function(){
        let minSalary=$('.dfs-right .minSalary').val();
        let maxSalary=$('.dfs-right .maxSalary').val();
        replaceFn(['min_salary','max_salary'],[minSalary,maxSalary])
    })
    ...
    $('.dfm-left .time').find('.fItem div').click(function(){ //发布时间筛选
        let pub=$(this).attr('data-pub');
        replaceFn(['pubdate'],[pub]);
    })
    ...
    // 热招、浏览记录、人才求职为空直接隐藏
    if(!$('.dlr-history .item')[0]){ //浏览记录
        $('.dlr-history').hide();
    }
    if(!$('.dlr-hot .item')[0]){ //热招公司
        $('.dlr-hot').hide();
    }
    if(!$('.dlr-find .item')[0]){ //人才求职
        $('.dlr-find').hide();
    };
    let sendSmsData = [];
    if (geetest) {
