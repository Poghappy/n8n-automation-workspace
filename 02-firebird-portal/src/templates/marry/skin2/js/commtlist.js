...
/*===================评价相关js======================*/ 
	...
	//发表评论
	$('.fabu_commt,.subtn').click(function(){
		var tis = $(this);
		if(tis.hasClass('disabled')) return false;

		var imglist = [];
		var par = tis.closest('.comm_text');

		if(par.hasClass('comm_con1')){//页面自带发表
			var con = $('#plcontent').val();  //评论内容;
			var anony = 0;
			var imgEach = $('#listSection1 .pubitem');
		}else{//弹窗发表
			var con = $('#com_con').val();  //评论内容;
			var anony = $("#anonymous").val();
			var imgEach = $('#listSection2 .pubitem');
		}
		...
		var typeStr = typeof(typeid) != 'undefined' && typeid ? '&type=marry-' + typeid :  '&type=marry-store'
		tis.addClass('disabled');
		$.ajax({
		    url: "/include/ajax.php?service=member&action=sendComment"+typeStr+"&aid="+newsid ,
		    type: "GET",
		    dataType: "json",
			data:data,
	 ...
	 //加载评论
	    function loadComment(){
	        if(id && id != undefined){
				...
				var url = '/include/ajax.php?service=member&action=getComment&aid=' + newsid + typeStr +"&page=" + page + "&pageSize=" + pageSize + "&son=1";
	            //异步获取用户信息
	            $.ajax({
	                url: url,
	                type: "GET",
...
//拼接评论列表
    function getCommentList(list,type){
        ...
        for(var i = 0; i < list.length; i++){
			...
			html.push('			<span class="com_time">'+list[i].ftime+(list[i].iphome ? ' · ' + list[i].iphome : '')+'</span>');
