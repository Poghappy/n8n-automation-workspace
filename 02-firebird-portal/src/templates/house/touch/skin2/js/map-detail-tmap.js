$(function(){
    ...
    var it = {
        initMap: function() {
            ...
            function init() {
                ...
                map = new T.Map("map-wrapper");
                centerPoint = new T.LngLat(longitude, latitude);
                centerLabel = {
                    labelHtml: '<div class="center-label"></div>',
                    labelOpts: {
                        position: centerPoint,
                        offset: new T.Point(-46, -46)
                    },
                    style: {}
                };
                ...
                var config = {
                    color: "#50bfff", //折线颜色
                    fillColor: "#50bfff",    //填充颜色。当参数为空时，折线覆盖物将没有填充效果
                    weight: "2", //折线的宽度，以像素为单位
                    opacity: 0.5,    //折线的透明度，取值范围0 - 1
                    fillOpacity: 0.1,
                    lineStyle: "solid" //折线的样式，solid或dashed
                };
                //定义该矩形的显示区域
                var circle = new T.Circle(centerPoint, 2000, config);
            ...
            function addLabel(labelHtml, labelOpts, style, index) {
                ...
                var label = new T.Label({
                    text: labelHtml,
                    position: labelOpts.position,
                    offset: new T.Point(-35, -25)
                })
                // label.setStyle(style);
                label.setBorderLine(0);
                label.setBackgroundColor('transparent');
                label.setZindex(500+index);
                map.addOverLay(label)
            }
            function search(type, render, callback) {
                var search = new T.LocalSearch(map);
                search.setSearchCompleteCallback(function(results) {
                    var baidu_nearby = [], pois = results.getPois();
                    var success = true;
                    if(pois.length > 0){
                        for (var i = 0; i < pois.length; i++) {
                            var pointObj = pois[i];
                            var lonlat = pointObj.lonlat.split(' ');
                            var pointA = new T.LngLat(lonlat[0], lonlat[1]);
                    ...
                    callback && callback(type, success, pois.length ? pois.length : 0);
            ...
            function renderDistance(baidu_nearby) {
                ...
                nearestThree.forEach(function(item) {
                    ...
                    str += '<li class="nearest-item">' + '<div class="left">' + '<span class="name">' + item.name + "</span> (" + item.address + ")" + "</div>" + '<span class="right distance">' + '<i class="icon"></i>' + '<span class="distance-item">' + distance + "</span> " + "</span>" + "</li> "
                });
            ...
            function renderLayer(list) {
                list.forEach(function(a, i) {
                    ...
                    var labelHtml = '<div class="label-container"><div class="label-wrapper bounce "><div class="self-label">' + a.name + '</div><i class="label-bottom-icon"></i></div></div>';
                    var opts = {
    ...
    var swiper = new Swiper('.topSwiper .swiper-container', {pagination: {el: '.topSwiper .swiper-pagination',type: 'fraction',} ,autoplay: true,loop: true,grabCursor: true,paginationClickable: true,
        on: {
            slideChangeTransitionStart: function(){
                ...
                if(len==1){
                    ...
                    $('.pmark').removeClass('curr');
                    $('.pmark').eq((sindex > 1 && sindex < this.slides.length-1 ? 1 : 0)).addClass('curr');
    ...
    $(".topSwiper").delegate('.topcomm', 'click', function() {
        ...
        videoSwiper.slideTo(i-1, 0, false);
