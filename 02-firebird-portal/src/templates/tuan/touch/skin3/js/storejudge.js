$(function(){
	...
	function getComments(tr){
		...
		$.ajax({
			url: "/include/ajax.php?service=tuan&action=getComment&type=tuan-store",
			data: data.join("&"),
			type: "GET",
			dataType: "json",
			success: function (data) {
				if(data && data.state == 100){
					...
					if(list.length > 0){
						...
						var scoreCount = pageinfo.sco1 + pageinfo.sco2 * 2 + pageinfo.sco3 * 3 + pageinfo.sco4 * 4 + pageinfo.sco5 * 5
						var scoreAvg = (scoreCount / totalCount).toFixed(1);
						$(".judge>.judge-tit>.shop-star .icon-star s").css('width',(scoreAvg/5*100)+'%')
						$(".judge>.judge-tit>.shop-star .grade").html(scoreAvg + '分')
