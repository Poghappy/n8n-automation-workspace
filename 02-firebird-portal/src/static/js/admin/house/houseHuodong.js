$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("houseHuodong.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("houseHuodong.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	$("#list").delegate(".more", "click", function(event){
		...
		var htmlCreateStateMenu = function(){
			...
			htmlMenu.push('      <li class="smart_menu_li"><a href="javascript:" class="smart_menu_a">已拒绝</a></li>');
			htmlMenu.push('    </ul>');
			htmlMenu.push('  </div>');
			htmlMenu.push('</div>');
...
function getList(){
	...
	huoniao.operaJson("houseHuodong.php?dopost=getList", data.join("&"), function(val){
			...
			for(i; i < tuanList.length; i++){
				...
				switch (tuanList[i].state) {
					...
					case "1":
						...
						break;
					case "2":
						state = '<span class="refuse">已拒绝</span>';
						break;
				}
				list.push('  <td class="row20 left"><img src="'+tuanList[i].litpic+'" class="litpic" />'+tuanList[i].title+'<br>楼盘：'+tuanList[i].loupantitle+'</td>');
				list.push('  <td class="row25 left">'+tuanList[i].ktime+'<br />'+tuanList[i].etime+'</td>');
				list.push('  <td class="row10 ">'+state+'<span class="more"><s></s></span></td>');
				list.push('  <td class="row15 left"><a href="'+tuanList[i].url+'" target="_blank">'+tuanList[i].countnum+'</a></td>');
				list.push('  <td class="row15 left">'+tuanList[i].pubdate+'</td>');
