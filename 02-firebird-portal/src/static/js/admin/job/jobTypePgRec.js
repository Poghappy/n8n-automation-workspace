$(function(){
	...
	// 	try {
	// 		e.preventDefault();
	// 		parent.addPage(id, "job", title, url);
	// 	} catch(e) {}
	// })
	...
	huoniao.operaJson("?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < jobSentence.length; i++){
				...
				list.push('  <td class="row25 left"><a class="userinfo" href="javascript:;" data-id="'+jobSentence[i].uid+'">'+jobSentence[i].nickname+'</a></td>');
				list.push('  <td class="row35 left">'+jobSentence[i].title+'</td>');
				list.push('  <td class="row20 left">'+jobSentence[i].pubdate+'</td>');
