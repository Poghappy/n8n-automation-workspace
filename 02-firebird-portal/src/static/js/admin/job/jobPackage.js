$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("jobPackage.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
	...
	$("#addNew").bind("click", function(event){
		...
		try {
			...
			parent.addPage("jobComboAdd", "job", "添加增值包", "job/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("jobPackage.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < jobCompany.length; i++){
				...
				list.push('  <td class="row10 left">'+jobCompany[i].type+'</td>');
				list.push('  <td class="row15 left">'+jobCompany[i].mprice+" / "+jobCompany[i].price+'</td>');
				list.push('  <td class="row10 left">'+jobCompany[i].buy+'</td>');
				list.push('  <td class="row25 left">'+jobCompany[i].job+"个 / 每天"+jobCompany[i].resume+'次 / 每天'+jobCompany[i].refresh+'次 / '+jobCompany[i].top+'天</td>');
				list.push('  <td class="row12">');
				list.push('<a data-id="'+jobCompany[i].id+'" data-title="'+jobCompany[i].title+'" href="jobPackageAdd.php?dopost=edit&id='+jobCompany[i].id+'" title="修改" class="edit">修改</a>');
