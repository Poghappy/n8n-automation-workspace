$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("jobCombo.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
	...
	$("#addNew").bind("click", function(event){
		...
		try {
			...
			parent.addPage("jobComboAdd", "job", "新增招聘套餐", "job/"+href);
		} catch(e) {}
	});
	...
	$("#delBtn").bind("click", function(){
		$.dialog.confirm('确定后将导致已购买套餐的商家套餐失效。<br />此操作不可恢复，您确定要删除吗？', function(){
			init.del();
		});
	});
...
function getList(){
	...
	huoniao.operaJson("jobCombo.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < jobCompany.length; i++){
				...
				list.push('  <td class="row25 left">'+jobCompany[i].title+'</a></td>');
				list.push('  <td class="row10 left">'+jobCompany[i].valid+'</td>');
				list.push('  <td class="row15 left">'+jobCompany[i].money+'</td>');
				list.push('  <td class="row10 left">'+jobCompany[i].buy+'</td>');
				list.push('  <td class="row25 left">'+jobCompany[i].job+' / '+jobCompany[i].resume+' / 每天'+jobCompany[i].refresh+'次 / '+jobCompany[i].top+'天</td>');
				list.push('  <td class="row12">');
				list.push('<a data-id="'+jobCompany[i].id+'" data-title="'+jobCompany[i].title+'" href="jobComboAdd.php?dopost=edit&id='+jobCompany[i].id+'" title="修改" class="edit">修改</a>');
