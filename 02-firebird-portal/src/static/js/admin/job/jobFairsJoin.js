		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("jobFairsJoin.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
  ...
  $("#keyword").keyup(function (e) {
	...
	$("#addNew").bind("click", function(event){
		...
		try {
			...
			parent.addPage("jobFairsJoin_wlAdd", "job", "新增参会信息", "job/"+href);
		} catch(e) {}
	});
	...
	//地区链接点击
	$("#list").delegate(".shenhe", "click", function(event){
		event.preventDefault();
		var id = $(this).closest('tr').attr("data-id");

        huoniao.operaJson("jobFairsJoin.php?dopost=updateState", "id="+id, function(data){
            if(data.state == 100){
                huoniao.showTip("success", data.info, "auto");
                $("#selectBtn a:eq(1)").click();
                setTimeout(function() {
...
function getList(){
	...
	huoniao.operaJson("jobFairsJoin.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < jobFairs.length; i++){
				...
				list.push('  <td class="row25 left"><a href="'+jobFairs[i].url+'" class="link" target="_blank">'+jobFairs[i].title+'</a></td>');
				list.push('  <td class="row20 left"><a href="'+jobFairs[i].curl+'" class="link" target="_blank">'+jobFairs[i].company+'</a></td>');
				list.push('  <td class="row15 left">'+jobFairs[i].nickname+'<br />'+jobFairs[i].phone+'</td>');
				list.push('  <td class="row15 left">'+jobFairs[i].pubdate+'</td>');
				list.push('  <td class="row10 left">');
				if(jobFairs[i].type===2){
					list.push(jobFairs[i].state);
				}
				list.push('</td>');
				list.push('  <td class="row12">');
				if(jobFairs[i].type===2){
					list.push('<a data-id="'+jobFairs[i].id+'" data-title="'+jobFairs[i].title+'" href="jobFairsJoin.php?dopost=edit&id='+jobFairs[i].id+'" title="修改" class="edit">修改</a>');
				}
