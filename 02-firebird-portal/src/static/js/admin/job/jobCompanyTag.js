$(function(){
		...
		printTypeTree: function(){
			...
			for(var i = 0; i < l; i++){
				(function(){
                    ...
                    style = " style='border-radius: 4px 0 0 4px!important;'";

					typeList.push('<div class="tr clearfix tr_'+jsonArray["id"]+'">');
					if(jsonArray["parentid"] == 0){
						typeList.push('  <div class="row3">&nbsp;</div>');
						typeList.push('  <div class="row60 left"><input'+style+' class="tagname" type="text" data-id="'+jsonArray["id"]+'" value="'+jsonArray["typename"]+'">'+hot+addType+'<div class="color_pick"><em style="background:'+jsonArray["color"]+';"></em></div><input type="hidden" class="color-input" value="'+jsonArray["color"]+'" /></div>');
					}else{
						typeList.push('  <div class="row3"></div>');
					...
					typeList.push('  <div class="row17"><a href="javascript:;" class="del" title="删除">编辑删除</a></div>');
					typeList.push('</div>');
	...
	$(".root").find('.color_pick').each(function(){
		...
		t.colorPicker({
			callback: function(color) {
                ...
                var id = t.closest('.tr').find('input[type=text]').attr('data-id');
				
                huoniao.operaJson("?dopost=updateType&id="+id+"&action="+action, "type=color&color="+color, function(data){
                    if(data.state == 100){
	...
	$("#addNew_").bind("click", function(){
		...
		html.push('    <div class="row60 left"><input class="tagname" style="border-radius: 4px 0 0 4px!important;" data-id="0" type="text" value=""><div class="color_pick"><em></em></div><input type="hidden" class="color-input" value="" /></div>');
		html.push('    <div class="row20"><a href="javascript:;" class="up">向上</a><a href="javascript:;" class="down">向下</a></div>');
		html.push('    <div class="row17"><a href="javascript:;" class="del">删除</a></div>');
		html.push('  </div>');
		html.push('</li>');
	...
	$("#list").delegate("input", "blur", function(){
		...
		if(id != "" && id != 0){
			huoniao.operaJson("?dopost=updateType&id="+id+"&action="+action, "type=name&typename="+value, function(data){
				if(data.state == 100){
					huoniao.showTip("success", data.info, "auto");
				}else if(data.state == 101){
					//huoniao.showTip("warning", data.info, "auto");
	...
	//设置权益
	$("#list").delegate(".equity", "click", function(event){
		event.preventDefault();
		var id = $(this).attr("data-id");
		try {
			event.preventDefault();
			parent.addPage("taskMemberLevelEquity" + id, "task", "设置权益", "task/taskMemberLevelEquity.php?id="+id);
		} catch(e) {}
	});
...
function saveOpera(type){
	...
	for(var i = 0; i < first.length; i++){
		(function(){
			var html =arguments[0], count = 0, jArray = $(html).find(">ul>li"), tr = $(html).find(".tr input[type=text]"), id = tr.attr("data-id"), val = tr.val(), color = $(html).find(".tr .color-input").val();

			if(jArray.length > 0 && val != ""){
				json = json + '{"id": "'+id+'", "name": "'+encodeURIComponent(val)+'", "color": "'+color+'", "lower": [';
				for(var k = 0; k < jArray.length; k++){
					if($(jArray[k]).find(">ul>li").length > 0){
						arguments.callee(jArray[k]);
					}else{
						var tr = $(jArray[k]).find(".tr input"), id = tr.attr("data-id"), val = tr.val();
						color = $(jArray[k]).find(".tr .color-input").val();
						if(val != ""){
							json = json + '{"id": "'+id+'", "name": "'+encodeURIComponent(val)+'", "color": "'+color+'", "lower": null},';
						}else{
							count++;
						}
					}
	...
	huoniao.operaJson("?dopost=typeAjax", "data="+json, function(data){
		if(data.state == 100){
			huoniao.showTip("success", data.info, "auto");
			if(type == ""){
				//window.scroll(0, 0);
