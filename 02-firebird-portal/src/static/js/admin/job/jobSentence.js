$(function(){
	...
	var defaultBtn = $("#delBtn, #batchAudit"),
		...
		init = {
			...
			//更新信息状态
			,updateState: function(type_, refuse = ''){
				huoniao.showTip("loading", "正在操作，请稍候...");
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("jobSentence.php?dopost=updateState&type="+type, "id="+id+"&state="+state+"&refuse=" + encodeURIComponent(refuse), function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	huoniao.operaJson("jobSentence.php?dopost=getList&type="+type, data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < jobSentence.length; i++){
				...
				list.push('  <td class="row20 left"><a href="'+jobSentence[i].url+'" class="link" target="_blank">'+jobSentence[i].title+'</a></td>');
				list.push('  <td class="row10 left"><a href="javascript:;" class="userinfo" data-id="'+jobSentence[i].userid+'">'+jobSentence[i].nickname+'</a></td>');
				list.push('  <td class="row10 left">'+jobSentence[i].people+'</td>');
				list.push('  <td class="row10 left">'+jobSentence[i].contact+'</td>');
				list.push('  <td class="row15 left">'+jobSentence[i].pubdate+'</td>');
				var state = "";
				switch (jobSentence[i].state) {
					...
					case "1":
						...
						break;
					case "2":
						state = '<span class="refuse statusTips1" data-toggle="tooltip" data-placement="bottom" data-original-title="'+jobSentence[i].refuse+'"><i class="icon-question-sign" style="margin-top: 3px;"></i>拒绝</span>';
						break;
				}
