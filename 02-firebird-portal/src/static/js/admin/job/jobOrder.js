$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("jobOrder.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
	...
	$("#stateBtn, #pageBtn, #paginationBtn, #paytypeBtn, #typeBtn").delegate("a", "click", function(){
...
$("#export").click(function(e){
	...
	paytype     = $("#paytypeBtn").attr("data-id") ? $("#paytypeBtn").attr("data-id") : "";
...
function getList(){
	...
	huoniao.operaJson("jobOrder.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < jobCompany.length; i++){
				...
				list.push('  <td class="row15 left"><a class="userinfo" href="javascript:;" data-id='+jobCompany[i].userid+'>'+jobCompany[i].username+'</a></td>');
				list.push('  <td class="row10 left">'+jobCompany[i].type_name+'</td>');
				list.push('  <td class="row20 left">'+jobCompany[i].ordernum+'<br>'+(jobCompany[i].info ?? '<br />' + jobCompany[i].info)+'</td>');
				list.push('  <td class="row10 left">'+jobCompany[i].amount+'</td>');
				list.push('  <td class="row15 left">'+jobCompany[i].orderdate+'</td>');
				list.push('  <td class="row10 left">'+jobCompany[i].paytype+'</td>');
				let orderstate = "";
				if(jobCompany[i].orderstate===1){
					orderstate = "<span class='text-success'>"+jobCompany[i].orderstate_name+"</span>"
				}else{
					orderstate = jobCompany[i].orderstate_name;
				}
				list.push('  <td class="row7">'+orderstate+'</td>');
