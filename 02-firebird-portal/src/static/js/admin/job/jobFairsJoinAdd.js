...
$(function(){
	...
	$("#fname").bind("input", function(){
		...
		if(val != ""){
			...
			huoniao.operaJson("jobFairsJoin.php?dopost=checkFairs", "key="+val, function(data){
	...
	$("#company").bind("input", function(){
		...
		if(val != ""){
			t.addClass("input-loading");
			huoniao.operaJson("jobFairsJoin.php?dopost=checkCompany", "key="+val, function(data){
				t.removeClass("input-loading");
				if(!data) {
	...
	$("#btnSubmit").bind("click", function(event){
		...
		//异步提交
		huoniao.operaJson("jobFairsJoin.php", $("#editform").serialize() + "&submit="+encodeURI("提交"), function(data){
			if(data.state == 100){
				if($("#dopost").val() == "Add"){
