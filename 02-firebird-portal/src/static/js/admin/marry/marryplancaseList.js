	...
	huoniao.operaJson("marryplancaseList.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < plancaseList.length; i++){
				...
				if(plancaseList[i]['typeid']  ==  "10"){
					list.push('  <td class="row15 left"><a href="javascript:;" >'+plancaseList[i].title+'</a>&nbsp;&nbsp;<a href="'+plancaseList[i].zjurl+'" target="_blank"><i class="icon-share" style="margin-top:4px;"></i></a></td>');
				}else{
				...
				list.push('<a data-id="'+plancaseList[i].id+'" data-title="'+plancaseList[i].title+'" href="marryplancaseAdd.php?dopost=edit&id='+plancaseList[i].id+"&typeid="+typeid+'" title="修改" class="edit">修改</a>');
