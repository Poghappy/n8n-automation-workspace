$(function(){
		...
		printTypeTree: function(){
			...
			for(var i = 0; i < l; i++){
				(function(){
					...
					typeList.push('  <div class="row60 left"><input type="text" data-type="name" class="input-medium" data-id="'+jsonArray["id"]+'" value="'+jsonArray["typename"]+'" placeholder="名称"></div>');
	...
	$("#addNew").bind("click", function(){
		...
		html.push('    <div class="row60 left"><input data-id="0" type="text" data-type="name" class="input-medium" placeholder="名称"></div>');
	...
	$("#list").delegate("input", "blur", function(){
		...
		if(id != "" && id != 0){
			huoniao.operaJson("shopAuthAttr.php?dopost=updateType&id="+id+"&type="+type, "action=single&value="+value, function(data){
				if(data.state == 100){
	...
	$(".root").delegate(".del", "click", function(event){
		...
		if(type.indexOf("编辑") > -1){
			$.dialog.confirm("确定要删除吗？", function(){
				...
				huoniao.operaJson("shopAuthAttr.php?dopost=del", "id="+id, function(data){
					if(data.state == 100){
						huoniao.showTip("success", data.info, "auto");
						setTimeout(function() {
							location.reload();
...
function saveOpera(type){
	...
	huoniao.operaJson("shopAuthAttr	.php?dopost=typeAjax&pid="+pid+"&cid="+cid+"&did="+did, "data="+json, function(data){
		if(data.state == 100){
			huoniao.showTip("success", data.info, "auto");
			if(type == ""){
				//window.scroll(0, 0);
