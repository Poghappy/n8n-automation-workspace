...
$(function(){
	...
	if(typeid && typeid != '0' ){
		...
		var title_Top = $("#title").offset().top - 160,
			...
			store_Top  =  $("#store").offset().top - 160;
	}

	$("#store").bind("change", function(){
		var id = $(this).val();
		if(id == 0){
				$(this).siblings('.tip-inline').addClass('error');
			$("#category").html("");
			$("#categoryObj").hide();
		}else{
			$(this).siblings('.tip-inline').removeClass('error');
			createStoreType(id);
			getpsList(id);
            getkdList(id);
			checkShopConfig(id); // 获取物流
		}
...
// 获取配送方式
function checkShopConfig(id){
	$.ajax({
		url: 'productAdd.php?dopost=getStoreConfig&id='+id,
		type: "POST",
		dataType: "json",
		success: function (data) {
			if(data.state == 100){
				var shopConfig = data.shopconfig[0];
				if(shopConfig.toshop == '1'){
					$('.saleCon,.saleCon .checkbox .toshop').show();
				}else{
					$('.saleCon .checkbox .toshop').hide();
				}
				if(shopConfig.merchant_deliver == '1'){
					$('.saleCon,.saleCon .checkbox .merchant_deliver').show();
					$('.saleCon .checkbox .distribution').hide();
				}else if(shopConfig.distribution == '1'){
					$('.saleCon,.saleCon .checkbox .distribution').show();
					$('.saleCon .checkbox .merchant_deliver').hide();
				}else{
					$('.saleCon .checkbox .distribution,.saleCon .checkbox .merchant_deliver').hide();
				}
				if(shopConfig.express == '1'){
					$('.saleCon,.saleCon .checkbox .express').show();
				}else{
					$('.saleCon .checkbox .express').hide();
				}
				if(shopConfig.express == '0' && shopConfig.distribution == '0' && shopConfig.merchant_deliver == '0' && shopConfig.toshop == '0'){
					$('.saleCon').hide();
				}
			}
		},
	...
	// 				console.log(logistic , list[i].id)
	// 				arr.push('<option value="'+list[i].id+'" '+(logistic == list[i].id ? "selected" : "")+'>'+list[i].title+'</option>');
	// 			}
	...
	$("#proItem").delegate(".adddiv","click",function(){
		$(this).parent().before('<dl class="clearfix cusItem"><dt><input type="text" name="cusItemKey[]" placeholder="请输入参数名" data-regex="S+" value=""></dt><dd style="position:static;"><input type="text" name="cusItemVal[]" placeholder="请输入参数值" data-regex="S+" value=""><a style="vertical-align: middle; margin-left: 5px;" data-toggle="tooltip" data-placement="right" data-original-title="删除自定义产品参数" href="javascript:;" class="icon-trash"></a></dd></dl>');
        $('#proItem .icon-trash').tooltip();
	});
	...
	$("#specification").delegate(".sure_add","click",function(){
		...
		if(dd.attr("data-title")=="颜色"){
			...
			console.log(id+'_'+inp.val(),text)
			selfadd.append('<div class="self_inp color_inp fn-clear"><input class="fn-hide" checked="checked" type="checkbox" name="speCustom'+changeText(id)+'[]" title="'+changeText(inp.val())+'" value="custom_'+changeText(id+'_'+inp.val())+'"><input type="text" class="inp" size="12" value="'+changeText(inp.val())+'"><i class="del_inp"></i><div class="img_box">'+img+'</div><div class="upimg filePicker1 '+hide1+'" id="filePicker'+pii+'">选择图片</div><div class="del_img '+hide2+'">删除图片</div><input class="spePic" type="hidden" name="speCustomPic'+id+'[]" value="'+imgurl+'" /></div>');
			renderbtn($('#filePicker'+pii))
			pii++;
		}else{
			var text = changeText(id+'_'+inp.val())
			selfadd.append('<div class="self_inp fn-clear"><input class="fn-hide" checked="checked" type="checkbox" name="speCustom'+changeText(id)+'[]" title="'+changeText(inp.val())+'" value="custom_'+text+'"><input type="text" class="inp"  size="22" value="'+changeText(inp.val())+'" /><i class="del_inp"></i></div>')
		}
		inp.val('');
 ...
 //       	}
	...
	var m = 1,
		manyHtml = function(){
			return '<table class="tab tab'+m+'"><tr><td class="mingc"><input type="text" class="mealName" placeholder="输入套餐名称（选填）" value=""><div class="allOpr fn-clear"><span class="oprbtn move" title="'+langData['siteConfig'][6][19]+'"><i></i></span><span class="oprbtn del" title="'+langData['siteConfig'][6][8]+'"><i></i></span></div></td></tr><tr><td class="items"><table><thead><tr><th width="38%" style="text-align:left;">所含商品</th><th width="15%">原单价</th><th width="15%">'+langData['siteConfig'][19][548]+'</th><th width="17%">'+langData['siteConfig'][6][11]+'</th></tr></thead><tbody><tr><td><input type="text"class="tit"/></td><td><input type="text"class="pric"/></td><td><input type="text"class="coun"/></td><td><span class="oprbtn move"title="'+langData['siteConfig'][6][19]+'"><i></i></span><span class="oprbtn del"title="'+langData['siteConfig'][6][8]+'"><i></i></span><span title="'+langData['siteConfig'][6][18]+'"class="oprbtn add"><i></i></span></td></tr></tbody></table></td></tr></table>';
		};

	$(".many").dragsort({ dragSelector: ".mingc .move", placeHolderTemplate: '<table class="tab"></table>' });
	// $(".many .tab .items tbody").dragsort({ dragSelector: ".oprbtn .move", placeHolderTemplate: '<tr class="holder"><td colspan="5"></td></tr>' });
	$(".many .tab .items tbody").dragsort({ dragSelector: ".oprbtn.move", placeHolderTemplate: '<tr class="holder"><td colspan="5"></td></tr>' });

	//删除套餐列
	$(".taocon").delegate(".danOpr .del", "click", function(){
		var t = $(this);
	...
	$(".taocon").delegate(".add", "click", function(){
		var t = $(this);
		t.closest("tr").after('<tr><td><input type="text" class="tit"></td><td><input type="text" class="pric"></td><td><input type="text" class="coun"></td><td class="danOpr"><span class="oprbtn move" title="'+langData['siteConfig'][6][19]+'"><i></i></span><span class="oprbtn del" title="'+langData['siteConfig'][6][8]+'"><i></i></span><span title="'+langData['siteConfig'][6][18]+'" class="oprbtn add"><i></i></span></td></tr>');
	});

	//删除套餐内容
	$(".taocon").delegate(".mingc .del", "click", function(){
		$(this).closest(".tab").remove();
	});
	//删除套餐内容
	$(".taocon").delegate(".items .del", "click", function(){
	...
	function createtuanTbody(fth){
		...
		if(fth.length > 0){
			...
			for(var i = 0; i < fth.length; i++){
				...
				$('.guigeCon input[name="f_mprice_'+item+'"]').val(mprice)
				$('.guigeCon input[name="f_price_'+item+'"]').val(price)
				$('.guigeCon input[name="f_inventory_'+item+'"]').val(f_inventory)
	...
	//商家配送运费模板
	getpsList(shopid);
  	function getpsList(id){
	  	$.ajax({
			url: 'logisticTemplate.php?do=ajax&sid='+id+'&logistype=1',
			type: "POST",
			dataType: "json",
			async:false,
			success: function (data) {
  	...
  	//快递配送运费模板
	getkdList(shopid);
  	function getkdList(id){
	  	$.ajax({
			url: 'logisticTemplate.php?do=ajax&sid='+id+'&logistype=0&modAdrr=' + modAdrr,
			type: "POST",
			dataType: "json",
			async:false,
			success: function (data) {
  	...
  	//选择销售类型
  	$("body").delegate(".saleCon input[type=checkbox]", "click", function(){
  		pshow();
  	})

  	// 兼容以前的商品
	if(typeof proid != 'undefined' && typesalearr != 'undefined' && typesalearr == ''){
		pshow()
 	...
 	function guigeCheck(){
		...
		var kongnum = 0;
 		eachTr.find('input').each(function(index, element) {
 			if($(this).hasClass('npri') || $(this).hasClass('mpri')  || $(this).hasClass('nkuc') ){
	...
	$(".submit").bind("click", function(event){
		...
		var btn        = $(this);
		if($(".video-js").size() > 0){
			$("#video").val($(".video-js").attr('data-val'))
		}
		...
		if(!$("#store").val() || $("#store").val() == '0' ){
			$.dialog.alert('请先选择商品所属的店铺');
			$("#store").siblings('.tip-inline').addClass('error')
			offsetTop = store_Top
            return false;
		}
		...
		console.log(proItem_Top,miaoshu_Top,buynote_Top,saleCon_Top,offsetTop)
		if(offsetTop){
		...
		$.ajax({
			// url: action,
			url: 'productAdd.php?action=productAdd&submit=' + encodeURI("提交") +'&dopost='+($("#id").val() > 0 ? "edit" : "save"),
			data:dataArr,
			type: "POST",
			dataType: "json",
			success: function (data) {
