	...
	huoniao.operaJson("shopBranchStore.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < shopStoreList.length; i++){
				...
				list.push('  <td class="row20 left">'+shopStoreList[i].title+'<br /><a href="'+shopStoreList[i].shopUrl+'" target="_blank"><small style="display: inline-block;">'+shopStoreList[i].company+'</small></a></td>');
				list.push('  <td class="row15 left"><a class="userinfo" href="javascript:;" data-id="'+shopStoreList[i].userid+'">'+shopStoreList[i].username+'</a><br />'+shopStoreList[i].contact+'</td>');
				list.push('  <td class="row10 left">'+shopStoreList[i].industry+'</td>');

                var pubdate = shopStoreList[i].pubdate;
                var pubdateArr = pubdate.split(' ');
				list.push('  <td class="row13 left">'+pubdateArr[0]+'<br />'+pubdateArr[1]+'</td>');
				...
				list.push('  <td class="row9 left">'+certi+'</td>');
				var state = "";
