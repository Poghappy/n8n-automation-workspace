		...
		init = {
			...
			,updateState: function(type, refuse = ''){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("renovationStore.php?dopost=updateState", "id="+id+"&state="+state+"&refuse=" + encodeURIComponent(refuse), function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
			...
			for(i; i < renovationStore.length; i++){
				...
				switch (renovationStore[i].state) {
					...
					case "1":
						...
						break;
					case "2":
						state = '<span class="refuse statusTips1" data-toggle="tooltip" data-placement="bottom" data-original-title="'+renovationStore[i].refuse+'"><i class="icon-question-sign" style="margin-top: 3px;"></i>拒绝</span>';
						break;
				}
