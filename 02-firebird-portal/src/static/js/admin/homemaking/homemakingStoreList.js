	...
	var defaultBtn = $("#delBtn, #batchAudit"),
		...
		init = {
            ...
            ,selectTypeList: function(type){
                ...
                typeListArr = $.parseJSON(typeListArr.replace(/[\r|\n|\t]/g,""));
			...
			,updateState: function(type, refuse = ''){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("homemakingStoreList.php?dopost=updateState", "id="+id+"&state="+state+"&refuse=" + encodeURIComponent(refuse), function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	huoniao.operaJson("homemakingStoreList.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < storeList.length; i++){
				...
				list.push('  <td class="row5 left">'+storeList[i].cityname+'</td>');
				list.push('  <td class="row10 left"><a href="javascript:;" data-id="'+storeList[i].userid+'" class="userinfo">'+storeList[i].username+'</a></td>');
				list.push('  <td class="row10 left">'+storeList[i].tel+'</td>');
				list.push('  <td class="row12 left">'+storeList[i].address+'</td>');
				...
				switch (storeList[i].state) {
					...
					case "1":
						...
						break;
					case "2":
						state = '<span class="refuse statusTips1" data-toggle="tooltip" data-placement="bottom" data-original-title="'+storeList[i].refuse+'"><i class="icon-question-sign" style="margin-top: 3px;"></i>拒绝</span>';
						break;
				}
