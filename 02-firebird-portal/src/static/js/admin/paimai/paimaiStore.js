$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("paimaiStore.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("paimaiStore.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	$("#addNew").bind("click", function(){
		...
		try {
			...
			parent.addPage("paimaiStoreAddphp", "paimai", "新增商家", "paimai/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("paimaiStoreEdit"+id, "paimai", title, "paimai/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("paimaiStore.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, paimaiStore = val.paimaiStore;
		...
		if(val.state == "100"){
			...
			for(i; i < paimaiStore.length; i++){
				list.push('<tr data-id="'+paimaiStore[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row15 left"> <a href="'+paimaiStore[i].url+'" target="_blank">'+paimaiStore[i].company+'</a></td>');
				list.push('  <td class="row15 left"><a href="javascript:;" data-id="'+paimaiStore[i].uid+'" class="userinfo">'+paimaiStore[i].username+'</a></td>');
				list.push('  <td class="row15 left">'+paimaiStore[i].addrname+'</td>');
				list.push('  <td class="row17 left">'+paimaiStore[i].tel+'<br />'+paimaiStore[i].address+'</td>');

				var state = "";
				switch (paimaiStore[i].state) {
					case "0":
				...
				list.push('  <td class="row15">'+paimaiStore[i].jointime+'</td>');
				list.push('  <td class="row10">');
				list.push('<a data-id="'+paimaiStore[i].id+'" data-title="'+paimaiStore[i].company+'" href="paimaiStoreAdd.php?dopost=edit&id='+paimaiStore[i].id+'" title="修改" class="edit">修改</a>');
