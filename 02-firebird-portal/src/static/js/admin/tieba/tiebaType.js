$(function(){
	...
	var init = {
		...
		printTypeTree: function(){
			...
			for(var i = 0; i < l; i++){
				(function(){
					...
					typeList.push('  <div class="row17 left"><a href="javascript:;" class="edit" title="编辑">编辑</a>&nbsp;&nbsp;&nbsp;<a href="javascript:;" class="del" title="删除">删除编辑</a></div>');
					typeList.push('</div>');
		...
		//树形递归分类
		,treeTypeList_: function(_id, _pid){
			var typeList = ['<option value="0">顶级分类</option>'], cl = "";
			var l=typeListArr;
			for(var i = 0; i < l.length; i++){
				(function(){
					var jsonArray =arguments[0], jArray = jsonArray.lower;
					typeList.push('<option '+(_pid == jsonArray['id'] ? 'selected' : '')+' value="'+jsonArray["id"]+'">'+cl+"|--"+jsonArray["typename"]+'</option>');
					// for(var k = 0; k < jArray.length; k++){
					// 	cl += '    ';
					// 	if(jArray[k]['lower'] != ""){
					// 		arguments.callee(jArray[k]);
					// 	}else{
					// 		typeList.push('<option '+(_pid == jArray[k]["id"] ? 'selected' : '')+' value="'+jArray[k]["id"]+'">'+cl+"|--"+jArray[k]["typename"]+'</option>');
					// 	}
		...
		,quickEdit: function(id, rank, t){
			if(id == ""){
				...
				huoniao.operaJson("tiebaType.php?dopost=getTypeDetail&action="+action, "id="+id, function(data){
					if(data != null){
						data = data[0];
						huoniao.hideTip();
						...
						$.dialog({
							...
							ok: function(){
								...
								huoniao.operaJson("tiebaType.php?dopost=updateType&action="+action, "id="+id+"&"+serialize, function(data){
									if(data.state == 100){
						...
						self.parent.$("#parentid").html(init.treeTypeList_(data.id, data.parentid, rank));
						if(data.parentid != 0){
