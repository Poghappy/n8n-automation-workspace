$(function(){
	...
	var init = {
		...
		,quickEdit: function(id, rank){
			if(id == ""){
				huoniao.showTip("warning", "请选择要修改的类型！", "auto");
			}else{
				...
				huoniao.operaJson("?dopost=getTypeDetail&action="+action, "id="+id, function(data){
					if(data != null){
						...
						$.dialog({
							...
							ok: function(){
								...
								var typename  = self.parent.$("#typename").val(),
									...
									count    = self.parent.$("#count").val(),
									cxtj_xianshi = self.parent.$("#cxtj_xianshi").val(),
                                ...
                                if($.trim(count) == "" || $.trim(count) == 0){
									alert("请输入数量");
									return false;
								}

                                if($.trim(cxtj_xianshi) == "" || $.trim(cxtj_xianshi) == 0){
									alert("请输入拒审重提限时");
									return false;
								}
						...
						self.parent.$("#count").val(data.count);
						self.parent.$("#cxtj_xianshi").val(data.cxtj_xianshi);

                        var flag = data.fabuParam.split(",");
