		...
		init = {
			...
			,updateState: function(type, refuse = ''){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("pensionstoreList.php?dopost=updateState", "id="+id+"&state="+state+"&refuse=" + encodeURIComponent(refuse), function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	huoniao.operaJson("pensionstoreList.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < storeList.length; i++){
				...
				list.push('  <td class="row10 left">'+storeList[i].cityname+'</td>');
				list.push('  <td class="row12 left"><a href="javascript:;" data-id="'+storeList[i].userid+'" class="userinfo">'+storeList[i].username+'</a></td>');
				list.push('  <td class="row10 left">'+storeList[i].tel+'</td>');
				list.push('  <td class="row15 left">'+storeList[i].catidname+'<br />'+storeList[i].price+'</td>');
				var state = "";
