...
var page = new Vue({
    ...
    data:{
        ...
        terminalList:[{ id:'h5', name:'H5' },{ id:'android', name:'安卓端' },{ id:'ios', name:'苹果端' },{ id:'harmony', name:'鸿蒙端' },{ id:'wxmini', name:'微信小程序' },{ id:'dymini', name:'抖音小程序' }],
        ...
        // 头部相关
        titleSetPop:false, //网站简称设置提示弹窗
        dyLogoSetPop:false, //抖音logo设置提示弹窗
    ...
    methods:{
        ...
        */
        ...
        choseAllPlat(val,ind){  // val => 更新后的值   ind => 当前更改的值的索引
            ...
            if(val){  //全选
                ...
                that.allModules[ind].android = 1
                that.allModules[ind].ios = 1
                that.allModules[ind].harmony = 1
            }else{ //取消
                ...
                that.allModules[ind].android = 0
                that.allModules[ind].ios = 0
                that.allModules[ind].harmony = 0
            }
            ...
            if (m.state == "SUCCESS") {
                ...
                switch (key){
                    ...
                    case 'hoverbtn':
                        ...
                        if(that.hoverBtnsFormData.btnsArr[that.hoverBtnIndex].btnType != 1){
                            that.hoverBtnsFormData.btnsArr[that.hoverBtnIndex].btnTop.styletype = 5
                        }
                        break;
                }
            }
        ...
        getObjStyle(data,box,ind){  
            ...
            if(box == 'searchBg'){
                ...
                if(that.currPlatform == 'h5'){
                    styleStrArr.push('padding-top:84px')
        ...
        getDataList(formDta,callback){
            ...
            for(let item in interfaceStr){
                ...
                if(val == 'rec=1'){
                    val = '';
                    that.listPopData.paramObj[item] = '';
                    dataArr.push('rec=1')
                }
        ...
        checkListHasData(modCon,checkDefault){ //modCon => checkDefault => 是否需要验证默认
         ...
         delBtns(btnType,ind){
            ...
            if(btnType == 'listadv'){  //自定义广告
                that.listFormData.dataObj.modCon.advList.list.splice(ind,1)
        ...
        changeProDefault(id){  //需要同步设置的数据 (接口、listArr)
            const that = this;
            console.log(that.prosFormData,id)
            ...
            if( currStyle){
                let mod = currStyle['options']['modArr'];
                for(let i = 0; i < mod.length; i++){
                    if(mod[i]['proObj']){
                        mod[i]['proObj']['style']['price'] = id == 3 ? '' : '#ff0000'
                    }else{
                        mod[i]['style']['price'] = id == 3 ? '' : '#ff0000'
                    }
        ...
        */ 
        ...
        getBottomBtns(){
            ...
            $.ajax({
                url: '/include/ajax.php?service=siteConfig&action=touchHomePageFooter&version=2.0&module=siteConfig&platform=' + platform,
                type: "GET",
                dataType: "json",
                success: function (data) {
                    if(data.state == 100){
        ...
        changeLink(str,type){
            ...
            if(!type) {  //普通链接  电话 扫一扫 
                ...
                if(obj.mini){
                    that.$set(that.linkSetForm,'mini',obj.mini)
                }
        ...
        changeLeftMods(val,type){
            ...
            // 微信小程序 只能二选一
            if(that.currPlatform == 'wxmini'){
                that.pageSetFormData.search.showLeftMods = [type]
            }
        ...
        getMiniQr(){
            ...
            if(!that.currPlatform || that.currPlatform == 'wxmini'){
                let miniPath = '/pages/packages/diy/index?preview=1&cityid=' + cityid
                $.ajax({
            ...
            if(val){ //表示 从隐藏 => 显示  
                ...
                if(oldPosiBtns[oldPosiBtns.length - 1].btnType == 2){ //需要改变
                    if(oldPosiBtns && oldPosiBtns.length > 1){
                        let ind = that.hoverBtnsFormData.btnsArr.findIndex(btn => {
                            return btn.id == oldPosiBtns[oldPosiBtns.length - 2].id
                        })
        ...
        // 微信小程序 选中城市/logo时 默认选择2行

        choseMods(val,id){
            const that = this;
            // 选中城市分站/logo并且是微信小程序  默认选中两行
            if(val && (id == 1 || id == 5) && that.currPlatform == 'wxmini'){
                that.$nextTick(() => {
                    if(that.searchColFormData.showMods.includes(5) && that.searchColFormData.showMods.includes(4)){
                        that.searchColFormData.layout = 2;
                    }
                })
            }
        },
