	...
	huoniao.operaJson("fenxiaoPoster.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < memberCoverBg.length; i++){
				...
				list.push('  <td class="row20 left"><img src="'+cfg_attachment+memberCoverBg[i].litpic+'" style="width: 50px; height: 90px; object-fit: cover;" /></td>');
				list.push('  <td class="row40 left">'+memberCoverBg[i].url+'</td>');
				list.push('  <td class="row20 left">'+memberCoverBg[i].date+'</td>');
