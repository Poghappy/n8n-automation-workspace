...
function getList(){
    ...
    huoniao.operaJson("userPayLogs.php?dopost=getList", data.join("&"), function(val){
        ...
        if(val.state == "100"){
            ...
            for(i; i < list.length; i++){
                ...
                var type = list[i].type==0?"支出":"收入";
                listArr.push('  <td class="row8 left">'+type+'</td>');
                listArr.push('  <td class="row10 left">'+list[i].paytype+'</td>');
                listArr.push('  <td class="row14 left">'+list[i].ordernum+'</td>');
                var user_info = list[i].userid>0 ? 'href="javascript:;" data-id="'+list[i].userid+'" class="userinfo"' : 'data-id="'+list[i].userid+'"';
