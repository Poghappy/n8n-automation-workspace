...
function getList(){
	...
	huoniao.operaJson("?dopost=getList", data.join("&"), function(val){
		...
		$("#totalAdd").html(val.pageInfo.totalAdd);
		$("#totalLess").html(val.pageInfo.totalLess);
		...
		if(val.state == "100"){
			...
			for(i; i < list.length; i++){
				...
				listArr.push('  <td class="row17 left">'+list[i].code+'</td>');
				listArr.push('  <td class="row10 left">'+list[i].amount+'</td>');
				listArr.push('  <td class="row15 left">'+list[i].expire+'</td>');

				var state = "";
				if(list[i].state == "0"){
					state = '<span class="audit">未使用</span>';
				}else{
					state = '<span class="gray">已使用</span>';
				}
				listArr.push('  <td class="row10 left">'+state+'</td>');
				listArr.push('  <td class="row15 left">'+list[i].time+'</td>');
				listArr.push('  <td class="row15 left">');
				if(list[i].state == "1"){
					listArr.push('  	<a href="javascript:;" data-id="'+list[i].uid+'" class="userinfo">'+list[i].username+'</a>');
				}
				listArr.push('  </td>');
	...
	$("#addNew").bind("click", function(){
		...
		try {
			...
			parent.addPage("couponAdd", "member", $(this).text(), "member/"+href);
		} catch(e) {}
	});
	...
	$("#export").click(function(e){
		...
		var url = '?do=export&sKeyword='+sKeyword+'&start='+start+'&end='+end+'&type='+type;
		console.log(url);
