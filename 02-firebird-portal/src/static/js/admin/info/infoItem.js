				...
				if(type == "edit"){
					huoniao.operaJson("infoItem.php?action=getDetail", "id="+id+"&tid="+typeid, function(data){
						if(data != null && data.length > 0){
							...
							self.parent.$("input[name='custom'][value='"+data.custom+"']").attr("checked", true);

							self.parent.$("#options").val(data.options);
							self.parent.$("#default").val(data.default);
							self.parent.$("#search"+data.search).prop("checked",true);
							self.parent.$("#heightlight"+data.heightlight).prop("checked",true);
	...
	huoniao.operaJson("infoItem.php?action=list", data.join("&"), function(val){
		...
		if(val.state == 100){
			...
			for(i; i < list.length; i++){
				...
				listArray.push('  <td class="row50 left"><small style="display: inline-block; padding-right: 5px;">['+list[i].id+']</small>'+list[i].title+'</td>');
				listArray.push('  <td class="row35">'+list[i].formtype+'</td>');
