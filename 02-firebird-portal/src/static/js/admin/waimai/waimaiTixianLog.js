...
function getList(){
	...
	huoniao.operaJson("?dopost=getList", data.join("&"), function(val){
		...
		$(".totalCount").html(val.info1+val.info2+val.info3);
		$(".state0").html(val.info1);
		$(".state1").html(val.info2);
		$(".state2").html(val.info3);
		$("#totalMoney").html('￥'+val.money);
		...
		if(val.state == "100"){
			...
			for(i; i < list.length; i++){
				...
				listArr.push('  <td class="row15 left">'+list[i].username+'</td>');
				listArr.push('  <td class="row15 left">'+list[i].date+'</td>');
				let bank = "";
				if(list[i].cardname){
					bank += list[i].cardname;
					bank += "（";
				}
				bank += list[i].bank;
				if(list[i].cardname){
				...
				listArr.push('  <td class="row25 left">'+bank+'</td>');
				listArr.push('  <td class="row15 left">¥'+list[i].price+'</td>');

				var state = "";
				if(list[i].state == 1){
					state = '<font color="#30992e">成功</font>';
				}
				else if(list[i].state=="2"){
					state = '<font color="#b1000f">失败</font>';
				}
				else{
					if(list[i].auditstate==1){
						state = '<font color="#aaa">审核通过，待打款</font>';
					}else{
						state = '<font color="#aaa">审核中</font>';
					}
				}
				// listArr.push('  <td class="row10 left">'+state+'</td>');
				listArr.push('  <td class="row20 left">'+state+'</td>');
				}
