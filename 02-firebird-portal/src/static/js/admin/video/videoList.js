...
$(function(){
	...
	//收费记录
	$("#list").delegate(".charge", "click", function(event){
		var id = $(this).attr("data-id"),
			title = $(this).attr("data-title"),
			href = $(this).attr("href");
		...
		try {
			event.preventDefault();
			parent.addPage("videoPayLogs"+id, "video", "付费记录【"+title+"】", href);
		} catch(e) {}
	});
	...
	huoniao.operaJson("videoJson.php?dopost="+action, data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < articleList.length; i++){
				...
				list.push('  <td class="row25 left">'+img+'<span><a href="'+articleList[i].url+'" target="_blank"'+color+'>'+articleList[i].title+'</a>'+append+'</span></td>');
				list.push('  <td class="row10 left"><a href="javascript:;" data-id="'+articleList[i].typeid+'" class="type">'+articleList[i].type+'</a></td>');
				list.push('  <td class="row0 left">'+(recycle != "" ? "&nbsp;" : articleList[i].cityname)+'</td>');
				var state = "";
				...
				list.push('  <td class="row10 state">'+(recycle != "" ? "&nbsp;" : state+'<span class="more"><s></s></span>')+'</td>');
				list.push('  <td class="row12 left"><a href="javascript:;" class="userinfo" data-id="'+articleList[i].admin+'">'+articleList[i].adminame+'</a></td>');
				list.push('  <td class="row10 left">'+(articleList[i].charge.count > 0 ? ('<a class="link charge" data-id="'+articleList[i].id+'" data-title="'+articleList[i].title+'" href="video/videoPayLogs.php?sKeyword=%23'+articleList[i].id+'">'+articleList[i].charge.count+'次 共'+articleList[i].charge.amount+echoCurrency('short')+'</a>') : '暂无付费')+'</td>');
				list.push('  <td class="row12 left">'+articleList[i].date+'</td>');
