jQuery.extend({
	createUploadIframe: function(d, b) {
		...
		$('#'+a).contents().find("head").append('<meta name="format-detection" content="telephone=no,email=no"/>')
	...
	ajaxFileUpload: function(k) {
		...
		var d = function(l) {
			...
			try {
				if (p.contentWindow) {
					p.contentWindow.document.head.innerHTML = '<meta name="format-detection" content="telephone=no,email=no"/>'
					f.responseText = p.contentWindow.document.body ? p.contentWindow.document.body.innerHTML.replace(/<.*?>/g,""): null;
					...
					if (p.contentDocument) {
						p.contentWindow.document.head.innerHTML = '<meta name="format-detection" content="telephone=no,email=no"/>'
						f.responseText = p.contentDocument.document.body ? p.contentDocument.document.body.innerHTML.replace(/<.*?>/g,""): null;
