...
var uploadCustom = {
    ...
    rotateAtlasPic: function (mod, direction, img, c) {
        showLoading = $.dialog.tips('操作中...', 600, 'loading.gif');
        showLoading.show();
        var g = {
            mod: mod,
            type: "rotateAtlas",
...
$(function () {
    ...
    $('.filePicker').each(function () {
        ...
        if (img != "") {
            if (type == "certs" || (type == "certificate" && img == "certs")) {
        ...
        function addFile(file) {
            ...
            if (type == "thumb" || (pubModelType == 'shop' && type == 'certificate')) {
            ...
            else if (type == "desc" || (pubModelType == 'renovation' && type == 'certificate')) {
                ...
                var $li = $('<li class="pubitem fn-clear" id="' + file.id + '"><a class="li-move" href="javascript:;" title="' + langData['siteConfig'][23][42] + '">' + langData['siteConfig'][6][19] + '</a>' + //拖动调整图片顺序 移动
                    ...
                    '<input type="text" style="margin:0 0 10px 0!important; width:47%; float:left;" class="i-link inputVal" placeholder="' + langData['siteConfig'][38][18] + '" value="">' + //请输入图片链接
                    '<input type="text" style="margin:0 0 10px 10px!important; width:44%; float:left;" class="i-desc" placeholder="' + langData['siteConfig'][38][19] + '" value="" />' + //请输入图片介绍
            ...
            } else if (type == "album" || type == "single" || (pubModelType == 'waimai' && type == 'certificate') || (pubModelType == 'business' && type == 'certificate')) {
            ...
            } else if (type == "certs" || type == "certificate") {
                var $li = $('<li class="pubitem fn-clear" id="' + file.id + '"><a class="li-move" href="javascript:;" title="' + langData['siteConfig'][23][42] + '">' + langData['siteConfig'][6][19] + '</a>' + //拖动调整图片顺序 移动
                    ...
                    '<a href="javascript:;" class="Rrotate" title="' + langData['siteConfig'][23][44] + '"></a>' + //顺时针旋转90度
                    '<a href="" target="_blank" class="enlarge" title="' + langData['siteConfig'][23][45] + '"></a>' + //放大
                    '</span><span class="ibg"></span><img data-val="" src=""></div><div class="li-input" style="display: block;"><input class="i-name" placeholder="' + langData['siteConfig'][38][17] + '" value="" style="margin: 0 10px 0 0;" />' + //请输入图片名称
        ...
        uploader.on('error', function (code) {
            ...
            switch (code) {
                case "Q_EXCEED_NUM_LIMIT":
                    txt = langData['siteConfig'][57][29];//图片数量已达上限
                    break;
                case "F_EXCEED_SIZE":
                    txt = langData['siteConfig'][57][28].replace(/ 0 /g, uploadFileType).replace('1', (size / 1024 / 1024)); //图片大小超出限制，单张图片最大不得超过1MB
                    break;
                case "F_DUPLICATE":
                    txt = langData['siteConfig'][57][30];//此图片已上传过
                    break;
            }
            alert(txt);
        });
            ...
            if (img == "certs") {
                if (certs != "") {
                    for (var j = 0; j < list.length; j++) {
                        ...
                        picList.push('  </div>');
                        picList.push('  <div class="li-input" style="display:block;"><input class="i-name" placeholder="' + langData['siteConfig'][38][29] + '" value="' + imgItem[1] + '" style="margin: 0 10px 0 0;" />');//请输入资质名称
                ...
                for (var j = 0; j < list_length; j++) {
                    ...
                    if (listJ != undefined && listJ != '') {
                        ...
                        if (type == "desc") {
                            if (path == undefined) {
                                var imgItem = list[j].indexOf('##') > -1 ? list[j].split("##") : list[j].split("|");
    ...
    function imgListVal(obj) {
        var dd = obj.closest('.listImgBox'), btn = dd.find('.filePicker'), type = btn.data("type"), realtype = btn.data("type-real"),
        ...
        if (listLi.length != 0) {
            for (var k = 0; k < listLi.length; k++) {
                if (type == "thumb") {
                    ...
                    $li_list.push(imgsrc);
                    dd.find('.imglist-hidden').val($li_list);
                } else if (type == "certs" || (pubModelType == 'renovation' && type == "certificate")) {
                    var imgsrc = listLi.eq(k).find('img').attr("data-val"), name = listLi.eq(k).find('.i-name').val(), note = listLi.eq(k).find('.i-note').val();

                    if (listLi.eq(k).find('.i-name').size() == 0) {
                        name = listLi.eq(k).find('.li-desc').val();
                        note = '';
                    }

                    $li_list.push(imgsrc + "##" + name + "##" + note);
                    dd.find('.imglist-hidden').val($li_list.join("||"));
                } else if ((pubModelType == 'business' || pubModelType == 'waimai') && type == "certificate") {
                    var imgsrc = listLi.eq(k).find('img').attr("data-val");
                    $li_list.push(imgsrc);
                    dd.find('.imglist-hidden').val($li_list.join(","));
                } else if (type == "quanj") {
                    ...
                    if (realtype == 'video') {
                        var imgsrc = listLi.find('video').attr('data-val');
                    } else {
    ...
    $(".listSection").delegate(".Lrotate", "click", function () {
        ...
        uploadCustom.rotateAtlasPic(pubModelType, "left", img, function (data) {
            ...
            if (data.state == "SUCCESS") {
                t.parent().siblings("img")
                    .attr('data-val', data.info)
                    .attr('data-url', data.info);
                url = '/include/attachment.php?f=' + data.info;

                t.siblings('.enlarge').attr('href', url);
    ...
    if (wxUploadObj.size() > 0 && $('#wxUploadImg').size() > 0) {
        //获取ticket
        $.ajax({
            type: "POST",
