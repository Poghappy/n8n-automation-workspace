...
//列表
if(empty($action)){
    $templates = "printerType.html";

    //js
    $jsFile = array(
        'ui/jquery.dragsort-0.5.1.min.js',
        'ui/jquery-ui-sortable.js',
        'admin/business/printerType.js'
    );

    //查询数据库中启用的打印机方式
    $print_list = array();
    $archives = $dsql->SetQuery("SELECT * FROM `#@__business_print_config` ORDER BY `weight`, `id`");
...
//安装
    ...
    if($submit == "提交"){
        ...
        //保存到主表
        $archives = $dsql->SetQuery("INSERT INTO `#@__business_print_config` (`print_code`, `print_name`, `print_desc`, `print_config`, `state`, `pubdate`) VALUES ('$code', '$print_name', '$print_desc', '$print_config', '$state', '".GetMkTime(time())."')");
        ...
        if($f != ""){
            ...
            foreach($config as $key => $val){
                ...
                if($val['type'] == "text"){
                    ...
                    $ddHtml .= ' <textarea id="printTemplate" name="printTemplate" style="display: none;"></textarea>';
...
//配置
    ...
    $archives = $dsql->SetQuery("SELECT * FROM `#@__business_print_config` WHERE `id` = ". $id);
...
//
        //格式化
        $json = objtoarr(json_decode($_POST['print_config'], true));
        $print_config = serialize($json);
        //保存到主表
        $state = (int)$state;
        $archives = $dsql->SetQuery("UPDATE `#@__business_print_config` SET `print_name` = '$print_name', `print_desc` = '$print_desc', `print_config` = '$print_config', `state` = '$state' WHERE `id` = ". $id);
        $return = $dsql->dsqlOper($archives, "update");
    ...
    $printConfig = $printData[0]['print_config'] ? unserialize($printData[0]['print_config']) : '';
        ...
        foreach($config as $key => $val){
            ...
            if ($printConfig != null && is_array($printConfig)) {
                foreach($printConfig as $k => $v){
                    ...
                    //外卖小票
                    if ($v['name'] =='printTemplate'){
                        $value = $v['value'] ? json_encode($v['value']) : '';
                    }
                }
            }
...
//卸载
    ...
    if($id != ""){
        $archives = $dsql->SetQuery("DELETE FROM `#@__business_print_config` WHERE `id` = ".$id);
        $results = $dsql->dsqlOper($archives, "update");
        if($results == "ok"){
            echo '{"state": 100, "info": "保存成功！"}';
        }else{
