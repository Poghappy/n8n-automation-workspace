...
checkPurview("carStoreList");
$dsql  = new dsql($dbo);
$tpl   = dirname(__FILE__)."/../templates/car";
$huoniaoTag->template_dir = $tpl; //设置后台模板目录
$templates = "carStoreList.html";
...
if($dopost == "getList"){
	...
	$archives = $dsql->SetQuery("SELECT `id`, `cityid`, `title`, `userid`, `areaCode`, `tel`, `address`, `state`, `flag`, `weight`, `pubdate`, `refuse` FROM `#@__".$tab."` WHERE 1 = 1".$where);
	...
	if(count($results) > 0){
		...
		foreach ($results as $key=>$value) {
			...
			$list[$key]["tel"]      = ($value["areaCode"] == '86' ? '' : $value["areaCode"]) . $value["tel"];
...
//删除
	...
	if($id != ""){
		...
		if(!empty($error)){
            ...
            dataAsync("car",$id,"store");  // 汽车门户-线下门店-删除
...
//更新状态
	...
	if($id != ""){
        ...
        dataAsync("car",$async,"store");  // 汽车门户-线下门店-更新状态
