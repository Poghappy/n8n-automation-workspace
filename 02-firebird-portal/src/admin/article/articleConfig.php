...
if($type != ""){
	...
	if($type == "site"){
		...
		$customRewardSwitch    = (int)$rewardSwitch;
		$customRewardLimit     = (float)$rewardLimit;
		$customRewardOption    = trim($rewardOption);
		$customDataShare       = (int)$dataShareSwitch;

        /*页面显示内容开关*/
        $customArticleTypeOption = is_array($articleTypeOption) && $articleTypeOption ? join(',',$articleTypeOption) : ',';
	...
	$customInc .= "\$customRewardSwitch = ".(int)$customRewardSwitch.";\r\n";
	$customInc .= "\$customRewardLimit = ".(float)$customRewardLimit.";\r\n";
	$customInc .= "\$customRewardOption = '".$customRewardOption."';\r\n";
	$customInc .= "\$customDataShare = ".(int)$customDataShare.";\r\n";
	$customInc .= "\$customfenXiao = ".(int)$customfenXiao.";\r\n";
    $customInc .= "\$customArticleTypeOption = '" . $customArticleTypeOption . "';\r\n";
	//模板风格
...
if(file_exists($tpl."/".$templates)){
	...
	$huoniaoTag->assign('rewardSwitchChecked', (int)$customRewardSwitch);

	$huoniaoTag->assign('rewardLimit', $customRewardLimit ? (float)$customRewardLimit : 100);
	$huoniaoTag->assign('rewardOption', $customRewardOption ? $customRewardOption : "1\r\n2\r\n5\r\n10\r\n20");
    ...
    //页面显示内容开关
    $huoniaoTag->assign('articleTypeOptionVal', array('pic', 'video', 'short_video', 'media', 'special'));
	$huoniaoTag->assign('articleTypeOptionName',array('图集', '视频', '短视频', '媒体号', '专题'));
	$huoniaoTag->assign('articleTypeOption', $customArticleTypeOption ? explode(',', $customArticleTypeOption) : array('pic', 'video', 'short_video', 'media', 'special'));
