 ...
 * 分类信息频道配置
 *
 * @version        $Id: sfcarConfig.php 2013-7-7 上午10:33:36 $
 * @package        HuoNiao.sfcar
 * @copyright      Copyright (c) 2013 - 2018, HuoNiao, Inc.
 * @link           https://www.ihuoniao.cn/
 */
define('HUONIAOADMIN', "..");
require_once(dirname(__FILE__) . "/../inc/config.inc.php");
checkPurview("awardlegouConfig");
$dsql = new dsql($dbo);
$tpl = dirname(__FILE__) . "/../templates/awardlegou";
$huoniaoTag->template_dir = $tpl; //设置后台模板目录
$templates = "awardlegouConfig.html";

$action = $action == "" ? "awardlegou" : $action;
...
if ($action == "delTpl") {
    ...
    if ($deldir) {
        ...
        echo '{"state": 100, "awardlegou": ' . json_encode("删除成功！") . '}';
    } else {
        echo '{"state": 200, "awardlegou": ' . json_encode("删除失败！") . '}';
    }
    die;
}
...
if ($type != "") {
    ...
    if ($type == "site") {
        ...
        $customLogo          = $articleLogo != '' ? $articleLogo : '';
        $customLogoUrl       = $litpic != '' ? $litpic : '';
        $customSharePic      = $sharePic != '' ? $sharePic : '';
        $customSubDomain     = $subdomain != '' ? $subdomain : '';
        $customChannelDomain = $channeldomain != '' ? $channeldomain : '';
        $customChannelSwitch = $channelswitch != '' ? $channelswitch : '';
        $customCloseCause    = $closecause != '' ? $closecause : '';
        $customfenXiao       = $fenXiao;
        $customDataShare     = (int)$dataShareSwtich;
        $customCanceltime    = (int)$canceltime;
        $customLgShiming     = (int)$shiming;

        $customWtuikuannum   = (int)$wtuikuannum;
        $customYtuikuannum   = (int)$ytuikuannum;

        $customWeichaononote = $weichaononote;
        $customYichaononote  = $yichaononote;
        $customWeichaonote   = $weichaonote;
        $customYichaonote    = $yichaonote;
        ...
        $customInsertselect   = (int)$insertselect;
        $customCommentCheck   = (int)$commentCheck;
        $customDataShare      = (int)$dataShareSwtich;
        $customconfirmDay     = (int)$confirmDay;

        $customautotuikuan     = (int)$autotuikuan;
        $customautotuihuo      = (int)$autotuihuo;
        $customofftuikuan      = (int)$offtuikuan;
        $custom_pointurl       = $pointurl;

        $customrefundReason    = $refundReason;


        if ($customChannelName == "" || $customLogo == "" || $customChannelDomain == "")
            die('{"state": 200, "awardlegou": ' . json_encode("请填写完整！") . '}');

        //验证域名是否被使用
        if (!operaDomain('check', $customChannelDomain, $action, 'config'))
            die('{"state": 200, "awardlegou": ' . json_encode("域名已被占用，请重试！") . '}');
    ...
    $customInc .= "\$customInsertselect = '" . (int)$customInsertselect . "';\r\n";
    ...
    $customInc .= "\$customLgShiming = ".(int)$customLgShiming.";\r\n";

    $customInc .= "\$customautotuikuan = ".(int)$customautotuikuan.";\r\n";
    $customInc .= "\$customautotuihuo = ".(int)$customautotuihuo.";\r\n";
    $customInc .= "\$customofftuikuan = ".(int)$customofftuikuan.";\r\n";


    $customInc .= "\$custom_pointurl = '".$custom_pointurl."';\r\n";
    $customInc .= "\$customDataShare = ".(int)$customDataShare.";\r\n";
    $customInc .= "\$customCanceltime = ".(int)$customCanceltime.";\r\n";
    //模板风格
    ...
    $customInc .= "\$customWtuikuannum = '" . $customWtuikuannum . "';\r\n";
    $customInc .= "\$customYtuikuannum = '" . $customYtuikuannum . "';\r\n";
    $customInc .= "\$customWeichaononote = '" . $customWeichaononote . "';\r\n";
    $customInc .= "\$customYichaononote = '" . $customYichaononote . "';\r\n";
    $customInc .= "\$customWeichaonote = '" . $customWeichaonote . "';\r\n";
    $customInc .= "\$customYichaonote = '" . $customYichaonote . "';\r\n";

    /*有奖乐购规则*/
    $customInc .= "\$custom_awardlegouGuize = '" . $custom_awardlegouGuize . "';\r\n";
    $customInc .= "?" . ">";

    $customIncFile = HUONIAOINC . "/config/" . $action . ".inc.php";
    $fp = fopen($customIncFile, "w") or die('{"state": 200, "awardlegou": ' . json_encode("写入文件 $customIncFile 失败，请检查权限！") . '}');
    fwrite($fp, $customInc);
    fclose($fp);

    die('{"state": 100, "sfcar": ' . json_encode("配置成功！") . '}');
    exit;
}
...
if (file_exists($tpl . "/" . $templates)) {
    ...
    $jsFile = array(
        ...
        'admin/awardlegou/awardlegouConfig.js',
		'publicAddr.js',
    );
    ...
    $huoniaoTag->assign('pointurl', $custom_pointurl);
    $huoniaoTag->assign('canceltime', $customCanceltime);
    ...
    //实名认证-单选
    $huoniaoTag->assign('lgShimingSwitch', array('1', '0'));
    $huoniaoTag->assign('lgShimingSwitchNames',array('禁用','启用'));
    $huoniaoTag->assign('lgShimingSwitchChecked', (int)$customLgShiming);
    ...
    //获取域名信息
    $domainawardlegou = getDomain($action, 'config');
    $huoniaoTag->assign('channeldomain', $domainawardlegou['domain']);
    ...
    $huoniaoTag->assign('insertselect', array('0', '1'));
    $huoniaoTag->assign('insertselectNames', array('输入', '选择'));
    $huoniaoTag->assign('insertselectChecked', (int)$customInsertselect);
    ...
    $huoniaoTag->assign('thumbSize', (int)$custom_thumbSize);
    ...
    $huoniaoTag->assign('customautotuikuan', $customautotuikuan);  /*自动退款*/
    $huoniaoTag->assign('customautotuihuo', $customautotuihuo);    /*自动同意退货*/
    $huoniaoTag->assign('customofftuikuan', $customofftuikuan);    /*自动关闭退款*/
    ...
    $huoniaoTag->assign('body', stripslashes($custom_awardlegouGuize));
    ...
    $huoniaoTag->assign('customWtuikuannum', $customWtuikuannum);
    $huoniaoTag->assign('customYtuikuannum', $customYtuikuannum);
