...
if($dopost == "getList"){
	...
	$archives = $dsql->SetQuery("SELECT `id`, `cityid`, `title`, `userid`, `tel`, `address`, `state`,  `weight`, `pubdate`, `flag`, `refuse` FROM `#@__".$tab."` WHERE 1 = 1".$where);
...
//删除
	...
	if($id != ""){
        ...
        dataAsync("education",$async,"store");  // 教育机构、店铺、删除
        if(!empty($error)){
...
//更新状态
	...
	if($id != ""){
		...
		dataAsync("education",$async,"store");  // 教育机构、店铺、更新状态
		if(!empty($error)){
			echo '{"state": 200, "info": '.json_encode($error).'}';
		}else{
			checkeducationStoreCache($id);
