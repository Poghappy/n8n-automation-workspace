...
if($dopost == "save" && $submit == "提交"){
    ...
    `openStart`, `openEnd`, `note`, `body`, `jointime`, `click`, `weight`, `state`, `cityid`, `top`, `video`, `video_pic`, `pic`, `phone`, `score`, `wechat_pic`, `shop_openid`,`is_vipguanggao`)
    VALUES ('$uid', '$stype', '$addrid', '$address', '$circle', '$subway', '$lnglat', '$tel', '$openStart', '$openEnd', '$note',
    '$body', '".GetMkTime(time())."', '$click', '$weight', '$state', '$cityid', '$istop',  '$video', '', '$imglist', '$phone', '$score', '$wechat_pic', '$shop_openid','$advertising')");
    ...
    if($submit == "提交"){
        ...
        //保存到表
        $archives = $dsql->SetQuery("UPDATE `#@__".$tab."` SET `uid` = '$uid', `stype` = '$stype', `addrid` = '$addrid', `address` = '$address', `circle` = '$circle', `subway` = '$subway', `lnglat` = '$lnglat', `tel` = '$tel', `openStart` = '$openStart', `openEnd` = '$openEnd', `note` = '$note', `body` = '$body', `click` = '$click', `weight` = '$weight', `state` = '$state', `cityid` = '$cityid', `top`='$istop',  `video`='$video', `pic`='$imglist', `phone`='$phone' , `wechat_pic` = '$wechat_pic', `shop_openid` = '$shop_openid',`is_vipguanggao` = '$advertising' WHERE `id` = ".$id);
...
//                $imglist = explode("###", $pics);
                $imglist = explode('###', str_replace('||', '', $pics));
            }
