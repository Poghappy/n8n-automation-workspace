...
if($userType == 3){
	//查询信息
	$comSql = $dsql->SetQuery("SELECT `id` FROM `#@__shop_store` WHERE `state` = 1" . getCityFilter('`cityid`'));
	...
	if($comResult) {
		...
		$sql = $dsql->SetQuery("SELECT `id` FROM `#@__shop_product` WHERE 1 = 2");
...
if($dopost == "getDetail"){
	...
	$archives = $dsql->SetQuery("SELECT `aid`, `userid`, `content`, `dtime`, `ip`, `ipaddr`, `rating`, `ischeck`, `sco1`, `pics`, `sco2`, `sco3` FROM `#@__".$action."_comment_all` WHERE `id` = ".$id.$where);
...
//更新评论信息
	...
	//会员通知
	$sql = $dsql->SetQuery("SELECT l.`id`, l.`title`, l.`pubdate`, l.`store`, c.`userid`, c.`ischeck`, c.`dtime`,c.`rid`,c.`pid` FROM `#@__".$action."_comment_all` c LEFT JOIN `#@__shop_product` l ON l.`id` = c.`aid` WHERE c.`id` = " . $id . $where);
	$ret = $dsql->dsqlOper($sql, "results");
	if($ret){
		$aid     = $ret[0]['id'];
		$title   = $ret[0]['title'];
	...
	$archives = $dsql->SetQuery("UPDATE `#@__".$action."_comment_all` SET `content` = '$content', `dtime` = '$dtime', `ip` = '$ip', `ischeck` = '$isCheck', `rating` = '$rating', `sco1` = '$sco1', `sco2` = '$sco2', `sco3` = '$sco3', `pics`='$pics' WHERE `id` = ".$id.$where1);
...
//更新评论状态
	...
	foreach($each as $val){
		...
		//会员通知
		$sql = $dsql->SetQuery("SELECT l.`id`, l.`title`, l.`pubdate`, l.`store`, c.`userid`, c.`ischeck`, c.`dtime`,c.`rid`,c.`pid` FROM `#@__".$action."_comment_all` c LEFT JOIN `#@__shop_product` l ON l.`id` = c.`aid` WHERE c.`id` = " . $val . $where);
		$ret = $dsql->dsqlOper($sql, "results");
		if($ret){
			$aid     = $ret[0]['id'];
			$title   = $ret[0]['title'];
...
//获取评论列表
	...
	if ($adminCity){
		$comSql = $dsql->SetQuery("SELECT `id` FROM `#@__shop_store` WHERE `state` = 1" . getWrongCityFilter('`cityid`', $adminCity));
		$comResult = $dsql->dsqlOper($comSql, "results");
		if($comResult) {
			$comid = array();
			foreach ($comResult as $key => $com) {
