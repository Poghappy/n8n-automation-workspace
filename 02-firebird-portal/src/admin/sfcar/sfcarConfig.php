...
checkPurview("sfcarConfig");
$dsql = new dsql($dbo);
$tpl = dirname(__FILE__) . "/../templates/sfcar";
$huoniaoTag->template_dir = $tpl; //设置后台模板目录
$templates = "sfcarConfig.html";

$action = $action == "" ? "sfcar" : $action;
...
if ($action == "delTpl") {
    ...
    if ($deldir) {
        ...
        echo '{"state": 100, "sfcar": ' . json_encode("删除成功！") . '}';
    } else {
        echo '{"state": 200, "sfcar": ' . json_encode("删除失败！") . '}';
    }
    die;
}
...
if ($type != "") {
    ...
    if ($type == "site") {
        ...
        $custom_displayConfig = serialize($displayConfig);

        if ($customChannelName == "" || $customLogo == "" || $customChannelDomain == "")
            die('{"state": 200, "sfcar": ' . json_encode("请填写完整！") . '}');

        //验证域名是否被使用
        if (!operaDomain('check', $customChannelDomain, $action, 'config'))
            die('{"state": 200, "sfcar": ' . json_encode("域名已被占用，请重试！") . '}');
    ...
    $customInc .= "\$customFabuCheckPhone = " . (int)$customFabuCheckPhone . ";\r\n";
    $customInc .= "\$custom_displayConfig = '" . $custom_displayConfig . "';\r\n";
    //模板风格
    ...
    $fp = fopen($customIncFile, "w") or die('{"state": 200, "sfcar": ' . json_encode("写入文件 $customIncFile 失败，请检查权限！") . '}');
    fwrite($fp, $customInc);
    fclose($fp);
...
if (file_exists($tpl . "/" . $templates)) {
    ...
    //获取域名信息
    $domainsfcar = getDomain($action, 'config');
    $huoniaoTag->assign('channeldomain', $domainsfcar['domain']);
    ...
    $huoniaoTag->assign('fabuCheckPhoneNames', array('需要验证', '不需要验证'));
    ...
    $huoniaoTag->assign('displayConfig', $displayConfig);
