...
//获取列表
	...
	if($sKeyword != ""){
        $where .= " AND (t.`name` like '%$sKeyword%' OR t.`phone` like '%$sKeyword%' OR l.`title` like '%$sKeyword%')";
        // $houseid = array();
	...
	$archives = $dsql->SetQuery("SELECT t.`id` FROM `#@__".$db."` t LEFT JOIN `#@__house_loupan` l ON l.`id` = t.`aid`");

	// $where.= " AND t.`hid` !=0";
	//总条数
	$totalCount = (int)$dsql->dsqlOper($archives." WHERE 1 = 1".$where, "totalCount");
	//总分页数
	$totalPage = ceil($totalCount/$pagestep);

	$atpage = $pagestep*($page-1);
	$limit = " LIMIT $atpage, $pagestep";
	$archives = $dsql->SetQuery("SELECT t.`id`, t.`aid`, t.`uid`, t.`name`, t.`phone`, t.`pubdate`, t.`state`, t.`hid` FROM `#@__".$db."` t LEFT JOIN `#@__house_loupan` l ON l.`id` = t.`aid` WHERE 1 = 1".$where.$order.$limit);
	...
	if(count($results) > 0){
		...
		foreach ($results as $key=>$value) {
            ...
            $huodongtitle = '看房团';
            if($value['hid']){
                $huodongSql = $dsql->SetQuery("SELECT `title` FROM `#@__house_huodong` WHERE `id` = ".$value['hid']);
                $huodongResult = $dsql->dsqlOper($huodongSql, "results");
                $huodongtitle = $huodongResult[0]["title"];
            }
            $list[$key]["huodongtitle"] = $huodongtitle;
