...
if ($type != "") {
    ...
    //入驻费用
        ...
        //免费套餐数量校验
        if ($trialFree != null) {
            if ((int)$trialFree['house'] < 0 || (int)$trialFree['refresh'] < 0 || (int)$trialFree['settop'] < 0) {
                $info = "请填写完整";
                die('{"state": 200, "info": ' . json_encode($info) . '}');
            }
        }
        $custom_zjuserPriceFree = serialize($trialFree);
    ...
    //增加经纪人免费套餐
    $customInc .= "\$custom_zjuserPriceFree = '".$custom_zjuserPriceFree."';\r\n";
...
if (file_exists($tpl . "/" . $templates)) {
    ...
    $huoniaoTag->assign('agentCheckNum', array('1', '0'));
    $huoniaoTag->assign('agentCheckNames', array('不需要审核', '需要审核'));
    ...
    //经纪人免费套餐
    $huoniaoTag->assign('trialFree', $custom_zjuserPriceFree ? unserialize($custom_zjuserPriceFree) : array());
