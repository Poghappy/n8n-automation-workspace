 ...
 * 管理养老机构评论
 *
 * @version        $Id: pensionCommon.php 2019-8-02 下午17:41:26 $
...
checkPurview("pensionCommon");
$dsql = new dsql($dbo);
$tpl = dirname(__FILE__)."/../templates/pension";
$huoniaoTag->template_dir = $tpl; //设置后台模板目录
$templates = "pensionCommon.html";
...
if($userType == 3){
	//查询养老机构
	$sql = $dsql->SetQuery("SELECT `id` FROM `#@__pension_store` WHERE `state` = 1" . getCityFilter('`cityid`'));
...
if($dopost == "getDetail"){
	...
	if(count($results) > 0){
		...
		$typeSql = $dsql->SetQuery("SELECT p.`title` FROM `#@__pension_store` p WHERE p.`id` = ". $results[0]["aid"]);
...
//更新评论信息
	...
	//会员通知
	$sql = $dsql->SetQuery("SELECT l.`id`, l.`title`, l.`pubdate`, l.`userid` uid, c.`userid`, c.`ischeck`, c.`dtime`,c.`rid`,c.`pid` FROM `#@__".$action."_comment_all` c LEFT JOIN `#@__pension_store` l ON l.`id` = c.`aid` WHERE c.`id` = " . $id . $where);
	$ret = $dsql->dsqlOper($sql, "results");
	if($ret){
		$aid     = $ret[0]['id'];
		$title   = $ret[0]['title'];
...
//更新评论状态
	...
	foreach($each as $val){
		...
		//会员通知
		$sql = $dsql->SetQuery("SELECT l.`id`, l.`title`, l.`pubdate`, l.`userid` uid, c.`userid`, c.`ischeck`, c.`dtime`,c.`rid`,c.`pid` FROM `#@__".$action."_comment_all` c LEFT JOIN `#@__pension_store` l ON l.`id` = c.`aid` WHERE c.`id` = " . $val.$where);
		$ret = $dsql->dsqlOper($sql, "results");
		if($ret){
			$aid     = $ret[0]['id'];
			$title   = $ret[0]['title'];
	...
	if(!empty($error)){
		...
		adminLog("更新养老机构评论状态", $id."=>".$arcrank);
		echo '{"state": 100, "info": '.json_encode("修改成功！").'}';
	}
	die;
...
//获取评论列表
	...
	if($sKeyword != ""){
		...
		//按信息标题搜索
			...
			$archives = $dsql->SetQuery("SELECT l.`id` FROM `#@__pension_store` l WHERE l.`title` like '%$sKeyword%'");
			$results = $dsql->dsqlOper($archives, "results");
			if(count($results) > 0){
				$list = array();
	...
	if(count($results) > 0){
		...
		foreach ($results as $key=>$value) {
			...
			$typeSql = $dsql->SetQuery("SELECT `title` FROM `#@__pension_store` WHERE `id` = ". $value["aid"]);
			$typename = $dsql->getTypeName($typeSql);
...
if(file_exists($tpl."/".$templates)){
	...
	$jsFile = array(
		...
		'admin/pension/pensionCommon.js'
	);
