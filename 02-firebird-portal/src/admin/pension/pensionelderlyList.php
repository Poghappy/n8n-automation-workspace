 ...
 * 管理老人信息
 *
 * @version        $Id: pensionelderlyList.php 2019-07-10 上午11:28:16 $
...
checkPurview("pensionelderlyList");
$dsql  = new dsql($dbo);
$tpl   = dirname(__FILE__)."/../templates/pension";
$huoniaoTag->template_dir = $tpl; //设置后台模板目录
$templates = "pensionelderlyList.html";
...
if($dopost == "getList"){
	...
	if($sKeyword != ""){
		$where .= " AND `elderlyname` like '%$sKeyword%'";
	}
	...
	$archives = $dsql->SetQuery("SELECT `id`, `cityid`, `elderlyname`, `sex`, `userid`, `tel`, `catid`, `state`, `rzmaxprice`, `pubdate`, `rzminprice`, `monthmaxprice`, `monthminprice`, `switch` FROM `#@__".$tab."` WHERE 1 = 1".$where);
	...
	if(count($results) > 0){
		...
		foreach ($results as $key=>$value) {
			...
			$list[$key]["elderlyname"] = $value["elderlyname"];
			$list[$key]["rzmaxprice"] = $value["rzmaxprice"];
			$list[$key]["rzminprice"] = $value["rzminprice"];
			$list[$key]["monthmaxprice"] = $value["monthmaxprice"];
			$list[$key]["monthminprice"] = $value["monthminprice"];
			$list[$key]["sex"] = $value["sex"];
			$list[$key]["switch"] = $value["switch"];
...
//删除
	...
	if(!testPurview("pensionelderlyDel")){
	...
	if($id != ""){
		...
		foreach($each as $val){
			...
			$archives = $dsql->SetQuery("SELECT `photo` FROM `#@__pension_elderly` WHERE `id` = ".$val);
			$results = $dsql->dsqlOper($archives, "results");

			delPicFile($results[0]['photo'], "delThumb", "pension");
		...
		if(!empty($error)){
			...
			checkpensionElderlyCache($id);
			adminLog("删除老人信息", join(", ", $title));
...
// 检查缓存
function checkpensionElderlyCache($id){
...
if(file_exists($tpl."/".$templates)){
	...
	$jsFile = array(
		...
		'admin/pension/pensionelderlyList.js'
	);
