 ...
 * 管理邀请记录信息
 *
 * @version        $Id: pensionInvitate.php 2019-07-18 下午20:22:45 $
...
checkPurview("pensionInvitate");
$dsql = new dsql($dbo);
$tpl = dirname(__FILE__)."/../templates/pension";
$huoniaoTag->template_dir = $tpl; //设置后台模板目录
$templates = "pensionInvitate.html";

$action = "pension_invitation";
...
if($dopost == "delAppoint"){
	...
	if(!empty($error)){
		...
		checkpensionInvitationCache($id);
		adminLog("删除邀请记录信息", $id);
...
//获取预约列表
	...
	$archives = $dsql->SetQuery("SELECT `id`, `userid`, `store`, `elderly`, `tel`, `people`, `pubdate`, `state` FROM `#@__".$action."` WHERE 1 = 1".$where);
	$results = $dsql->dsqlOper($archives, "results");
	...
	if(count($results) > 0){
		...
		foreach ($results as $key=>$value) {
			...
			$list[$key]["username"] = $value["people"];
			$list[$key]["userid"] = $value["userid"];
 ...
 */
	...
	if(!testPurview("pensionInvitate")){
...
// 检查缓存
function checkpensionInvitationCache($id){
...
if(file_exists($tpl."/".$templates)){
	...
	$jsFile = array(
		...
		'admin/pension/pensionInvitate.js'
	);
