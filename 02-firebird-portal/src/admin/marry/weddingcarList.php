...
if($dopost == "getList"){
	...
	$archives = $dsql->SetQuery("SELECT `id`, `title`, `company`, `state`, `weight`, `pubdate`, `price`, `duration`, `kilometre`, `type` FROM `#@__".$tab."` WHERE 1 = 1".$where);
...
//删除
	...
	if($id != ""){
		...
		dataAsync("marry",$async,"weddingcar");  // 删除婚车
		if(!empty($error)){
...
//更新状态
	...
	if($id != ""){
		...
		dataAsync("marry",$async,"weddingcar");  // 更新婚车
		if(!empty($error)){
			echo '{"state": 200, "info": '.json_encode($error).'}';
		}else{
			checkweddingcarCache($id);
