...
if($dopost == "getList"){
	...
	$archives = $dsql->SetQuery("SELECT `id`, `cityid`, `title`, `userid`, `tel`, `address`, `state`, `typeid`, `weight`, `pubdate`, `bind_module`, `refuse` FROM `#@__".$tab."` WHERE 1 = 1".$where);
...
//删除
	...
	if($id != ""){
		...
		dataAsync("marry",$async,"hstore");  // 婚嫁、店铺、删除
		dataAsync("marry",$async,"nhstore");  // 婚嫁、店铺、删除
		if(!empty($error)){
...
//更新状态
	...
	if($id != ""){
		...
		dataAsync("marry",$async,"hstore");  // 婚嫁、店铺、更改状态
		dataAsync("marry",$async,"nhstore");  // 婚嫁、店铺、更改状态
		if(!empty($error)){
			echo '{"state": 200, "info": '.json_encode($error).'}';
		}else{
			checkMarryStoreCache($id);
