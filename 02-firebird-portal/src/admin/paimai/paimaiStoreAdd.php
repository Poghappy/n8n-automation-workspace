 ...
 * 添加拍卖商家
 *
 * @version        $Id: paimaiStoreAdd.php 2015-8-13 下午15:35:21 $
...
$templates = "paimaiStoreAdd.html";

$tab = "paimai_store";
$dopost = $dopost ? $dopost : "save";        //操作类型 save添加 edit修改
if($dopost == "edit"){
	$pagetitle = "修改拍卖商家";
	checkPurview("paimaiStore");
}else{
	$pagetitle = "添加拍卖商家";
	checkPurview("paimaiStore");
}
...
if($dopost == "save" && $submit == "提交"){
	...
	//保存到表
    $archives = $dsql->SetQuery("INSERT INTO `#@__".$tab."` (`cityid`,`uid`, `stype`, `address`, `addrid`, `tel`, `jointime`, `state`, `wechatcode`,`shopFee`) VALUES ('$cityid','$uid', '$stype', '$address', '$addrid',  '$tel',".GetMkTime(time()).",'$state','$wechatcode','$shopFee')");
	...
	if($submit == "提交"){
        ...
        $shopFee = (float)$shopFee;
		//保存到表
		$archives = $dsql->SetQuery("UPDATE `#@__".$tab."` SET `uid` = '$uid', `stype` = '$stype', `addrid` = '$addrid', `address` = '$address', `tel` = '$tel', `state` = '$state', `cityid` = '$cityid', `wechatcode` = '$wechatcode',`shopFee` = '$shopFee' WHERE `id` = ".$id);
...
if(file_exists($tpl."/".$templates)){
	...
	$jsFile = array(
        ...
        'publicAddr.js',
        'publicUpload.js',
		'admin/paimai/paimaiStoreAdd.js'
	);
	$huoniaoTag->assign('jsFile', includeFile('js', $jsFile));

	require_once(HUONIAOINC."/config/paimai.inc.php");
	global $customUpload;
	if($customUpload == 1){
	...
	if($dopost == "edit"){
		...
		$typeSql = $dsql->SetQuery("SELECT `typename` FROM `#@__paimaitype` WHERE `id` = ". $stype);
	...
	$huoniaoTag->assign('typeListArr', json_encode($dsql->getTypeList(0, "paimaitype")));
	...
	// $huoniaoTag->assign('addrListArr', json_encode($dsql->getTypeList(0, "paimaiaddr", false)));
