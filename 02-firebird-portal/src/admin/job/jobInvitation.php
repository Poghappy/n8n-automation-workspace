...
if($dopost == "getList") {
    ...
    if ($start != "") {
        $where .= " AND i.`date` >= " . GetMkTime($start);
    }

    if ($end != "") {
        $where .= " AND i.`date` <= " . GetMkTime($end . " 23:59:59");
    }

    $archives = $dsql->SetQuery("SELECT count(*) FROM `#@__" . $tab . "` i left join `#@__job_post` p on i.`pid`=p.`id` left join `#@__job_company` c on p.`company`=c.`id` left join `#@__member` m on i.`userid`=m.`id` LEFT JOIN `#@__job_resume` r ON r.`id` = i.`rid` WHERE 1 = 1");
    ...
    $now = time();
    //待面试
    $state1 = $dsql->getOne($archives . " AND i.`state` = 1" . $where);
    //已取消
    $state2 = $dsql->getOne($archives . " AND i.`state` = 2" . $where);
    //沟通offer
    $state3 = $dsql->getOne($archives . " AND i.`state` = 3" . $where);
    //待入职
    $state4 = $dsql->getOne($archives . " AND i.`state` = 4" . $where);
    //不合适
    $state5 = $dsql->getOne($archives . " AND i.`state` = 5" . $where);
    ...
    $archives = $dsql->SetQuery("SELECT i.*,p.`title`,p.`min_salary`,p.`max_salary`,p.`salary_type`,p.`mianyi`,p.`type`,c.`title` 'ctitle',c.`contact`,m.`username`,m.`nickname`,r.`name`,r.`phone` FROM `#@__" . $tab . "` i left join `#@__job_post` p on i.`pid`=p.`id` left join `#@__job_company` c on p.`company`=c.`id` left join `#@__member` m on i.`userid`=m.`id` LEFT JOIN `#@__job_resume` r ON r.`id` = i.`rid` WHERE 1 = 1" . $where);
    ...
    if (count($results) > 0) {
        ...
        foreach ($results as $key => $value) {
            ...
            $stateArr = array(1=>'待面试',2=>'已取消',3=>'沟通offer',4=>'待入职',5=>'不合适');
            $list[$key]['state'] = $stateArr[$value['state']];
        }

        if (count($list) > 0) {
            if($do!="export"){
                echo '{"state": 100, "info": ' . json_encode("获取成功") . ', "pageInfo": {"totalPage": ' . $totalPage . ', "totalCount": ' . $totalCount . ',  "state1": ' . $state1 . ', "state2": ' . $state2 . ', "state3": ' . $state3 . ', "state4": ' . $state4. ', "state5": ' . $state5.', "time": ' . time() . '}, "jobPost": ' . json_encode($list) . '}';
            }
        } else {
            if($do!="export"){
                echo '{"state": 101, "info": ' . json_encode("暂无相关信息") . ', "pageInfo": {"totalPage": ' . $totalPage . ', "totalCount": ' . $totalCount . ',  "state1": ' . $state1 . ', "state2": ' . $state2 . ', "state3": ' . $state3 . ', "state5": ' . $state5. ', "state5": ' . $state5. '}}';
            }
        }

    } else {
        if($do!="export"){
            echo '{"state": 101, "info": ' . json_encode("暂无相关信息") . ', "pageInfo": {"totalPage": ' . $totalPage . ', "totalCount": ' . $totalCount . ',  "state1": ' . $state1 . ', "state2": ' . $state2 . ', "state3": ' . $state3 . ', "state4": ' . $state4. ', "state5": ' . $state5. '}}';
        }
    }
    if($do == "export"){
        $tit = array();
        array_push($tit, iconv('utf-8', 'gb2312//IGNORE', '面试用户'));
        ...
        $filePath = $folder.iconv("utf-8","gbk//IGNORE","招聘面试日程.csv");
//        MkdirAll($folder);
        ...
        header("Content-Disposition:attachment;filename = 招聘面试日程.csv");
