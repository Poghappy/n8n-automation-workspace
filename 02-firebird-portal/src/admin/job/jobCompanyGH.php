 ...
 * 管理招聘企业，公海客户
 *
 * @version        $Id: jobCompanyGH.php 2014-3-17 上午00:21:17 $
 * @package        HuoNiao.Job
...
checkPurview("jobCompanyGH");
$dsql  = new dsql($dbo);
$tpl   = dirname(__FILE__)."/../templates/job";
$huoniaoTag->template_dir = $tpl; //设置后台模板目录
$templates = "jobCompanyGH.html";
...
if($dopost == "getList"){
    ...
    $where = " AND c.`state` = 1 AND c.`admin` = 0" . getCityFilter('c.`cityid`');
	...
	$archives = $dsql->SetQuery("SELECT c.`userid`,c.`combo_id`,c.`combo_enddate`,c.`cityid`,c.`id`, c.`title`, c.`logo`, c.`userid`, c.`people`, c.`contact`, c.`addrid`, c.`state`, c.`pubdate`,c.`changeState`,c.`certification`,c.`tag` FROM `#@__".$tab."` c left join `#@__member` m on m.`id`=c.`userid` WHERE 1 = 1".$where);
...
//绑定管理员·
	...
	if($id != ""){
		...
		if(!empty($error)){
			...
			adminLog("领取招聘企业管理员", $id);
			echo '{"state": 100, "info": '.json_encode("领取成功").'}';
		}
	}
	die;
}
