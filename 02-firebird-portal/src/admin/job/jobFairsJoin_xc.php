...
if($dopost != ""){
	$templates = "jobFairsJoinAdd_xc.html";
    $pagetitle = "新增参会信息";
	//js
	$jsFile = array(
        'publicUpload.js',
		...
		'admin/job/jobFairsJoinAdd_xc.js',
		'vue/vue.min.js'
	);
	$huoniaoTag->assign('jsFile', includeFile('js', $jsFile));
}else{
	$templates = "jobFairsJoin_xc.html";
...
/*    if ($cityid){
    ...
    if($sKeyword != ""){
        ...
        $_where = array("f.`title` like '%$sKeyword%'", "j.`company` like '%$sKeyword%'");
		...
		if($result){
			...
			if(!empty($fid)){
                array_push($_where, "f.`fid` in (".join(",", $fid).")");
			}
		}

        if(is_numeric($sKeyword)){
            array_push($_where, "f.`id`=$sKeyword");
        }
    ...
    $baseSql = $dsql::SetQuery("SELECT j.`id`, j.`fid`, j.`cid`, j.`company`, j.`phone`,j.`state`,j.`pubdate`,f.`type`,j.`seat` FROM `#@__".$tab."` j LEFT JOIN `#@__job_fairs` f ON j.`fid`=f.`id` WHERE 1 = 1");
...
/*			$list[$key]["addrid"] = $fname[0]["addr"];
			...
			elseif($state==2){
			    $state = "<span style='color: red'>审核拒绝</span>";
            }
			...
			if($cid){
			    $name = $dsql::SetQuery("select `people`,`contact`,`title` from `#@__job_company` where `id`=$cid");
...
elseif($dopost == "Add"){
    ...
    if($submit == "提交"){
        ...
        $state = (int)$state;
        $cid = (int)$cid;
        $archives = $dsql->SetQuery("INSERT INTO `#@__".$tab."` (`fid`,`cid`,`company`,`phone`,`state`,`seat`,`jobs`,`pubdate`) VALUES ('$fid',$cid,'$company','$phone','$state','$seat','$jobs','".GetMkTime(time())."')");
...
elseif($dopost == "edit"){
	...
	if($submit == "提交"){
        ...
        $fid = (int)$fid;
        $state = (int)$state;
		$archives = $dsql->SetQuery("UPDATE `#@__".$tab."` SET `fid` = $fid,  `cid`=$cid, `company`='$company', `phone`='$phone', `state` = $state, `seat`='$seat',`jobs`='$jobs' WHERE `id` = ".$id);
...
elseif($dopost == "checkFairs"){
	...
	if(!empty($key)){
		...
		$sql = $dsql->SetQuery("SELECT `id`, `title` FROM `#@__job_fairs` WHERE `type`=1 and `enddate`>unix_timestamp(current_timestamp) and `title` like '%$key%'".$where." LIMIT 0, 10");
		$result = $dsql->dsqlOper($sql, "results");
		if($result){
