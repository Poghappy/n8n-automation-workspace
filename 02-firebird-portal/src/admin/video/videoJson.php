...
if($action == ""){
	...
	if($dopost == "pic"){
		$archives = $dsql->SetQuery("SELECT `id`, `cityid`, `title`, `litpic`, `color`, `flag`, `redirecturl`, `typeid`, `weight`, `writer`, `arcrank`, `pubdate` FROM `#@__".$dopost."list` WHERE `del` = $del".$where);
	}else{
		$archives = $dsql->SetQuery("SELECT `id`, `cityid`, `title`, `color`, `flag`, `redirecturl`, `typeid`, `weight`, `writer`, `arcrank`, `pubdate` FROM `#@__".$dopost."list` WHERE `del` = $del".$where);
	}
	...
	if(count($results) > 0){
		...
		foreach ($results as $key=>$value) {
			...
			//查询收费次数和金额
            $sql = $dsql::SetQuery("select count(`id`) 'count',sum(`amount`) 'amount' from `#@__video_order` where `aid`={$value['id']}");
            $list[$key]["charge"] = $dsql->getArr($sql);
